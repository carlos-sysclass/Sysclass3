	...
	*/
	...
	protected function createTranslateArrayXML($fromLanguage, $toLanguage, $inputStrArr, $startIndex, $contentType = 'text/html') {
		...
		foreach ($inputStrArr as $index => $inputStr) {
			...
			//$count++;
			$stringToAdd = "<string xmlns=\"http://schemas.microsoft.com/2003/10/Serialization/Arrays\">" . $this->wrapXmlString($inputStr, $contentType) . "</string>";
			if ((mb_strlen($stringToAdd) + $currentSize) > 10000) {
				// break here
				$stopIndex = $index;
				break;
			}
