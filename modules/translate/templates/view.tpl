				...
				<div class="form-group">
					...
					<select class="select2-me form-control" name="src_language" data-placeholder="Select an option.">
						{foreach $T_LANGUAGES as $lang}
							<option value="{$lang.id}" {if $lang.id == $T_SYSTEM_LANGUAGE}selected="selected"{/if}>{$lang.local_name} ({$lang.name})</option>
						{/foreach}
					</select>
				</div>
			</div>
			...
			<div class="col-md-5 pull-right">
				<div class="form-group">
					...
					<select class="select2-me form-control" name="dst_language">
						{foreach $T_LANGUAGES as $lang}
							<option value="{$lang.id}" {if $lang.id == $T_USER_LANGUAGE}selected="selected"{/if}>{$lang.local_name} ({$lang.name})</option>
						{/foreach}
					</select>
				</div>
			</div>
