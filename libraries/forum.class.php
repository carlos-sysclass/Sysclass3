 ...
 *
 */
class f_forums extends MagesterEntity {
    ...
    public static function getForumTree($forums = false) {
        ...
        foreach ($forums as $key => $forum) {
            ...
            $forumTree[$parent_id][] = $forum['id']; //Append to the tree array, at the forum id index, the id of its child
            $forumTree[$forum['id']] = array();
            //$forums[$forum['id']] = $forum; //Copy node to forums, which will be used later as forums source
    ...
    public function persist() {
        ...
        if (mb_strlen($fields['comments']) > 3) {
            MagesterSearch::insertText(strip_tags($fields['comments']), $this->{$this->entity}['id'], "f_forums", "data");
        }
    }
    ...
    public static function create($fields = array()) {
        ...
        // Timelines add event
        MagesterEvent::triggerEvent(
            array(
                "type" => MagesterEvent::NEW_FORUM,
 ...
 *
 */
class f_topics extends MagesterEntity {
    ...
    public static function create($fields = array()) {
        ...
        if (mb_strlen($message_fields['body']) > 3) {
            MagesterSearch::insertText(strip_tags($message_fields['body']), $new_id, "f_messages", "data");
        }
        // Timelines add event
        MagesterEvent::triggerEvent(
            array(
                "type" => MagesterEvent::NEW_TOPIC,
                "users_LOGIN" => $_SESSION['s_login'],
                "lessons_ID" => isset($GLOBALS['currentLesson']) ? $GLOBALS['currentLesson']->lesson['id'] : null,
                "lessons_name" => isset($GLOBALS['currentLesson']) ? $GLOBALS['currentLesson']->lesson['name'] : null,
                "entity_ID" => $new_id,
...
class f_messages extends MagesterEntity {
    ...
    public function persist() {
        ...
        if (mb_strlen($fields['body']) > 3) {
            MagesterSearch::insertText(strip_tags($fields['body']), $this->{$this->entity}['id'], "f_messages", "data");
        }
    }
...
class f_poll extends MagesterEntity {
    ...
    public static function create($fields = array()) {
        ...
        // Timelines add event
        MagesterEvent::triggerEvent(array("type" => MagesterEvent::NEW_POLL,
            "users_LOGIN" => $_SESSION['s_login'],
            "lessons_ID" => isset($GLOBALS['currentLesson']) ? $GLOBALS['currentLesson']->lesson['id'] : null,
            "lessons_name" => isset($GLOBALS['currentLesson']) ? $GLOBALS['currentLesson']->lesson['name'] : null,
            "entity_ID" => $new_id,
