 ...
 */
class MagesterDirectionException extends Exception
{
 /**
 ...
 * This class represents a direction in SysClass
 ...
 */
class MagesterDirection extends ArrayObject
{
 /**
 ...
 /**
	 ...
	 * MagesterDirection object, based on the direction values
 ...
 /**
	 ...
	 * @param boolean $returnObjects Whether to return MagesterLesson objects or a simple array

	 * @param boolean $subDirections Whether to return subDirections lessons as well

	 * @return array An array of lesson ids/names pairs or MagesterLesson objects
 ...
 /**
	 ...
	 * @param boolean $returnObjects Whether to return MagesterCourse objects or a simple array

	 * @param boolean $subDirections Whether to return subDirections courses as well

	 * @return array An array of course ids/names pairs or MagesterCourse objects
 ...
 /**
	 ...
	 * MagesterDirection :: createDirection($fields);
	 ...
	 * @return MagesterDirection The new direction
 ...
 /**
	 ...
	 * This function is the same as MagesterDirection :: delete(),
	 ...
	 *   MagesterDirection :: delete(32);					 //32 is the category id
 ...
 * This class represents the directions tree and extends MagesterTree class
 ...
 */
class MagesterDirectionsTree extends MagesterTree
{
 /**
 ...
 public function reset()
 {
  ...
  foreach ($directions as $node) { //Assign previous direction ids as keys to the previousNodes array, which will be used for sorting afterwards
   $nodes[$node['id']] = new MagesterDirection($node); //We convert arrays to array objects, which is best for manipulating data through iterators
  }
  $rejected = array();
  $tree = $nodes;
 ...
 /**
	 ...
	 * @param array $lessons An array of MagesterLesson Objects

	 * @param array $courses An array of MagesterCourse Objects
  ...
  foreach ($courses as $id => $course) {
   ...
   if ($options['catalog'] && !$displayCatalogEntry[$course -> course['id']]) { //Remove inactive courses
    ...
    if ($courses[$id] -> course['instance_source']) {
     $instanceSource = new MagesterCourse($courses[$id] -> course['instance_source']);
...
*/
 ...
 private function printCategoryTitle($iterator, $display, $imageString, $classString)
 {
  ...
  foreach (new MagesterNodeFilterIterator(new ArrayIterator($this -> getNodeChildren($current), RecursiveIteratorIterator :: SELF_FIRST)) as $key => $value) {
   $children[] = $key;
  }
  $treeString .= '
