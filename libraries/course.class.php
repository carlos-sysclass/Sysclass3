 ...
 *
 */
class MagesterCourseException extends Exception
{
...
/**
 * This class represents a course in SysClass
 *
 * @package SysClass
 * @since 3.5.0
 */
class MagesterCourse
{
	/**
     ...
     * <br/>Example:
     * <code>
     * $course = new MagesterCourse(5);       //create object for course with id 5
     * </code>
     *
    ...
    private function initializeDataFromSource($source)
    {
    	if (is_array($source)) {
    		...
    		$source = sC_getTableData("courses", "*", "id = $source");
    		if (empty($source)) {
    			throw new MagesterCourseException(_COURSEDOESNOTEXIST, MagesterCourseException :: COURSE_NOT_EXISTS);
    		}
    ...
    private function setCategoryId()
    {
    	if ($this -> course['instance_source']) {
    		...
    		$result = sC_getTableData("directions", "id", "active=1", "", "", 1);
    		if (!empty($result)) {
    ...
    private function buildPriceString()
    {
    	if ($this -> validateFloat($this -> course['price'])) { //Create the string representing the course price
    		...
    		$this -> course['price_presencial_string'] = formatPrice($this -> course['price_presencial'], $recurring);
    		$this -> course['price_web_string'] = formatPrice($this -> course['price_web'], $recurring);
    	} else {
    		$this -> course['price_string'] =
    		$this -> course['price_presencial_string'] =
   			$this -> course['price_web_string']	=
   			formatPrice(0);
    	}
    }
    ...
    public function getCourseLessons($constraints = array())
    {
    	...
    	if (!isset($constraints['return_objects']) || $constraints['return_objects'] == true) {
    		return MagesterCourse :: convertDatabaseResultToLessonObjects($result);
    	} else {
    ...
    public function countCourseLessons($constraints = array())
    {
    	...
    	$result = sC_countTableData($from, "l.id",
    	implode(" and ", $where));
    ...
    /**
     * Return an array of MagesterLesson objects, based on the specified constraints. If any of the lessons
    ...
    public function getLessonScheduleInCourse($lesson)
    {
    	...
    	$result = sC_getTableData("lessons_to_courses", "start_date, end_date", "courses_ID=".$this -> course['id']." and lessons_ID=".$lesson -> lesson['id']);
    	return $result[0];
    }
    ...
    public function addLessons($lessons)
    {
    	...
    	$result = sC_getTableDataFlat("lessons_to_courses", "lessons_ID", "courses_ID=".$this -> course['id']); //We don't call getCourseLessons() because we need all and every lesson, so this is faster
    	foreach ($lessonObjects as $key => $lesson) {
    		if (!in_array($key, $result['lessons_ID'])) {
    			sC_insertTableData("lessons_to_courses", array('courses_ID' => $this -> course['id'],
                                    'lessons_ID' => $key,
    	...
    	return MagesterCourse::convertLessonObjectsToArrays($this -> getCourseLessons());
    }
    ...
    public function removeLessons($lessons)
    {
    	...
    	foreach ($lessonObjects as $key => $lesson) {
    		...
    		sC_deleteTableData("lessons_to_courses", "courses_ID=".$this -> course['id']." and lessons_ID=".$key);
    ...
    {
    	$result = sC_getTableDataFlat("lessons_to_courses lc", "lc.lessons_ID, count(lc.lessons_ID)", "", "", "lc.lessons_ID");
     ...
     *
     * @param MagesterLesson $lesson The lesson to add users to
     * @since 3.6.1
     * @access private
     */
    ...
    public function insertCourseSkill()
    {
    	...
    	$courseSkillId = sC_insertTableData("module_hcd_skills", array("description" => _KNOWLEDGEOFCOURSE . " ". $this -> course['name'], "categories_ID" => -1));

    	// Insert question to course skill records for all course questions
    	$questions = sC_getTableData("questions", "id", "lessons_ID in ('". implode("','", array_keys($this->getCourseLessons())) . "')");
    	$insert_string = "";
    	...
    	return sC_insertTableData("module_hcd_course_offers_skill", array("courses_ID" => $this -> course['id'], "skill_ID" => $courseSkillId));
    }
     ...
     *
     * @param MagesterLesson $lesson The lesson to retrieve questions for
     * @since 3.6.1
     * @access private
     */
    private function addLessonQuestionsToCourseSkill($lesson)
    {
    	$lessonQuestions = sC_getTableDataFlat("questions", "id", "lessons_ID = ". $lesson ->lesson['id']);
    	...
    	sC_insertTableDataMultiple("questions_to_skills", $fields);
    }
    /**
    ...
    private function removeLessonQuestionsFromCourseSkill($lesson)
    {
    	...
    	if (!empty($lessonQuestions['id'])) {
    		sC_deleteTableData("questions_to_skills", "questions_id IN ('".implode("','",$lessonQuestions['id'])."') AND skills_ID = ".$courseSkill['skill_ID']);
    	}
    }
    /**
     * Get course users
     ...
     * </code>
     *
     * @param boolean $returnObjects Whether to return MagesterUser Objects
     * @return array An array of users or MagesterUser objects
     * @since 3.5.0
     * @access public
     ...
     * @return mixed An array of users or MagesterUser objects
     * @since 3.6.1
     * @access public
     * @todo: Replace with getCourseUsersXXX()
     */
    ...
    public function getProfessorUsers($returnObjects = false, $constraints = array())
    {
    	...
    	foreach ($courseUsers as $key => $value) {
    		...
    		if (!MagesterUser::isProfessorRole($value['role'])) {
    			unset($courseUsers[$key]);
    		}
    	}
    	return $courseUsers;
     ...
     *
     * @param mixed $user a login or an MagesterUser object
    ...
    public function getCourseUsersAggregatingResults($constraints = array())
    {
    	...
    	$from = MagesterCourse :: appendTableFiltersUserConstraints($from, $constraints);
    	...
    	/*
    	 ...
    	 #endif
    	 */
    	$result = sC_getTableData($from, $select, implode(" and ", $where), $orderby, $groupby, $limit);
    	if (!isset($constraints['return_objects']) || $constraints['return_objects'] == true) {
    ...
    public function countCourseUsersAggregatingResults($constraints = array())
    {
    	...
    	$select = "u.login";
    	$groupby = "r.users_LOGIN";
    	$result = sC_countTableData($from, $select, implode(" and ", $where), false, $groupby);
    	return $result[0]['count'];
    }
    /**
    ...
    public function getCourseUsers($constraints = array())
    {
    	...
    	$result = sC_getTableData("users u, users_to_courses uc", $select,
    ...
    public function countCourseUsers($constraints = array())
    {
    	...
    	$where[] = "u.login=uc.users_LOGIN and uc.courses_ID='".$this -> course['id']."' and uc.archive=0";
    	$result = sC_countTableData("users u, users_to_courses uc", "u.login", implode(" and ", $where));
    	return $result[0]['count'];
    }
    /**
    ...
    public function getCourseUsersAggregatingResultsIncludingUnassigned($constraints = array())
    {
     ...
     group by users_LOGIN) r on u.login=r.users_login";
    	$result = sC_getTableData($from, "u.*, r.*",
    ...
    /**
     * Get the possible roles for a user in a course. This function caches MagesterLessonUser :: getLessonsRoles() results
    ...
    private function initializeUsers($constraints = array())
    {
    	...
    	list($where, $limit, $orderby) = MagesterCourse :: convertCourseUserConstraintsToSqlParameters($constraints);
    ...
    public function getNonUsers($returnObjects = false)
    {
    	...
    	$result = sC_getTableData("($subquery) s", "s.*, s.has_course is null");
    	$users = array();
    	foreach ($result as $user) {
    		$user['user_types_ID'] ? $user['role'] = $user['user_types_ID'] : $user['role'] = $user['basic_user_type'];
    		$returnObjects ? $users[$user['login']] = MagesterUserFactory :: factory($user['login']) : $users[$user['login']] = $user;
    	}
    	return $users;
    }
    /**
    ...
    private function addUsersToCourse($usersData, $archivedCourseUsers = false)
    {
    	...
    	foreach ($usersData as $value) {
          ...
          'to_timestamp' => 0);
    			...
    			//sC_updateTableData("users_to_lessons", array("active" => 1, "archive" => 0), "users_LOGIN='".$value['login']."' and lessons_ID=".$this -> lesson['id']);
    		} else {
    	...
    	if (!empty($newUsers)) {
    		sC_insertTableDataMultiple("users_to_courses", $fields);
    	}
    	if (!defined(_DISABLE_EVENTS) || _DISABLE_EVENTS !== true) {
    		foreach ($usersData as $value) {
    			$event = array("type" => MagesterUser::isStudentRole($value['role']) ? MagesterEvent::COURSE_ACQUISITION_AS_STUDENT : MagesterEvent::COURSE_ACQUISITION_AS_PROFESSOR,
    ...
    {
    	$result = sC_getTableDataFlat("users_to_courses", "users_LOGIN", "archive!=0 and courses_ID=".$this->course['id']);
    	if (empty($result)) {
    		return array();
    	} else {
    ...
    public function addUsers($users, $userRoles = 'student', $confirmed = true, $course_type = 'Presencial')
    {
    	...
    	$userRoles = MagesterUser::verifyRolesList($userRoles, sizeof($users));
    	foreach ($userRoles as $key => $value) {
    		if (!MagesterUser::isStudentRole($value) && !MagesterUser::isProfessorRole($value)) {
    	...
    	$result = sC_getTableData("users_to_courses uc, users u", "uc.users_LOGIN, uc.archive, uc.user_type, uc.to_timestamp, u.archive as user_archive, uc.modality_id", "u.login=uc.users_LOGIN and uc.courses_ID=".$this->course['id']);
    	$courseUsers = array();
    	$courseRoles = $this -> getPossibleCourseRoles();
    	$courseStudents = 0;
    	foreach ($result as $value) {
    		$courseUsers[$value['users_LOGIN']] = $value;
    		if (!$value['user_archive'] && !$value['archive'] && MagesterUser::isStudentRole($value['user_type'])) {
    			$courseStudents++;
    		}
    	}
    	 ...
    	 */
    	$result = sC_getTableData("lessons_to_courses lc left outer join users_to_lessons ul on lc.lessons_ID=ul.lessons_ID",
    	...
    	$result = sC_getTableData("projects", "id, lessons_ID", "auto_assign=1 and deadline < ".time()." and lessons_ID in (select lessons_ID from lessons_to_courses where courses_ID=".$this -> course['id'].")");
    	foreach ($result as $value) {
    	...
    	foreach ($users as $key => $user) {
			...
			'course_type'	=> empty($course_type) ? "Via Web" : $course_type,
			'modality_id'	=> is_numeric($course_type) ? $course_type : 0,
			'completed' => 0,
			'score' => 0,
			'issued_certificate' => '',
			'comments' => '',
         ...
         'from_timestamp' => $confirmed ? time() : 0);
    				...
    				sC_updateTableData("users_to_lessons", $fields, "users_LOGIN='".$user."' and lessons_ID=".$id);
    			}
    		}
    	}
    	if (!empty($newUsers)) {
    		sC_insertTableDataMultiple("users_to_courses", $newUsers);
    	}
    	if (!empty($newLessonUsers) && !$ignoreNewLessons) {
    		sC_insertTableDataMultiple("users_to_lessons", $newLessonUsers);
    	}
    	...
    	if (!defined(_DISABLE_EVENTS) || _DISABLE_EVENTS !== true) {
    		foreach ($eventArray as $value) {
    			$event = array("type" => MagesterUser::isStudentRole($value['user_type']) ? MagesterEvent::COURSE_ACQUISITION_AS_STUDENT : MagesterEvent::COURSE_ACQUISITION_AS_PROFESSOR,
    	...
    	// GET LIST OF MODULES AND CALL onNewUserOnCourse
    	...
    	foreach ($cached_modules as $module) {
    		$module -> onNewUserOnCourse($value['users_LOGIN'], $this -> course['id']);
    	}
    }
    /**
    ...
    public function removeUsers($users)
    {
    	...
    	foreach ($users as $user) {
    		sC_deleteTableData("users_to_courses", "users_LOGIN='$user' and courses_ID=".$this -> course['id']);
    ...
    private function removeUsersFromCourseLessons($users)
    {
    	if (sizeof($users) == 1) {
    		...
    		$lessonsToCourses = MagesterLesson::countLessonsOccurencesInCoursesForAllUsers();
    	}
     ...
     *
     * Usada para incluir ou altera a turma do aluno dentro do curso.
     * <code>
     * $courseObject -> putUserInClass('jdoe', 18);   //Archive user with login 'jdoe'
     * </code>
     *
     * @param string $user_login the user login to archive
     * @param integer $classe_id the id of class to set
     ...
     * <br/>Example:
     * <code>
     * $course = new MagesterCourse(32);
     * $course -> confirm('jdoe');
     * </code>
     *
     * @param mixed $login Either the user login, or an MagesterLessonUser object
     * @since 3.5.2
     * @access public
     */
    ...
    private static function convertArgumentToGroupId($group)
    {
    	...
    	} elseif (!sC_checkParameter($group, 'id')) {
    ...
    public function persist()
    {
    	...
    	sC_updateTableData("courses", $fields, "id=".$this -> course['id']);
    	if (!defined(_DISABLE_SEARCH) || _DISABLE_SEARCH !== true) {
    		MagesterSearch :: removeText('courses', $this -> course['id'], '');
    		MagesterSearch :: insertText($fields['name'], $this -> course['id'], "courses", "title");
    	}
    	return true;
    }
    /**
    ...
    private static function validateAndSanitizeCourseFields($courseFields)
    {
    	...
    	$fields = array(
			...
			'enable_presencial' => self :: validateAndSanitize($courseFields['enable_presencial'], 'boolean'),
			'price_presencial' => self :: validateAndSanitize($courseFields['price_presencial'], 'float'),
			'enable_web' => self :: validateAndSanitize($courseFields['enable_web'], 'boolean'),
			'price_web' => self :: validateAndSanitize($courseFields['price_web'], 'float'),
    ...
    public static function validateAndSanitize($field, $type)
    {
    	try {
    		...
    		if ($e -> getCode() == MagesterCourseException::INVALID_PARAMETER) {
    ...
    {
    	$result = sC_getTableData("lessons", "*", "originating_course=".$this -> course['id']);
    	foreach ($result as $value) {
    ...
    public function unarchive()
    {
    	...
    	if (in_array($this -> course['directions_ID'], $result['id'])) { // If the original category exists, no problem
    		...
    		throw new MagesterLessonException(_NOCATEGORIESDEFINED, MagesterLessonException::CATEGORY_NOT_EXISTS);
    ...
    public function delete()
    {
    	...
    	$courseUsers = sC_getTableDataFlat("users_to_courses", "users_LOGIN", "courses_ID=".$this -> course['id']);
    	...
    	sC_deleteTableData("courses", "id=".$this -> course['id']);
    	MagesterSearch :: removeText('courses', $this -> course['id'], '');
    }
    /**
    ...
    {
    	$result = sC_getTableData("courses", "*", "instance_source=".$this -> course['id']);
    	foreach ($result as $value) {
     ...
     * <br/>Example:
     * <code>
     * $instance = MagesterCourse :: createInstance(43);
     * </code>
     *
     * @param mixed $instanceSource Either a course id or an MagesterCourse object.
     * @return MagesterCourse The new course instance
     * @since 3.6.1
     * @access public
     * @static
     */
    ...
    private static function createDbEntryForInstance($instanceSource)
    {
    	...
    	$result = sC_getTableData("courses", "*", "id=".$instanceSource -> course['id']); //Get all the fields of the course directly from the database
    	unset($result[0]['id']);
    	$result[0]['name'] .= ' ('._INSTANCE.' #'.($currentSourceInstances+1).")";
    	$result[0]['instance_source'] = $instanceSource -> course['id'];
    	$result[0]['created'] = time();
    	$instance = MagesterCourse :: createCourse($result[0]);
    ...
    {
    	$result = sC_getTableDataFlat("lessons_to_courses lc, lessons l", "l.id, l.instance_source", "l.id=lc.lessons_ID and lc.courses_ID=".$instanceSource -> course['id']);
    	...
    	foreach ($instanceSourceLessons as $key => $foo) { //Do this to get the lessons in the correct order
    		...
    		if ($value) {
    			$lessonInstance = MagesterLesson :: createInstance($value, $instance -> course['id']);
     ...
     * </code>
     *
     * @param mixed $login The user to revoke certificate for, either a login string or an MagesterUser object
    ...
    public function revokeCertificate($login)
    {
    	...
    	$where = "users_LOGIN='$login' and courses_ID=".$this -> course['id'];
    	self::persistCourseUsers($fields, $where, $this -> course['id'], $login);
    	MagesterEvent::triggerEvent(array("type" => MagesterEvent::COURSE_CERTIFICATE_REVOKE,
          "users_LOGIN" => $login,
          "lessons_ID" => $this -> course['id'],
          "lessons_name" => $this -> course['name']));
    	return true;
    ...
    public function issueCertificate($login, $certificate)
    {
    	...
    	MagesterEvent::triggerEvent(array("type" => MagesterEvent::COURSE_CERTIFICATE_ISSUE,
          "users_LOGIN" => $login,
          "lessons_ID" => $this -> course['id'],
          "lessons_name" => $this -> course['name'],
    ...
    public function toHTML($lessons = false, $options = array())
    {
    	...
    	if ($this -> course['start_date'] > time()) {
    		...
    		if ($this -> course['remaining'] > 0) {
    			$courseString .= '<span style = "vertical-align:middle">&nbsp;('.sC_convertIntervalToTime($this -> course['remaining'], true).' '.mb_strtolower(_REMAINING).')</span>';
    		} else {
    ...
    public function getBranches($only_own = false)
    {
    	if (!isset($this -> branches) || !$this -> branches) {
    		...
    		$branches = sC_getTableData("module_hcd_branch LEFT OUTER JOIN module_hcd_branch as branch1 ON module_hcd_branch.father_branch_ID = branch1.branch_ID LEFT OUTER JOIN module_hcd_course_to_branch ON (module_hcd_course_to_branch.branches_ID = module_hcd_branch.branch_ID AND module_hcd_course_to_branch.courses_ID='".$this -> course['id']."')", "module_hcd_branch.*, module_hcd_branch.branch_ID as branches_ID, module_hcd_course_to_branch.courses_ID, branch1.name as father","");
    ...
    public function toSelect()
    {
    	...
    	foreach ($courseLessons as $lessonId => $value) {
    		$eligible[$lessonId] = new MagesterLesson($lessonId);
    		$eligible[$lessonId] -> eligible = $value;
     ...
     * <br/>Example:
     * <code>
     * $course = new MagesterCourse(13);						//Instantiate course with id 13
     * $exportedFile = $course -> export();					//Export course
     * </code>
     *
     * @return MagesterFile The exported file
     * @since 3.5.2
     * @access public
     */
    ...
    private function createCourseTempDirectory()
    {
    	...
    	if (is_dir($courseTempDir)) { //If the user's temp directory does not exist, create it
    		$foo = new MagesterDirectory($courseTempDir);
    		$foo -> delete();
    	}
    ...
    {
    	$data = array();
    	$data['courses'] = sC_getTableData("courses", "*", "id=".$this -> course['id']);
    ...
    private function createCourseExportFile($courseTempDir)
    {
    	...
    	$file = new MagesterFile(current($newList));
    	$newFileName = MagesterFile :: encode($this -> course['name']).'.zip';
    	if (!sC_checkParameter($newFileName, 'file')) {
    		$newFileName = $file['name'];
    	}
    	$newFileName = str_replace(array('"', '>', '<', '*', '?', ':'), array('&quot;', '&gt;', '&lt;', '&#42;', '&#63;', '&#58;'), $newFileName);
    	$file -> rename($userTempDir['path'].'/'.$newFileName, true);
    	//$file   -> rename($userTempDir['path'].'/'.MagesterFile :: encode($this -> course['name']).'.zip', true);
     ...
     * <br/>Example:
     * <code>
     * $course = new MagesterCourse(23);					//Instantiate course with id 23
     * $file = $course -> export();						//Export course to a file
     * $newCourse = new MagesterCourse(43);				//Instantiate course with id 43
     ...
     * </code>
     *
     * @param mixed $file An MagesterFile object or a path to a file to uncompress
     ...
     * @return MagesterCourse The course itself
     * @since 3.5.2
     * @access public
     */
     ...
     * data into an array
     *
     * @param MagesterFile $file The exported file
     ...
     *
     * @param array $data The exported course data
     * @param MagesterFile $courseFile The file of the exported course
     * @since 3.6.1
     * @access private
     */
    private function importLessonsToCourse($data, $courseFile)
    {
    	...
    	foreach ($data['lessons_to_courses'] as $value) {
    		...
    		$lessonFile = new MagesterFile($courseFile['directory'].'/'.$value['lessons_ID'].'_exported.zip');
     ...
     * @param array $courseInformation The information to display (According to the MagesterCourse :: getInformation() format)
     * @since 3.5.0
     * @access public
     * @todo refactor
     */
     ...
     * $course = MagesterCourse :: createCourse($fields);
     * </code>
     *
     * @param array $fields The new fields
     * @return MagesterCourse the new course
     * @since 3.5.0
     * @access public
     */
    public static function createCourse($fields)
    {
    	...
    	$newId = sC_insertTableData("courses", $fields);
    	...
    	MagesterSearch :: insertText($fields['name'], $newId, "courses", "title");
    	$course = new MagesterCourse($newId);
    	return $course;
    }
    /**
     ...
     * <br/>Example:
     * <code>
     * $lessons = _MagesterCourse :: getCourses();
     * </code>
     *
     * @param boolean $returnObjects whether to return MagesterCourse objects
    ...
    public static function getCoursesWithPendingUsers($constraints = array())
    {
    	...
    	$result = sC_getTableData("users u, courses c, users_to_courses uc", "c.*, uc.users_LOGIN", implode(" and ", $where), $orderby, $groupby, $limit);
    	return $result;
    }
    ...
    public static function getCoursesWithSpecificUserParticipation($constraints = array(), $login)
    {
    	...
    	$result = sC_getTableData("courses c left outer join ($innerQuery) r on c.id=r.courses_ID", "c.*,r.*,r.courses_ID is not null as has_course, (select count( * ) from courses l where instance_source=c.id) as has_instances", implode(" and ", $where), $orderby, $groupby, $limit);
    ...
    public static function getAllCourses($constraints = array())
    {
    	...
    	$select['num_students'] = MagesterCourse :: setCourseUserSelection($constraints);
    	$select = MagesterCourse :: convertCourseConstraintsToRequiredFields($constraints, $select);
    	list($where, $limit, $orderby) = MagesterCourse :: convertCourseConstraintsToSqlParameters($constraints);
    	//$result = sC_getTableData("courses c", $select, implode(" and ", $where), $orderby, false, $limit);
    	...
    	$result = sC_getTableData("courses, ($sql) t", implode(",", $from), "courses.id=t.id");
    ...
    public static function countAllCourses($constraints = array())
    {
    	...
    	$result = sC_countTableData("courses c", "c.id", implode(" and ", $where));
    	return $result[0]['count'];
    }
    ...
    private static function addWhereConditionToLessonConstraints($constraints)
    {
    	...
    	if (isset($constraints['filter']) && sC_checkParameter($constraints['filter'], 'text')) {
    		$result = sC_describeTable("lessons");
    		$tableFields = array();
    		foreach ($result as $value) {
    ...
    private static function addWhereConditionToCourseConstraints($constraints)
    {
    	...
    	if (isset($constraints['ies_id'])) {
    		is_array($constraints['ies_id']) ?
    		$where[] = sprintf("c.ies_id IN (%s)", implode(', ', $constraints['ies_id'])) :
    		$where[] = sprintf("c.ies_id = %d", $constraints['ies_id']);
    	}
    	...
    	if (is_array($constraints['table_filters'])) {
    		foreach ($constraints['table_filters'] as $constraint) {
    ...
    public function assignSkill($skill_ID, $specification)
    {
    	...
    	if ($this -> skills[$skill_ID]['courses_ID'] == "") {
    		sC_insertTableData("module_hcd_course_offers_skill", array("skill_ID" => $skill_ID, "courses_ID" => $this -> course['id'], "specification" => $specification));
    		$this -> skills[$skill_ID]['courses_ID'] = $this -> course['id'];
    		$this -> skills[$skill_ID]['specification'] = $specification;
    	} else {
    		sC_updateTableData("module_hcd_course_offers_skill", array("specification" => $specification), "skill_ID = '".$skill_ID."' AND courses_ID = '". $this -> course['id'] ."'");
    		$this -> skills[$skill_ID]['specification'] = $specification;
    	}
    	return true;
    }
    ...
    public function removeSkill($skill_ID)
    {
    	...
    	if ($this -> skills[$skill_ID]['courses_ID'] == $this -> course['id']) {
    		sC_deleteTableData("module_hcd_course_offers_skill", "skill_ID = '".$skill_ID."' AND courses_ID = '". $this -> course['id'] ."'");
    ...
    public function assignBranch($branch_ID)
    {
    	...
    	if ($this -> branches[$branch_ID]['courses_ID'] == "") {
    		sC_insertTableData("module_hcd_course_to_branch", array("branches_ID" => $branch_ID, "courses_ID" => $this -> course['id']));
    ...
    public function removeBranch($branch_ID)
    {
    	...
    	if ($this -> branches[$branch_ID]['courses_ID'] == $this -> course['id']) {
    		sC_deleteTableData("module_hcd_course_to_branch", "branches_ID = '".$branch_ID."' AND courses_ID = '". $this -> course['id'] ."'");
     ...
     * <br>Example:
     * <code>
     * $course = new MagesterCourse(43);
     * $instances = $course -> getInstances();	//Return an array of MagesterCourse objects, where keys are the ids
     * </code>
     *
    ...
    public function getInstances($constraints = array())
    {
    	...
    	/*
    	 #ifdef ENTERPRISE
    	 $result 	  = sC_getTableData("module_hcd_course_to_branch cb, module_hcd_branch b", "cb.branches_ID, cb.courses_ID, b.name", "b.branch_ID=cb.branches_ID");
    	 $branchResult = array();
    	 foreach ($result as $value) {
     ...
     *
     *
     * @param mixed $lesson An MagesterLesson or a lesson id
     ...
     *
     * @param MagesterLesson $lesson The lesson to set the mode for
     * @since 3.6.1
     * @access private
     */
    private function setUniqueLessonMode($lesson)
    {
    	...
    	$result = sC_getTableData("lessons", "*", "instance_source=".$lesson -> lesson['id']." and originating_course=".$this -> course['id']);
    	if (sizeof($result) > 0) {
    		$lessonInstance = new MagesterLesson($result[0]);
    		$this -> addLessons($lessonInstance);
    	} else {
    		$lessonInstance = MagesterLesson :: createInstance($lesson, $this);
    		$this -> addLessons($lessonInstance);
    	}
    	...
    	sC_deleteTableData("lessons_to_courses", "courses_ID=".$this -> course['id']." and lessons_ID=".$lesson->lesson['id']);
    }
    /**
     ...
     *
     * @param MagesterLesson $oldLesson The lesson to replace
     * @param MagesterLesson $newLesson The new lesson to put
     * @since 3.6.1
     * @access private
     */
     ...
     *
     * @param mixed $course The course argument, can be an id or an MagesterCourse object
     ...
     *
     * @param mixed $user A user login or an MagesterUser object
    ...
    public function checkRules($user, $courseLessons = false)
    {
    	...
    	$courseLessons = MagesterCourse::convertLessonObjectsToArrays($courseLessons);
    ...
    {
    	$lesson = MagesterLesson::convertArgumentToLessonObject($lesson);
    	$result = sC_getTableData("lessons_to_courses", "*", "lessons_ID=".$lesson -> lesson['id']." and courses_ID=".$this -> course['id']);
    	return !empty($result);
    }
    /**
    ...
    {
    	sC_updateTableData("users_to_courses", $fields, $where);
    ...
    {
    	sC_updateTableData("lessons_to_courses", $fields, $where);
    }
    public function handlePostAjaxRequestionForLessons()
    {
    	if (isset($_GET['id']) && sC_checkParameter($_GET['id'], 'id')) {
    		...
    		/*
    		 $courseLessons = $this -> getCourseLessons();
    		 $result = sC_getTableData("lessons", "*", "archive=0 and active=1 and course_only=1");
    		 $lessons = array();
    		...
    		$courseLessons = $this -> getCourseLessons();
    		isset($_GET['filter']) ? $courseLessons = sC_filterData($courseLessons, $_GET['filter']) : null;
    ...
    private function handlePostAjaxRequestForSingleUserClasses()
    {
    	...
    	if (!$courseclass -> isUserInClass($user)) {
    		$courseclass -> addUser($user);
    	} else {
    		$courseclass -> archiveClassesUser($user);
    	}
    }
    ...
    public function getCourseClasses($constraints = array())
    {
    	$fields = array(
			...
			if ($constraints['users_ID']) {
				$userLogin = sC_getTableData("users", "login", "id = " . $constraints['users_ID']);
			...
			$where[] = "c.courses_ID = '" . $this -> course['id'] . "' OR c.courses_ID IS NULL";
			$result = sC_getTableData($from, $select, implode(" and ", $where), $orderby, false, $limit);
    ...
    {
    	sC_updateTableData("users_to_courses", array("classe_id" => $classe_id), sprintf("users_LOGIN = '%s' AND courses_ID = %d", $login, $course_id));
    }
    ...
    private static function addWhereConditionToClassesConstraints($constraints)
    {
    	...
    	if (isset($constraints['filter']) && sC_checkParameter($constraints['filter'], 'text')) {
    		$result = sC_describeTable("classes");
    		$tableFields = array();
    		foreach ($result as $value) {
    			$tableFields[] = "l.".$value['Field'].' like "%'.$constraints['filter'].'%"';
    		}
    ...
    public static function getModalidades()
    {
		...
		'presencial'	=> array(
			...
			'fields' => array(
				'price_presencial'	=> array(
					'label'		=> _PRESENCIAL_PRICE,
					'type'		=> 'text',
					'default'	=> 0,
					'attr'		=> 'class = "small" alt="decimal"'
					),
				'enable_presencial'	=> array(
