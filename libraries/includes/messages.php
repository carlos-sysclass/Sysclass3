...
//---------------------------------------End of Volume-------------------------------------------
 if (isset($_GET['folders'])) {
  ...
  $form -> addElement('text', 'subject', _SUBJECT, 'id = "msg_subject" class = "inputText" style = "width:400px"');
        ...
        $form -> addElement('checkbox', 'email', _SENDASEMAILALSO, null, 'class = "inputCheckBox" checked="checked"');
     ...
     $recipients[] = $flippedLogins[$value];
            ...
            switch ($form -> exportValue('recipients')) {
                ...
                case 'specific_course':
                    $course = new MagesterCourse($form -> exportValue('specific_course'));
     ...
     '{$html}{if $required}
      ...
      {/if}');
        ...
        if ($currentMessage['attachments']) {
             ...
             */
            try {
                $attachment = new MagesterFile($currentMessage['attachments']);
