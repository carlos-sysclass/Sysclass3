...
if (isset($_GET['edit_user']) && eF_checkParameter($_GET['edit_user'], 'login')) {
 ...
 $load_editor = true;
    ...
    $seenContent = MagesterStats :: getStudentsSeenContent($currentLesson -> lesson['id'], $editedUser -> user['login']);
    $conditions = $currentLesson -> getConditions();
    foreach ($iterator = new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent -> tree), RecursiveIteratorIterator :: SELF_FIRST))) as $key => $value) {
        $visitableContentIds[$key] = $key; //Get the not-test unit ids for this content
    }
    foreach ($iterator = new MagesterTestsFilterIterator(new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent -> tree), RecursiveIteratorIterator :: SELF_FIRST)))) as $key => $value) {
        $testsIds[$key] = $key; //Get the not-test unit ids for this content
    }

    list($conditionsStatus, $lessonPassed) = MagesterStats :: checkConditions($seenContent[$currentLesson -> lesson['id']][$editedUser -> user['login']], $conditions, $visitableContentIds, $testsIds);
    $smarty -> assign("T_CONDITIONS", $conditions);
    $smarty -> assign("T_CONDITIONS_STATUS", $conditionsStatus);
    foreach ($iterator = new MagesterAttributeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent -> tree)), array('id', 'name')) as $key => $value) {
...
//		        Cache::resetCache($cacheKey);
    ...
    $doneTests = MagesterStats :: getDoneTestsPerUser($_GET['edit_user'], false, $currentLesson -> lesson['id']);

    $result = MagesterStats :: getStudentsDoneTests($currentLesson -> lesson['id'], $_GET['edit_user']);
    ...
    $userProjects = MagesterStats :: getStudentsAssignedProjects($currentLesson -> lesson['id'], $editedUser -> user['login']);
...
//Get users list through ajax
 try {
  if (isset($_GET['ajax']) && $_GET['ajax'] == 'usersTable') {
   ...
   foreach (MagesterLessonUser :: getLessonsRoles() as $key => $value) {
...
/*
	...
	try {
		...
		$users = MagesterStats::getUsersLessonStatus($currentLesson, array_keys($currentLesson -> getUsers('student')), array('notests' => 1, 'noprojects' => 1));
