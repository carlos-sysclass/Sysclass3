...
$import_export_types = MagesterImport::getImportTypes();
...
foreach ($import_export_types as $type => $name) {
 if ($type != "anything") {
  $help_info[$type] = array("mandatory" => implode(", ", MagesterImport::getMandatoryFields($type)),
             "optional" => implode(", ", MagesterImport::getOptionalFields($type)),
          "sample_type" => $type);
 }
}
...
if (isset($_GET['csv_sample']) && $_GET['csv_sample']==1 && isset($_GET['sample_type'])) {
 $sample_type = $_GET['sample_type'];
    ...
    echo implode(",", MagesterImport::getMandatoryFields($sample_type)) . "," . implode(",", MagesterImport::getOptionalFields($sample_type)) . "\n";
    exit;
}
...
if ($importForm -> isSubmitted()) {
    try {
        ...
        $importer = MagesterImportFactory::factory("csv", $uploadedFile, $options);
        ...
        if ($importType == "anything") {
         ...
         foreach ($import_types as $import_type => $import_name) {
          ...
          $headerType = MagesterImport::getImportTypeName($import_type);
          if ($headerType) {
...
if ($exportForm -> isSubmitted() && $exportForm -> validate()) {
    ...
    try {
     ...
     $exporter = MagesterExportFactory::factory("csv", $options);
        ...
        $smarty -> assign("T_EXCEPTION_TRACE", $e -> getTraceAsString());
        $message = _ERRORRESTORINGFILE.': '.$e -> getMessage().' ('.$e -> getCode().') &nbsp;<a href = "javascript:void(0)" onclick = "sC_js_showDivPopup(\''._ERRORDETAILS.'\', 2, \'error_details\')">'._MOREINFO.'</a>';
        $message_type = 'failure';
    }
