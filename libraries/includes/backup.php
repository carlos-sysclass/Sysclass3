...
if (str_replace(DIRECTORY_SEPARATOR, "/", __FILE__) == $_SERVER['SCRIPT_FILENAME']) {
    exit;
        ...
        if (isset($_GET['restore'])) {
            ...
            try {
                $restoreFile = new MagesterFile($_GET['restore']);
                if (!MagesterSystem :: restore($_GET['restore'], $_GET['force'] ? $_GET['force'] : false)) {
             ...
             handleNormalFlowExceptions($e);
                if ($e -> getCode() == MagesterSystemException::INCOMPATIBLE_VERSIONS) {
        ...
        try {
            ...
            $extraHeaderOptions = array(array('image' => 'images/16x16/go_into.png',
                      'title' => _BACKUP,
                      'action' => 'sC_js_showDivPopup(\''._BACKUP.'\', 0, \'backup_table\')'));
         /**The file manager*/
      include 'file_manager.php';
        ...
        $backup_form -> registerRule('checkParameter', 'callback', 'sC_checkParameter'); //Register this rule for checking user input with our function, sC_checkParameter
        ...
        if ($backup_form -> isSubmitted() && $backup_form -> validate()) {
            ...
            try {
                $backupFile = MagesterSystem :: backup($values['backupname'].'.zip', $values['backuptype']);
                sC_redirect("".basename($_SERVER['PHP_SELF'])."?ctg=backup&message=".urlencode(_SUCCESFULLYBACKEDUP)."&message_type=success");
