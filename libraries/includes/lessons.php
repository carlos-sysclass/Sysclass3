...
if (isset($_GET['delete_lesson']) && sC_checkParameter($_GET['delete_lesson'], 'id')) { //The administrator asked to delete a lesson
    ...
    try {
        $lesson = new MagesterLesson($_GET['delete_lesson']);
        $lesson->delete();
    } catch (Exception $e) {
    ...
    try {
        ...
        $lesson = new MagesterLesson($_GET['archive_lesson']);
        $lesson->archive();
    } catch (Exception $e) {
        header("HTTP/1.0 500 ");
        echo rawurlencode($e->getMessage()).' ('.$e->getCode().')';
    ...
    try {
        $lesson = new MagesterLesson($_GET['deactivate_lesson']);
        $lesson->deactivate();
        echo "0";
    } catch (Exception $e) {
        $message = _SOMEPROBLEMEMERGED.': '.$e->getMessage().' ('.$e->getCode().')';
    ...
    try {
        $lesson = new MagesterLesson($_GET['activate_lesson']);
        $lesson->activate();
        echo "1";
    } catch (Exception $e) {
        $message = _SOMEPROBLEMEMERGED.': '.$e->getMessage().' ('.$e->getCode().')';
    ...
    try {
        $lesson = new MagesterLesson($_GET['unset_course_only']);
    ...
    try {
        $lesson = new MagesterLesson($_GET['set_course_only']);
    ...
    if (isset($_GET['edit_lesson'])) { //If we are editing a lesson, we set the default form values to the ones stored in the database
        $editLesson = new MagesterLesson($_GET['edit_lesson']);
            ...
            if (isset($_GET['add_lesson'])) { //The second case is when the administrator adds a new lesson
                ...
                try {
                    ...
                    if ($values['copy_properties']) {
                        $copyPropertiesLesson = new MagesterLesson($values['copy_properties']);
                    ...
                    $newLesson = MagesterLesson::createLesson($fields_insert);
                    ...
                    if ($values['clone_lesson']) {
                        $cloneLesson = new MagesterLesson($values['clone_lesson']);
