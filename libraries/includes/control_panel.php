...
try {
	...
	if (!$_admin_ && isset($_SESSION['s_lessons_ID'])) {
		/*
		  ...
		  $fields_log = array ('users_LOGIN' => $_SESSION['s_login'],  //This is the log entry array$T_CONTROL_PANEL_OPTIONS
	...
	if (isset($_GET['op']) && $_GET['op'] == 'search') {
		...
		if ($_admin_) {
			...
			if (!isset($currentUser->coreAccess['lessons']) || $currentUser->coreAccess['lessons'] != 'hidden') {
				...
				$courses = MagesterCourse :: getCoursesWithPendingUsers($constraints);
		...
		if ($_student_) {
			...
			/* 	$hideFeedback = false;
			  ...
			  foreach (new MagesterNoFeedbackFilterIterator(new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('active' => 1)))) as $key => $value) {
			...
			if (!$currentLesson->options['tracking'] || $currentUser->coreAccess['content'] == 'hidden') {
				...
				if ($userProgress['current_unit']) { //If there exists a value within the 'current_unit' attribute, it means that the student was in the lesson before. Seek the first unit that he hasn't seen yet
					...
					$iterator = new MagesterContentCourseClassFilterIterator(new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent->tree), RecursiveIteratorIterator :: SELF_FIRST))), $courseClass);
					$iterator->next();
			...
			if ($currentLesson->options['digital_library'] && $currentUser->coreAccess['content'] != 'hidden') { //If the lesson digital library is enabled
				...
				if (sizeof($sharedFiles) > 0) {
					...
					$filesystemIterator = new MagesterFileOnlyFilterIterator(new MagesterNodeFilterIterator(new MagesterDBOnlyFilterIterator(new MagesterFileOnlyFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($sharedFiles), RecursiveIteratorIterator :: SELF_FIRST))), array('shared' => $currentLesson->lesson['id'])));
		...
		$controlPanelGroups[5] = array('title' => _COMMUNICATION, 'itens' => array());
		$controlPanelGroups[6] = array('title' => _MODULES, 'itens' => array());
		$controlPanelGroups[7] = array('title' => _VARIOUS, 'itens' => array());
...
//		$controlPanelOptions = array();
		...
		if ($_admin_) {
			...
			if (!isset($currentUser->coreAccess['lessons']) || $currentUser->coreAccess['lessons'] != 'hidden') {
				...
				$controlPanelOptions[] = array('text' => _LESSONS, 'image' => "32x32/lessons.png", 'href' => "administrator.php?ctg=lessons", 'group' => 3, 'class' => 'pencil');
			}
			if (!isset($currentUser->coreAccess['users']) || $currentUser->coreAccess['users'] != 'hidden') {
				$controlPanelOptions[] = array('text' => _PERSONALDATA, 'image' => "32x32/user_mapping.png", 'href' => "administrator.php?ctg=users&edit_user=" . $_SESSION['s_login'], 'group' => 4, 'class' => 'profile_one');
				$controlPanelOptions[] = array('text' => _USERS, 'image' => "32x32/user.png", 'href' => "administrator.php?ctg=users", 'group' => 4, 'class' => 'profile_two');
			}

			if (!isset($currentUser->coreAccess['user_types']) || $currentUser->coreAccess['user_types'] != 'hidden') {
				$controlPanelOptions[] = array('text' => _ROLES, 'image' => "32x32/user_types.png", 'class' => 'profile_one' , 'href' => "administrator.php?ctg=user_types", 'group' => 4, 'class' => 'group');
			}
			if (!isset($currentUser->coreAccess['users']) || $currentUser->coreAccess['users'] != 'hidden') {
				$controlPanelOptions[] = array('text' => _GROUPS, 'image' => "32x32/users.png", 'href' => "administrator.php?ctg=user_groups", 'group' => 4, 'class' => 'group_two');
			}
			if (!isset($currentUser->coreAccess['configuration']) || $currentUser->coreAccess['configuration'] != 'hidden') {
				$controlPanelOptions[] = array('text' => _CONFIGURATIONVARIABLES, 'class' => 'gear', 'image' => "32x32/tools.png", 'href' => "administrator.php?ctg=system_config", 'group' => 2);
			}
			if (!isset($currentUser->coreAccess['themes']) || $currentUser->coreAccess['themes'] != 'hidden') {
//                $controlPanelOptions[] = array('text' => _THEMES, 'image' => "32x32/themes.png", 'href' => "administrator.php?ctg=themes&theme=".$GLOBALS['currentTheme'] -> {$currentTheme -> entity}['id'], 'group' => 2);
			}
			if (!isset($currentUser->coreAccess['notifications']) || $currentUser->coreAccess['notifications'] != 'hidden') {
				$controlPanelOptions[] = array('text' => _EMAILDIGESTS, 'image' => "32x32/notifications.png", 'href' => "administrator.php?ctg=digests", 'group' => 5, 'class' => 'group');
			}
			if ((!isset($currentUser->coreAccess['personal_messages']) || $currentUser->coreAccess['personal_messages'] != 'hidden') && $GLOBALS['configuration']['disable_messages'] != 1) {
				$controlPanelOptions[] = array('text' => _MESSAGES, 'image' => "32x32/mail.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=messages", 'group' => 5);
			}
			/*
			...
			if (!isset($currentUser->coreAccess['backup']) || $currentUser->coreAccess['backup'] != 'hidden') {
				$controlPanelOptions[] = array('text' => _EXPORTIMPORTDATA, 'image' => "32x32/import_export.png", 'href' => "administrator.php?ctg=import_export", 'group' => 2, 'class' => 'transf_data');
			}
			if (!isset($currentUser->coreAccess['languages']) || $currentUser->coreAccess['languages'] != 'hidden') {
				$controlPanelOptions[] = array('text' => _LANGUAGES, 'image' => "32x32/languages.png", 'href' => "administrator.php?ctg=languages", 'group' => 2, 'class' => 'idiom');
			}
			if (!isset($currentUser->coreAccess['statistics']) || $currentUser->coreAccess['statistics'] != 'hidden') {
				$controlPanelOptions[] = array('text' => _STATISTICS, 'image' => "32x32/reports.png", 'href' => "administrator.php?ctg=statistics", 'group' => 2, 'class' => 'graph');
			}
			if (!isset($currentUser->coreAccess['backup']) || $currentUser->coreAccess['backup'] != 'hidden') {
				$controlPanelOptions['backup'] = array('text' => _BACKUP . " - " . _RESTORE, 'image' => "32x32/backup_restore.png", 'href' => "administrator.php?ctg=backup", 'group' => 2, 'class' => 'refresh');
			}
			if (!isset($currentUser->coreAccess['maintenance']) || $currentUser->coreAccess['maintenance'] != 'hidden') {
				$controlPanelOptions[] = array('text' => _MAINTENANCE, 'image' => "32x32/maintenance.png", 'href' => "administrator.php?ctg=maintenance", 'group' => 2, 'class' => 'gear');
			}
			if ((!isset($currentUser->coreAccess['forum']) || $currentUser->coreAccess['forum'] != 'hidden') && $GLOBALS['configuration']['disable_forum'] != 1) {
				$controlPanelOptions[] = array('text' => _FORUM, 'image' => "32x32/forum.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=forum", 'group' => 5, 'class' => 'chat');
			}
			if ($GLOBALS['configuration']['chat_enabled']) {
				if (!isset($currentUser->coreAccess['chat']) || $currentUser->coreAccess['chat'] != 'hidden') {
					$controlPanelOptions[] = array('text' => _CHAT, 'image' => "32x32/chat.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=chat", 'group' => 5, 'class' => 'chat');
				}
			}
			if (!isset($currentUser->coreAccess['modules']) || $currentUser->coreAccess['modules'] != 'hidden') {
				$controlPanelOptions[] = array('text' => _MODULES, 'image' => "32x32/addons.png", 'href' => "administrator.php?ctg=modules", 'group' => 2, 'class' => 'group_two');
			}
		}
		//Set control panel elements for professor
			...
			if ($currentUser->coreAccess['content'] != 'hidden') {
				$currentLesson->options['lesson_info'] ? $controlPanelOptions[0] = array('text' => _LESSONINFORMATION, 'image' => "32x32/information.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=lesson_information", 'group' => 3) : null;
				if ($currentContent && $currentContent->getFirstNode()) {
					$controlPanelOptions[1] = array('text' => _CONTENTMANAGEMENT, 'image' => "32x32/content.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=content&view_unit=" . $currentContent->getFirstNode()->offsetGet('id'), 'group' => 3);
				} else {
					$controlPanelOptions[1] = array('text' => _CONTENTMANAGEMENT, 'image' => "32x32/content.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=content", 'group' => 3);
				}
				if (!isset($currentUser->coreAccess['content']) || $currentUser->coreAccess['content'] == 'change') {
					$controlPanelOptions[5] = array('text' => _CONTENTTREEMANAGEMENT, 'image' => "32x32/content_reorder.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=order", 'group' => 3);
					$controlPanelOptions[7] = array('text' => _COPYFROMANOTHERLESSON, 'image' => "32x32/lesson_copy.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=copy", 'group' => 3);
				}
				($currentLesson->options['projects'] && $GLOBALS['configuration']['disable_projects'] != 1) ? $controlPanelOptions[2] = array('text' => _PROJECTS, 'image' => "32x32/projects.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=projects", 'group' => 3) : null;
				($currentLesson->options['tests'] && $GLOBALS['configuration']['disable_tests'] != 1) ? $controlPanelOptions[3] = array('text' => _TESTS, 'image' => "32x32/tests.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=tests", 'group' => 3) : null;
				$currentLesson->options['rules'] ? $controlPanelOptions[10] = array('text' => _ACCESSRULES, 'image' => "32x32/rules.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=rules", 'group' => 3) : null;
				$currentLesson->options['scorm'] ? $controlPanelOptions[19] = array('text' => _SCORM, 'image' => "32x32/scorm.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=scorm", 'group' => 3) : null;
				$currentLesson->options ? $controlPanelOptions[21] = array('text' => _IMS, 'image' => "32x32/autocomplete.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=ims", 'group' => 3) : null;
			}
			if ($currentUser->coreAccess['glossary'] != 'hidden' && $GLOBALS['configuration']['disable_glossary'] != 1) {
				$currentLesson->options['glossary'] ? $controlPanelOptions[11] = array('text' => _GLOSSARY, 'image' => "32x32/glossary.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=glossary", 'group' => 3) : null;
			}
			if ($currentUser->coreAccess['statistics'] != 'hidden') {
				$controlPanelOptions[14] = array('text' => _STATISTICS, 'image' => "32x32/reports.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=statistics", 'group' => 3);
			}
			if ($currentUser->coreAccess['settings'] != 'hidden') {
				$controlPanelOptions[13] = array('text' => _SCHEDULING, 'image' => "32x32/schedule.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=scheduling", 'group' => 3);
			}
			if ($currentUser->coreAccess['files'] != 'hidden') {
				$controlPanelOptions[17] = array('text' => _FILES, 'image' => "32x32/file_explorer.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=file_manager", 'group' => 3);
			}
			if ($currentUser->coreAccess['settings'] != 'hidden') {
				$controlPanelOptions[23] = array('text' => _LESSONSETTINGS, 'image' => "32x32/tools.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=settings", 'group' => 3);
			}
			if ($currentUser->coreAccess['feedback'] != 'hidden' && $GLOBALS['configuration']['disable_feedback'] != 1) {
				$currentLesson->options['feedback'] ? $controlPanelOptions[9] = array('text' => _FEEDBACK, 'image' => "32x32/feedback.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=feedback", 'group' => 3) : null;
			}
			...
			if ((!isset($currentUser->coreAccess['personal_messages']) || $currentUser->coreAccess['personal_messages'] != 'hidden') && $GLOBALS['configuration']['disable_messages'] != 1) {
				$controlPanelOptions[6] = array('text' => _MESSAGES, 'image' => "32x32/mail.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=messages", 'group' => 5);
			}
			...
			if ($currentUser->coreAccess['glossary'] != 'hidden' && $GLOBALS['configuration']['disable_glossary'] != 1 && $currentLesson->options['glossary']) {
				$option = array('text' => _GLOSSARY, 'image' => "32x32/glossary.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=glossary", 'group' => 7);
				$controlPanelOptions[] = $option;
				$headerOptions[] = $option;
			}
			if ($currentUser->coreAccess['statistics'] != 'hidden' && $currentLesson->options['reports']) {
				$option = array('text' => _STATISTICS, 'image' => "32x32/reports.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=statistics", 'group' => 7);
				$controlPanelOptions[] = $option;
				$headerOptions[] = $option;
			}
			...
			if ((!isset($currentUser->coreAccess['personal_messages']) || $currentUser->coreAccess['personal_messages'] != 'hidden') && $GLOBALS['configuration']['disable_messages'] != 1) {
				$option = array('text' => _MESSAGES, 'image' => "32x32/mail.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=messages", 'group' => 5);
				$controlPanelOptions[] = $option;
				$headerOptions[] = $option;
			}
			if ($currentLesson->options['projects'] && $GLOBALS['configuration']['disable_projects'] != 1 && sizeof($projectsInControlPanel) > 0) {
				$controlPanelOptions[] = array('text' => _PROJECTS, 'image' => "32x32/projects.png", 'href' => basename($_SERVER['PHP_SELF']) . "?ctg=projects", 'group' => 5);
			}
				...
				if ($centerLinkInfo) {
					...
					if ($_SESSION['s_lesson_user_type'] != 'student') {
						switch ($module_key) {
							...
							case "module_xuser" : {
								$controlPanelOption['group'] = 4;
								break;
							}
							case "module_ies" :
							case "module_polos" :
							case "module_xcontent" : {
								$controlPanelOption['group'] = 3;
								break;
							}
							default : {
								$controlPanelOption['group'] = 6;
								break;
							}
						}
					}
