...
try {
 ...
 if (sizeof($sharedFiles) > 0) {
  ...
  //$filesystemIterator = new MagesterFileOnlyFilterIterator(new MagesterNodeFilterIterator(new MagesterDBOnlyFilterIterator(new MagesterFileOnlyFilterIterator(new RecursiveIteratorIterator($filesystem -> tree, RecursiveIteratorIterator :: SELF_FIRST))), array('shared' => $currentLesson -> lesson['id'])));
