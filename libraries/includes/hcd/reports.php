...
//		echo $sql_query."<BR>";
 ...
 if (isset($_GET['ajax']) && $_GET['ajax'] == 'foundEmployees') {
  ...
  $allStats = MagesterStats :: getUsersTimeAll();
  //$allStats = MagesterStats :: getUsersTimeAll($from, $to);
  ...
  foreach ($probed_lessons as $id => $lesson) {
   ...
   //$userTraffic = MagesterStats :: getUsersTime($id, $user_logins, $from, $to);
...
$groupsResult = MagesterGroup::getGroups();
