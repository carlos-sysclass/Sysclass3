...
try {
 ...
 if (isset($_GET['edit_job_description'])) {
        $currentJob = new MagesterJob($_GET['edit_job_description']);
   ...
   $editedEmployee = $editedUser -> aspects['hcd'];
     ...
     **************************************************** */
       ...
       $smarty -> assign("T_TABLE_SIZE", $totalEntries);
                ...
                $allCourses = MagesterCourse::getAllCourses(array("return_objects" => false, "instance" => false, "sort" => "name"));
                if (!empty($allCourses)) {
        ...
        if ($form -> isSubmitted()) {
            if ($form -> validate()) {
                ...
                if (isset($_GET['add_job_description'])) {
                    ...
                    MagesterJob :: createJob($job_description_content);
