...
if (isset($_SESSION['s_login']) && ($_SESSION['s_type'] == 'administrator' || $currentEmployee -> isSupervisor())) {
 ...
 if (isset($_GET['ajax'])) {
  ...
  if ($_GET['ajax'] == "unattachedUsersTable" && $currentEmployee -> getType() == _SUPERVISOR) {
   ...
   $unattached_employee = sC_multiSort($unattached_employee, $_GET['sort'], $order);
   if (isset($_GET['filter'])) {
    $unattached_employee = sC_filterData($unattached_employee , $_GET['filter']);
   }
   ...
   if ($currentEmployee -> getType() == _SUPERVISOR) {
    ...
    $tree = new MagesterBranchesTree();
    $branchPaths = $tree -> toPathString();
    $employees = sC_getTableData("
     users
    ...
    foreach ($employees as $key => $value) {
     if (!$value['active'] || $value['archive'] || !$value['jobs_num']) {
      ...
      $employees[$key]['branch_name'] = sC_truncatePath($branchPaths[$value['branch_ID']], 10);
     }
 ...
 sC_redirect("" . $_SESSION['s_type'] . ".php?ctg=control_panel&message=".urlencode($message)."&message_type=$message_type");
 exit;
}
