...
if (!isset($currentContent)) {
    if (!$currentLesson) {
        if ($_GET['view_unit']) {
			$unit = new MagesterUnit($_GET['view_unit']);
			$currentLesson = new MagesterLesson($unit['lessons_ID']);
        } elseif ($_GET['package_ID']) {
			$unit = new MagesterUnit($_GET['package_ID']);
			$currentLesson = new MagesterLesson($unit['lessons_ID']);
        }
...
foreach (new MagesterContentCourseClassFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent -> tree), RecursiveIteratorIterator :: SELF_FIRST)), $courseClass) as $key => $value) {
    $legalValues[] = $key;
}
...
if (isset($_GET['add']) || (isset($_GET['edit']) && in_array($_GET['edit'], $legalValues)) && $_change_) {
 try {
     if ($_GET['edit']) {
         ...
         $unitData = new MagesterUnit($_GET['edit']);
  ...
  $form -> addElement('text', 'object_ids', _SPECIFYIDFORSREENMATCHING, 'class = "inputText"');
   ...
   $form -> addRule('scorm_size', _INVALIDFIELDDATA, 'checkParameter', 'id');
     ...
     if (mb_strpos($currentUnit['data'], "<iframe") !== false && mb_strpos($currentUnit['data'], "pdfaccept") !== false) {
         ...
         $form -> setDefaults(array('pdf_content' => MagesterFile :: decode(htmlspecialchars_decode(urldecode($contentParts[sizeof($contentParts)-1].'.pdf')))));
     ...
     if ($form -> isSubmitted() && $form -> validate()) {
         ...
         if ($_FILES['pdf_upload']['name'] != "") {
             if (strpos($_FILES['pdf_upload']['name'], ".pdf") !== false) {
                 $destinationDir = new MagesterDirectory(G_LESSONSPATH.$_SESSION['s_lessons_ID']);
   ...
   if (isset($_GET['edit'])) {
    ...
    $iterator = new MagesterInArrayFilterIterator(new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent -> tree), RecursiveIteratorIterator :: SELF_FIRST))), $parents);
...
*/		
			...
			$total_scheduled = eF_getTableData(
				...
				sprintf("schl.active = 1 AND cnt.content_id = %d ", $currentUnit->offsetGet('id'))
			);
			...
			if (count($total_scheduled) == 0) {
				...
				if (count($userSchedule) == 0) {
					$url = sprintf($xContentModule->moduleBaseUrl . "&action=%s&message_type=%s&message=%s",
						...
						"failure",
						urlencode("Favor selecionar um horário para agendar")
					);
					/*
		...
		if ($contentClasses) {
			if (
				...
				in_array($courseClass, $contentClasses['classe_id'])
			) {
			} else {
   ...
   /*$hideFeedback = false;
			...
			foreach (new MagesterNoFeedbackFilterIterator(new MagesterVisitableAndEmptyFilterIterator($visitableIterator)) as $key => $value) {
            ...
            $smarty -> assign("T_CONTENT_TREE", $currentContent -> toHTML(new MagesterVisitableAndEmptyFilterIterator($visitableIterator), 'dhtmlContentTree', $treeOptions, $scormState));
   ...
   $currentUnit = new MagesterUnit($currentContent -> getFirstNode() -> offsetGet('id'));
   $smarty -> assign("T_CURRENTUNITID", $currentUnit['id']);
    ...
    $currentUnit['data'] = glossary :: applyGlossary($currentUnit['data'], $currentLesson -> lesson['id']);
            ...
            $currentUnit['data'] = str_replace("##MAGESTERINNERLINK##", $_SESSION['s_lesson_user_type'], $currentUnit['data']);
        ...
        if ($_student_ && $_change_ && $currentLesson -> options['tracking']) {
            ...
            if (isset($_GET['set_seen'])) {
                try {
                    ...
                    $newUserProgress = MagesterStats :: getUsersLessonStatus($currentLesson, $currentUser -> user['login']);
