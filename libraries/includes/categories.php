...
if (isset($_GET['delete_direction']) && eF_checkParameter($_GET['delete_direction'], 'id')) {
 ...
 try {
  $direction = new MagesterDirection($_GET['delete_direction']);
  if (sizeof($direction -> getLessons(false, true)) > 0 || sizeof($direction -> getCourses(false, true)) > 0) {
   throw new MagesterDirectionException(_YOUMUSTDELETEALLLESSONSANDSUBDIRECTIONSINTHISDIRECTIONBEFOREDELETINGIT.': '.$direction, MagesterDirectionException :: NOT_EMPTY_CATEGORY);
  } else {
   $direction -> delete();
  }
 } catch (Exception $e) {
 ...
 try {
  $direction = new MagesterDirection($_GET['deactivate_direction']);
  if (sizeof($direction -> getLessons(false, true)) > 0 || sizeof($direction -> getCourses(false, true)) > 0) {
   throw new MagesterDirectionException(_YOUMUSTDELETEALLLESSONSANDSUBDIRECTIONSINTHISDIRECTIONBEFOREDELETINGIT.': '.$direction, MagesterDirectionException :: NOT_EMPTY_CATEGORY);
  } else {
   $direction['active'] = 0;
 ...
 try {
  $direction = new MagesterDirection($_GET['activate_direction']);
  $direction['active'] = 1;
  $direction -> persist();
  echo "1";
 } catch (Exception $e) {
 ...
 if (isset($_GET['add_direction'])) {
  ...
  $editDirection = new MagesterDirection($_GET['edit_direction']);
  ...
  foreach (new MagesterAttributeFilterIterator(new RecursiveIteratorIterator($directionsTree -> getNodeChildren($_GET['edit_direction'])), array('id')) as $key => $value) {
 ...
 if (isset($_GET['edit_direction'])) {
  ...
  $lessons = $editDirection -> getLessons(true);//_MagesterLesson :: getLessons();
  $courses = $editDirection -> getCourses(true);//_MagesterCourse :: getCourses();
 ...
 foreach ($flatTree as $key => $value) {
  ...
  $direction = new MagesterDirection($flatTree[$key]);
