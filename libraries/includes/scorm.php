...
if ($_GET['scorm_review']) {
    $iterator = new MagesterSCORMFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent -> tree), RecursiveIteratorIterator :: SELF_FIRST)));
  ...
  //$form -> addElement('file', 'scorm_file', _SCORMFILEINZIPFORMAT);
            ...
            try {
                ...
                if ($urlUpload != "") {
                    ...
                    if (!copy($urlUpload, $currentLesson -> getDirectory().$urlFile)) {
                        ...
                        $scormFiles[] = new MagesterFile($currentLesson -> getDirectory().$urlFile);
                    }
                } else {
                ...
                foreach ($scormFiles as $scormFile) {
                    ...
                    $scormFolderName = MagesterFile :: encode(basename($scormFile['name'], '.zip'));
    ...
    if ($form -> isSubmitted() && $form -> validate()) {
        ...
        try {
            ...
            foreach (new MagesterNodeFilterIterator(new RecursiveIteratorIterator($filesystem -> tree, RecursiveIteratorIterator :: SELF_FIRST)) as $key => $value) {
