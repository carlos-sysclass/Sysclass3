...
if (isset($_POST['search_text'])) {
 ...
 $cr = $crTemp;
    ...
    $results = MagesterSearch :: searchFull($_POST['search_text']);
//pr($results);exit;
    $lessons_have = $courses_have = null;
    $currentUser = MagesterUserFactory :: factory($_SESSION['s_login']); //Get active lessons of this user
 $currentUser -> applyRoleOptions();
    ...
    if ($currentUser -> user['user_type'] == 'administrator') {
        $results_users = MagesterSearch :: searchUsers($_POST['search_text']);
...
//pr($results);
    ...
    $type_str = _GLOSSARY;
                        ...
                        if ($results[$i]['table_name'] == "lessons") {
                            $basic_user_type = eF_getUserBasicType(false, $res1[0]['lessons_ID']);
                                $tmp_data = array('id' => $res1[0]['id'],
                                                               ...
                                                               'lesson_name' => MagesterSearch :: highlightText($lesson[0]['name'],$cr, 'resultsTitleBold'),
                                                               'score' => $results[$i]['score'] * 100,
                                                               'type' => $type_str,
       ...
       if (strcmp($stripedContent, "...") == 0) {
                             $stripedContent = _SEARCHTEXTWASINSCRIPT;
                                ...
                                $tmp_data = array('id' => $res1[0]['id'],
                                                               ...
                                                               'content' => $stripedContent,
                                                              // 'content'   => MagesterSearch :: highlightText(MagesterSearch :: wordLimiter(mb_substr(strip_tags($res1[0]['data']),strpos(strip_tags($res1[0]['data']), $cr[0]),2000), 40), $cr, 'resultsText'),
                                                               //'content1'  => strpos(strip_tags($res1[0]['data']), $cr[0]).MagesterSearch :: highlightText(strip_tags($res1[0]['data']), $cr, 'resultsText'),
                                                               'score' => $results[$i]['score'] * 100,
                                                               'type' => $type_str,
                                                               'user_type' => $basic_user_type,
                                                               'position' => $position_str);
      ...
      if (!isset($GLOBALS['currentUser'] -> coreAccess['forum']) || $GLOBALS['currentUser'] -> coreAccess['forum'] != 'hidden') {
       if ($lesson[0]['id'] != "") {
        $lessonTemp = new MagesterLesson($lesson[0]['id']);
       } else {
        $lesson[0]['id'] = 0;
       }
     ...
     if (!isset($GLOBALS['currentUser'] -> coreAccess['personal_messages']) || $GLOBALS['currentUser'] -> coreAccess['personal_messages'] != 'hidden') {
      $search_results_pmsgs[] = array('message_subject' => MagesterSearch :: highlightText($res1[0]['title'],$cr,'resultsTitleBold'),
                                                    'message_id' => $res1[0]['id'],
                                                    'folder_name' => $res1[0]['name'],
                                                    'folder_id' => $res1[0]['folder_id'],
                                                    'body' => MagesterSearch :: highlightText($res1[0]['body'],$cr,'resultsTitleBold'),
     ...
     try {
         $file = new MagesterFile($res1[0]['id']);
