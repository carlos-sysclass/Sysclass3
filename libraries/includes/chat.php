...
if (isset($_GET['chat_room_options'])) {
    ...
    if (isset($_POST['chat_room_submit']) && (!$currentUser -> coreAccess['chat'] || $currentUser -> coreAccess['chat'] == 'change')) { //Create the new room
        if (!sC_checkParameter($_POST['chat_room_name'], 'text')) {
            $message = _INVALIDNAME;
            $message_type = failure;
        } else {
            $result = sC_getTableData("chatrooms", "id", "name='".$_POST['chat_room_name']."'");
            if (sizeof($result) == 0 ) {
                ...
                sC_insertTableData("chatrooms", $fields_insert);
    ...
    if (isset($_GET['past_messages'])) { //Room options
        $options_str = '';
        $chatrooms = sC_getTableData("chatrooms", "name, id", "type = 'private' AND users_LOGIN = '".$_SESSION['s_login']."'"); //Get only private rooms that belong to this user

        if ($_SESSION['s_type'] == "administrator") {
            $chatrooms = array_merge(sC_getTableData("chatrooms", "id, name, active", "type = 'public'"), $chatrooms);
            ...
            if (isset($_GET['chat_room'])) {
                if ($_GET['chat_room'] != 0) {
                    $room_name = sC_getTableData("chatrooms", "name, id, active", "id = ". $_GET['chat_room']);
                    $smarty -> assign("T_CHATROOMS", array($room_name[0]));
                    $smarty -> assign("T_SINGLE_ROOM_NAME",$room_name[0]['name']);
                } else {
                    $smarty -> assign("T_CHATROOMS", array(array("name" => _MAGESTERMAIN, "active" => 1, "id" => 0)));
                    $smarty -> assign("T_SINGLE_ROOM_NAME",_MAGESTERMAIN);
                }
        ...
        $users = sC_getTableData("users", "login, name, user_type");
    ...
    if (isset($_POST['chat_submit_show_messages'])) {
        ...
        if ($_POST['select_chat_room'] != 0 && !sC_checkParameter($_POST['select_chat_room'], 'id')) {
            sC_printMessage(_INVALIDID);
            exit;
        } else {
            $chatroom = $_POST['select_chat_room'];
        }

        if ($_POST['select_user'] != 0 && !sC_checkParameter($_POST['select_user'], 'login')) {
            sC_printMessage(_INVALIDLOGIN);
            exit;
        } else {
            $_POST['select_user'] ? $user_sql = " AND users_LOGIN = '".$_POST['select_user']."'" : $user_sql = '';
        }

        $messages = sC_getTableData("chatmessages", "timestamp, content, users_LOGIN, users_USER_TYPE", "timestamp >= $from_timestamp AND timestamp <= $to_timestamp AND chatrooms_ID=$chatroom".$user_sql);
    ...
    if (isset($_POST['chat_submit_delete_messages'])) {
        ...
        if (sC_deleteTableData("chatmessages", "timestamp >= $from_timestamp AND timestamp <= $to_timestamp AND chatrooms_ID=$chatroom".$user_sql)) {
    ...
    if (isset($_POST['chat_submit_export_messages'])) {
        ...
        $messages = sC_getTableData("chatmessages", "timestamp, content, users_LOGIN, users_USER_TYPE", "timestamp >= $from_timestamp AND timestamp <= $to_timestamp AND chatrooms_ID=$chatroom".$user_sql);
        $file = MagesterSystem::exportChat($messages);
    ...
    if (isset($_GET['show_users']) && sC_checkParameter($_GET['show_users'], 'id')) {
        $room_name = sC_getTableData("chatrooms", "name", "id=".$_GET['show_users']);
        $users_list = sC_getTableDataFlat("users_to_chatrooms", "users_LOGIN", "chatrooms_ID=".$_GET['show_users']);
    ...
    $user_type = sC_getUserBasicType($_SESSION['s_login']);
    $smarty ->assign("T_USER",$user_type);
    if (isset($_GET['delete']) && sC_checkParameter($_GET['id'], 'id')) { //Delete the room
        if ($currentUser -> getType() == 'administrator') {
            $result = sC_getTableData("chatrooms", "id", "id = ".$_GET['id']);
            if (sizeof($result) > 0) {
                sC_deleteTableData("chatrooms", "id=".$_GET['id']);
                sC_deleteTableData("chatmessages", "chatrooms_ID=".$_GET['id']);
                sC_deleteTableData("users_to_chatrooms", "chatrooms_ID=".$_GET['id']);
    ...
    if (isset($_GET['activate_system_chat'])) {
        $_GET['activate_system_chat'] ? MagesterConfiguration::setValue("chat_enabled", 1) : MagesterConfiguration::setValue("chat_enabled", 0);
  ...
  $smarty ->assign("T_REFRESH_SIDE", true);
    ...
    if (isset($_GET['activate']) && sC_checkParameter($_GET['id'], 'id')) { //Activate / deactivate the room
        $result = sC_getTableData("chatrooms", "active", "id=".$_GET['id']);
        $result[0]['active'] == 1 ? $active = 0 : $active = 1;

        $room = sC_getTableData("chatrooms", "*", "id=".$_GET['id']);
        ...
        if ($room[0]['lessons_ID'] != 0) {
            $lesson = new MagesterLesson($room[0]['lessons_ID']);
            if ($active) {
            ...
            sC_updateTableData("chatrooms", $fields_update, "id=".$_GET['id']);
        }
    }
    ...
    if (isset($_GET['logout'])) {
        if (isset($_GET['chatrooms_ID']) && sC_checkParameter($_GET['chatrooms_ID'], 'id')) {
            $chatrooms_ID = $_GET['chatrooms_ID'];
            sC_deleteTableData("users_to_chatrooms", "users_LOGIN='".$_SESSION['s_login']."' AND chatrooms_ID=".$chatrooms_ID);
            ...
            sC_insertTableData("chatmessages", $fields_insert); //Insert the system message into the database

            unset ($_GET['chatrooms_ID']);
        } else {
            $user_in_rooms_flat = sC_getTableDataFlat("users_to_chatrooms", "chatrooms_ID", "users_LOGIN = '".$_SESSION['s_login']."'");
    ...
    if (isset($_GET['chatrooms_ID']) && sC_checkParameter($_GET['chatrooms_ID'], 'id') && isset($_GET['invite']) && sC_checkParameter($_GET['invite'], 'login')) { //The user asked to have a private conversation
        ...
        $private_id = sC_insertTableData("chatrooms", $fields_insert); //Create the room
        ...
        sC_insertTableData("chatmessages", $fields_insert);

        $user_type = sC_getTableData("users", "user_type", "login='".$_GET['invite']."'");
        ...
        $msg_id_to = sC_insertTableData("chatmessages", $fields_insert);
        ...
        $msg_id_from = sC_insertTableData("chatmessages", $fields_insert);

        sC_updateTableData("chatmessages", array('content' => '#for_user-'.$_SESSION['s_login'].':'.$msg_id_from.'#<span class = "chatSystemMessage">'._YOUHAVEINVITEDUSER.' '.$_GET['invite'].' '._TOAPRIVATECONVERSATION.'.<span class = "boldFont">('._SYSTEMMESSAGE.')</span></span>'), "id=$msg_id_from");
        sC_updateTableData("chatmessages", array('content' => '#for_user-'.$_GET['invite'].':'.$msg_id_to.'#<span class = "chatSystemMessage">'._THEUSER.' '.$_SESSION['s_login'].' '._INVITESYOUTOPRIVATECONVERSATION.'. '._DOYOU.' <a href = "javascript:void(0)" onclick = "popUp(&quot;chat/chat_index.php?chatrooms_ID='.$private_id.'&msg_id_from='.$msg_id_from.'&msg_id_to='.$msg_id_to.'&from_user='.$_SESSION['s_login'].'&reply=accept&standalone=1&quot;, 600, 400)">'._YOUACCEPT.'</a> '._OR.' <a href = "javascript:void(0)" onclick = "popUp(&quot;chat/chat_index.php?chatrooms_ID='.$private_id.'&msg_id_from='.$msg_id_from.'&msg_id_to='.$msg_id_to.'&from_user='.$_SESSION['s_login'].'&reply=deny&standalone=1&quot;, 200, 100)">'._YOUDENY.'</a>'._QUESTIONMARK.'<span>('._SYSTEMMESSAGE.')</span></span>'), "id=$msg_id_to");

        sC_redirect(basename($_SERVER['PHP_SELF']).'?chatrooms_ID='.$private_id.'&standalone=1');


    } elseif (isset($_GET['chatrooms_ID'])) { //The user selected a room to enter
        $chatrooms_ID = sC_checkParameter($_GET['chatrooms_ID'], 'uint');
        if ($chatrooms_ID === false) { //Check if the passed parameter is valid
            sC_printMessage(_WRONGROOMID);
            exit;
        }
        $res_active = sC_getTableData("chatrooms","active","id=$chatrooms_ID");
        if ($res_active[0]['active'] != 1) { //Check if the passed parameter is valid
            sC_printMessage(_CHATROOMDEACTIVATEDBYADMIN);
            exit;
        }

        $user_already_online = sC_getTableData("users_to_chatrooms","users_LOGIN","users_LOGIN = '".$_SESSION['s_login']."' AND chatrooms_ID = $chatrooms_ID");
        $fields_insert = array('users_LOGIN' => $_SESSION['s_login'],
                               'chatrooms_ID' => $chatrooms_ID,
                               'users_USER_TYPE' => $_SESSION['s_type'],
                               'timestamp' => time());
        if (count($user_already_online)==0) {
            sC_insertTableData("users_to_chatrooms", $fields_insert); //Insert new user to database
        }
        if (isset($_GET['msg_id_from']) && sC_checkParameter($_GET['msg_id_from'], 'id') && isset($_GET['msg_id_to']) && sC_checkParameter($_GET['msg_id_to'], 'id') && isset($_GET['from_user']) && sC_checkParameter($_GET['from_user'], 'login')) { //We have just accepted an invitation for a one-to-one conversation. Update messages to display the acceptance and reload chat page.
            ...
            sC_updateTableData("chatmessages", array("content" => "#for_user-".$_GET['from_user'].":0#$reply_str"), "id=".$_GET['msg_id_from']);
            sC_updateTableData("chatmessages", array("content" => "#for_user-".$_SESSION['s_login'].":0#$reply_str"), "id=".$_GET['msg_id_to']);
            $fields_insert = array('users_LOGIN' => $_SESSION['s_login'],
                               'users_USER_TYPE' => $_SESSION['s_type'],
                               'content' => $reply_str,
                               'timestamp' => time(),
        ...
        $room_name = sC_getTableData("chatrooms", "name", "id = $chatrooms_ID"); //Get the room name
        ...
        //pr(sC_local_roomInfo('public'));
        $smarty -> assign("T_PUBLIC_ROOMS", sC_local_roomInfo('public'));
        $smarty -> assign("T_PRIVATE_ROOMS", sC_local_roomInfo('private'));
 ...
 * print_r(sC_local_roomInfo('public'));
 ...
 */
function sC_local_roomInfo($room_type)
{
    if ($room_type == 'public') {
        if ($_SESSION['s_type'] == "administrator") {
            $rooms_array = sC_getTableData("chatrooms", "id, name, create_timestamp, users_LOGIN, active, lessons_ID", "");
        } else {
            $rooms_array = sC_getTableData("chatrooms,users_to_lessons as ul", "distinct id, name, create_timestamp, chatrooms.users_LOGIN, chatrooms.active", "ul.archive=0 and type = 'public' AND (chatrooms.lessons_ID=ul.lessons_ID OR chatrooms.lessons_ID IS NULL) AND ul.users_LOGIN='".$_SESSION['s_login']."'");
        }
    } elseif ($room_type == 'private') {
        $rooms_array = sC_getTableData("chatrooms", "id, name, create_timestamp, users_LOGIN, active", "users_LOGIN = '".$_SESSION['s_login']."' AND type = 'private'");
    } else {
        return array();
    }
    $roomsTemp = array();
  ...
  $rooms_array = $roomsTemp;
    ...
    for ($i = 0; $i < sizeof($rooms_array); $i++) {
        ...
        $num_of_users_in_room = sC_getTableData("users_to_chatrooms", "count(*)", "chatrooms_ID=".$rooms_array[$i]['id']);
