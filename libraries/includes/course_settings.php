...
if (!isset($currentUser -> coreAccess['course_settings']) || $currentUser -> coreAccess['course_settings'] == 'change') {
	$options[] = array('image' => '16x16/classes.png', 'title' => __COURSE_CLASSE, 'link' => $_GET['op'] != 'course_classes' ? basename($_SERVER['PHP_SELF']).'?'.$baseUrl.'&op=course_classes' : 'javascript:void(0)', 'selected' => $_GET['op'] != 'course_classes' ? false : true);
}
...
/*
		$users = MagesterStats::getUsersCourseStatus($currentCourse);
...
*/
  ...
  if (isset($_GET['edit_user']) && in_array($_GET['edit_user'], array_keys($users = $currentCourse -> getCourseUsers($defaultConstraints)))) {
   //$userStats = MagesterStats::getUsersCourseStatus($currentCourse, $_GET['edit_user']);
   ...
   if ($form -> isSubmitted() && $form -> validate()) {
    if ($form -> exportValue('completed')) {
     $courseUser = MagesterUserFactory :: factory($_GET['edit_user']);
     ...
     MagesterCourse::persistCourseUsers($fields, $where, $currentCourse -> course['id'], $_GET['edit_user']);
     if ($userStats['issued_certificate'] != "") {
      MagesterEvent::triggerEvent(array("type" => MagesterEvent::COURSE_CERTIFICATE_REVOKE, "users_LOGIN" => $_GET['edit_user'], "lessons_ID" => $currentCourse -> course['id'], "lessons_name" => $currentCourse -> course['name']));
     }
    }
  ...
  if (isset($_GET['ajax']) && $_GET['ajax'] == 'courseUsersTable') {
   ...
   $users = MagesterCourse :: convertUserObjectsToArrays($users);
   $dataSource = $users;
   $tableName = $_GET['ajax'];
   $alreadySorted = true;
   ...
   if (sizeof($result) == 1 || isset($_GET['preview'])) {
    ...
    if (!isset($_GET['preview'])) {
     ...
     if ($certificate_tpl_id_rtf <= 0) {
      ...
      $cfile = new MagesterFile($certificate_tpl_id_rtf);
     }
   ...
   if (!isset($duplicate)) {
        ...
        try {
            ...
            foreach ($order as $value) {
                ...
                if (in_array($value, array_keys($courseLessons))) {
                    ...
                    MagesterCourse::persistCourseLessons($fields, $where);
                }
                $previous = $result[0];
            }
    ...
    try {
     if (isset($_GET['set_schedule']) && in_array($_GET['set_schedule'], array_keys($courseLessons))) {
      $lesson = new MagesterLesson($_GET['set_schedule']);
...
*/
    ...
    try {
        $currentExportedFile = new MagesterFile($currentUser -> user['directory'].'/temp/'.MagesterFile :: encode($currentCourse -> course['name']).'.zip');
