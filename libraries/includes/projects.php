...
if (isset($_GET['delete_project']) && in_array($_GET['delete_project'], array_keys($projects)) && $_professor_ && $_change_) {
    ...
    try {
        ...
        $projectDirectory = new MagesterDirectory($projectDir);
        foreach ($projectFiles as $file) {
            try {
                $projectFile = new MagesterFile($file['id']);
                $newFileName = MagesterFile :: encode($file['users_LOGIN'].'_'.date("d.m.Y", $file['upload_timestamp']).'_'.$projectFile['name']);
                $projectFile -> copy($projectDir.'/'.$newFileName);
            } catch (MagesterFileException $e) { //Don't halt for a single file
    ...
    if (!$_change_) {
        ...
        if ($form -> isSubmitted() && $form -> validate()) {
          ...
          'auto_assign' => $values['auto_assign'] ? 1 : 0);
                        ...
                        $newProject = MagesterProject :: createProject($fields);

                        MagesterEvent::triggerEvent(array("type" => MagesterEvent::PROJECT_EXPIRY, "timestamp" => $deadline, "lessons_ID" => $currentLesson -> lesson['id'], "lessons_name" => $currentLesson -> lesson['name'], "entity_ID" => $newProject -> project['id'], "entity_name" => $newProject -> project['title']));
  ...
  //pr($users);
    ...
    if (isset($_GET['ajax']) && $_GET['ajax'] == 'resultsTable') {
        ...
        foreach ($users as $key => $user) {
            if ($user['filename']) {
                try {
                    $projectFile = new MagesterFile($user['filename']);
   ...
   if (isset($_GET['reset_user']) && eF_checkParameter($_GET['reset_user'], 'login')) {
    ...
    try {
        ...
        if ($projectUser['filename']) {
            try {
                $projectFile = new MagesterFile($projectUser['filename']);
        ...
        if ($form -> isSubmitted() && $form -> validate() && !$currentProject -> expired) {
            try {
                ...
                if (!is_dir($projectDirectory)) {
                    MagesterDirectory :: createDirectory($projectDirectory);
                }
