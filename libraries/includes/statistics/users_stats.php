...
if ($currentUser -> user['user_type'] == 'administrator') {
 ...
 if ($lessonRoles[$lessonUsers[$currentUser -> user['login']]['role']] == 'professor') {
  ...
  throw new MagesterUserException(_USERDOESNOTHAVETHISLESSON.": ".$statisticsLesson -> lesson['name'], MagesterUserException :: USER_NOT_HAVE_LESSON);
 }
...
if (isset($_GET['sel_user'])) {
 ...
 if ($_GET['specific_lesson_info'] && $_GET['lesson']) {
  ...
  $status = MagesterStats :: getUsersLessonStatus($_GET['lesson'], $infoUser -> user['login']);
  $doneTests = MagesterStats :: getStudentsDoneTests($_GET['lesson'], $infoUser -> user['login']);
  ...
  try {
   ...
   if (isset($_GET['ajax']) && $_GET['ajax'] == 'courseLessonsTable' && eF_checkParameter($_GET['courseLessonsTable_source'], 'id')) {
    ...
    $smarty -> assign("T_DATASOURCE_COLUMNS", array('name', 'location', 'user_type', 'num_lessons', 'status', 'completed', 'score', 'operations', 'sort_by_column' => 4));
    $smarty -> assign("T_DATASOURCE_OPERATIONS", array('progress'));
    $lessons = $infoUser -> getUserStatusInCourseLessons(new MagesterCourse($_GET['courseLessonsTable_source']));
    $lessons = MagesterLesson :: convertLessonObjectsToArrays($lessons);
    $dataSource = $lessons;
   }
   $smarty -> assign("T_DATASOURCE_COLUMNS", array('name', 'location', 'user_type', 'num_lessons', 'status', 'completed', 'score', 'operations', 'sort_by_column' => 4));
  ...
  try {
   ...
   $userInfo['communication'] = MagesterStats :: getUserCommunicationInfo($infoUser);
   ...
   $userInfo['usage'] = MagesterStats :: getUserUsageInfo($infoUser);

   try {
    $avatar = new MagesterFile($userInfo['general']['avatar']);
   ...
   //$allUserTimes = MagesterStats :: getUsersTimeAll(false, false, array_keys($userLessons));
   ...
   try {
    if (isset($_GET['ajax']) && $_GET['ajax'] == 'graph_access') {
     ...
     $lesson = new MagesterLesson($_GET['entity']);
...
if (isset($_GET['excel']) && $_GET['excel'] == 'user') {
 ...
 if ($infoUser -> user['user_type'] != 'administrator') {
  ...
  $assignedProjects = MagesterStats :: getStudentsAssignedProjects(false, $infoUser -> user['login']);
 ...
 if ($infoUser -> user['user_type'] != 'administrator' && (!empty($userCourses) || !empty($userLessons))) {
  ...
  foreach ($userCourses as $courseId => $value) {
   ...
   $data[$courseId] = array(_NAME => $value['name'],
         ...
         'active' => $value['active']);
   $courseLessons = $infoUser -> getUserStatusInCourseLessons(new MagesterCourse($value));
   if (!empty($courseLessons)) {
    ...
    $result = MagesterStats :: getStudentsDoneTests($courseLessons, $infoUser -> user['login']);
    $userDoneTests = array();
...
*/
  ...
  $result = MagesterStats :: getStudentsDoneTests($userLessons, $editedUser -> user['login']);
  foreach ($result[$infoUser -> user['login']] as $value) {
   $userDoneTests[$value['lessons_ID']][] = $value;
  }
