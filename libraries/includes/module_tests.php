...
/*
    ...
    if (isset($_GET['edit_test']) && in_array($_GET['edit'], $legalUnits)) {
        $currentTest = new MagesterTest($_GET['edit'], true);
...
*/
    ...
    if (isset($_GET['edit']) && in_array($_GET['edit'], $legalUnits)) {
        $unit = new MagesterUnit($_GET['edit']);
        if ($unit->isTest()) {
            $test = new MagesterTest($_GET['edit'], true);
    ...
    if (isset($_GET['delete_test']) && in_array($_GET['delete_test'], $legalValues)) {
        try {
            ...
            $currentTest = new MagesterTest($_GET['delete_test']);
            $currentTest->delete();
        } catch (Exception $e) {
            header("HTTP/1.0 500 ");
        ...
        try {
            ...
            $currentTest = new MagesterTest($_GET['publish_test']);
        ...
        if (isset($_GET['view_unit'])) {
            ...
            $showTest = new MagesterTest($_GET['show_test']);
        }
        ...
        if (!$skillgap_tests) {
            ...
            $iterator = new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent->tree), RecursiveIteratorIterator::SELF_FIRST), array('active' => 1));
        ...
        $currentTest = new MagesterTest($_GET['questions_order']);
        ...
        $currentTest = new MagesterTest($_GET['test_results']);
        $doneTests = MagesterStats::getDoneTestsPerTest(false, $currentTest->test['id']);
        ...
        if (isset($_GET['ajax']) && $_GET['reset_all'] == 1) {
            try {
                if (!in_array($_GET['login'], array_keys($doneTests[$currentTest->test['id']]))) {
                    throw new MagesterTestException(_INVALIDLOGIN.': '.$_GET['login'], MagesterTestException::INVALID_LOGIN);
                }
 ...
 */
        ...
        if (!$skillgap_tests) {
            ...
            foreach ($tests as $key => $test) {
                $names = array();
                foreach ($iterator = new MagesterAttributeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent->getNodeAncestors($test['content_ID']))), array('name')) as $k => $v) {
                    $names[] = $v;
                }
        ...
        foreach ($tests as $key => $test) {
            ...
            $doneTests = MagesterStats::getDoneTestsPerTest(false, $test['id']);
            ...
            foreach ($questions as $key => $question) {
                ...
                if ($question['content_ID'] && isset($currentContent)) {
                    if (!isset($names[$question['content_ID']])) {
                        foreach ($iterator = new MagesterAttributeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent->getNodeAncestors($question['content_ID']))), array('name')) as $k => $v) {
    ...
    try {
        if ($_student_) {
            ...
            if (isset($_GET['view_unit']) && eF_checkParameter($_GET['view_unit'], 'id') && (!($GLOBALS['currentLesson']->options['rules']) || $ruleCheck === true)) {
                ...
                $visitableIterator = new MagesterTestsFilterIterator(new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent->tree), RecursiveIteratorIterator::SELF_FIRST))));
            ...
            if ($skillgap_tests && isset($_GET['create_random_test']) && isset($_GET['create_random_test']) && eF_checkParameter($_GET['create_random_test'], 'id') && in_array($_GET['create_random_test'], $availableTests)) {
                ...
                if ($form->isSubmitted()) {
                    ...
                    foreach ($directions_to_cover as $directionId => $directionInfo) {
                        $direction = new MagesterDirection($directionId);
                        ...
                        foreach ($directionCourses as $courseId => $courseInfo) {
                            ...
                            $courseLessons = MagesterCourse::convertLessonObjectsToArrays($course->getCourseLessons());
