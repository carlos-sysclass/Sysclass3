 ...
 * SysClass email digests

 *

 * This page is used for the functionalities SysClass email digests functionalities
...
if (isset($_GET['add_default']) && $_GET['add_default'] == 1) {
 MagesterNotification::addDefaultNotifications();
}
if (isset($_GET['activate_notification'])) {
 if (isset($_GET['event']) && $_GET['event']) {
  MagesterNotification::activateEventNotification($_GET['activate_notification']);
 } else {
  $notification = new MagesterNotification($_GET['activate_notification']);
 ...
 if (isset($_GET['event']) && $_GET['event']) {
  ...
  $notification = new MagesterNotification($_GET['deactivate_notification']);
...
try {
 if (isset($_GET['delete_notification']) ) {
  if (isset($_GET['event'])) {
   MagesterNotification::deleteEventNotification($_GET['delete_notification']);
  } else {
   $notification = new MagesterNotification($_GET['delete_notification']);
   $notification -> delete();
  }
 ...
 if (isset($_GET['add_notification']) || isset($_GET['edit_notification'])) {
  ...
  $options = array(
            ...
            if ($GLOBALS['configuration']['onelanguage']) {
             ...
             $form -> addElement('select', 'languages_NAME', _LANGUAGE, MagesterSystem :: getLanguages(true) , 'class = "inputSelectMed" onchange="addLanguageTag(this)"');
            ...
            if ($_GET['edit_notification'] && $_GET['event'] == "1") {
             ...
             if ($mode == "courses") {
              ...
              $basic_event_recipients[MagesterNotification::ALLLESSONUSERS] = _ALLLESSONUSERS;
              $basic_event_recipients[MagesterNotification::LESSONUSERSNOTCOMPLETED] = _LESSONUSERSNOTCOMPLETED;
              $basic_event_recipients[MagesterNotification::LESSONPROFESSORS] = _LESSONPROFESSORS;
             }
            }

            $smarty -> assign("T_LESSON_EVENT_RECIPIENTS",array( "alllesson" => MagesterNotification::ALLLESSONUSERS, "lessonprof" => MagesterNotification::LESSONPROFESSORS, "lessonnotcompleted" => MagesterNotification::LESSONUSERSNOTCOMPLETED, "explicitlyselected" => MagesterNotification::EXPLICITLYSEL));
            $smarty -> assign("T_COURSE_EVENT_RECIPIENTS",array( "courseprof" => MagesterNotification::COURSEPROFESSORS, "allcourse" => MagesterNotification::ALLCOURSEUSERS));
              ...
              if ($notification_type == "0") {
               ...
               if (isset($_GET['add_notification'])) {
                ...
                if (!isset($_GET['event'])) {
                 ...
                 MagesterNotification::editEventNotification($_GET['edit_notification'], $events_type, $subject, $message, $condition, $_POST['event_recipients'], $html_message, $after_time, $send_immediately);
                }
               }
  ...
  if (isset($_GET['ajax']) && $_GET['ajax'] == 'msgQueueTable') {
     ...
     if (is_array($sending_queue_msg['send_conditions'])) {
      if (isset($sending_queue_msg['send_conditions']['lessons_ID'])) {
       if ($sending_queue_msg['send_conditions']['lessons_ID'] != 0) {
        $lesson = new MagesterLesson($sending_queue_msg['send_conditions']['lessons_ID']);
   ...
   foreach ($sending_queue_msgs as $key => $message) {
    $notification = new MagesterNotification($message);
   ...
   try {
    foreach ($notifications as $key => $notification) {
     ...
     if (isset($notification['send_conditions']['lessons_ID'])) {
      ...
      if ($events_table[$notification['event_type']]['category'] == "system" || $events_table[$notification['event_type']]['category'] == "branch" || $events_table[$notification['event_type']]['category'] == "job" || $notification['event_type'] == MagesterEvent::NEW_SYSTEM_ANNOUNCEMENT) {
  ...
  if ($config_form -> isSubmitted()) {
   foreach ($notification_configurations as $conf_option) {
    MagesterConfiguration :: setValue($conf_option, $config_form -> exportValue($conf_option));
   }
