...
$user_types = sC_getTableDataFlat("user_types", "user_type");
...
$form -> addElement('radio', 'recipients', null, null, 'all_users', 'selected = "selected" onclick = "sC_js_selectRecipients(\'all_users\')"');
...
$form -> addElement('radio', 'recipients', null, null, 'active_users', 'onclick = "sC_js_selectRecipients(\'active_users\')"');
...
$form -> addElement('radio', 'recipients', null, null, 'specific_lesson', 'onclick = "sC_js_selectRecipients(\'specific_lesson\')"');//'onclick = "document.getElementById(\'lesson_recipients\').disabled = \'\';document.getElementById(\'user_type_recipients\').disabled = \'disabled\';document.getElementById(\'user_recipients\').disabled = \'disabled\'"');
...
$form -> addElement('radio', 'recipients', null, null, 'specific_type', 'onclick = "sC_js_selectRecipients(\'specific_type\')"');//'onclick = "document.getElementById(\'lesson_recipients\').disabled = \'disabled\';document.getElementById(\'user_type_recipients\').disabled = \'\';document.getElementById(\'user_recipients\').disabled = \'disabled\'"');
...
$form -> addElement('radio', 'recipients', null, null, 'specific_user', 'onclick = "sC_js_selectRecipients(\'specific_user\')"');//'onclick = "document.getElementById(\'lesson_recipients\').disabled = \'disabled\';document.getElementById(\'user_type_recipients\').disabled = \'disabled\';document.getElementById(\'user_recipients\').disabled = \'\'"');
...
if ($form -> isSubmitted()) {
    if ($form -> validate()) {
        ...
        case 'all_users':
            $result = sC_getTableDataFlat("users", "login, email");
            break;
        case 'active_users':
            $result = sC_getTableDataFlat("users", "login, email", "active=1");
            break;
        case 'specific_lesson':
            $result = sC_getTableDataFlat("users, users_to_lessons", "login, email", "users_to_lessons.archive=0 and users.active=1 AND users_to_lessons.active=1 AND users.login=users_to_lessons.users_LOGIN AND users_to_lessons.lessons_ID=".($form -> exportValue('lesson')));
            break;
        case 'specific_type':
            $result = sC_getTableDataFlat("users", "login, email", "users.active=1 AND users.user_type='".($form -> exportValue('user_type'))."'");
            break;
        case 'specific_user':
            $result = sC_getTableDataFlat("users", "login, email", "login = '".($form -> exportValue('user'))."'");
            break;
        ...
        case 'specific_branch_job_description':
            ...
            if ($form -> exportValue('job_description_recipients') != "0") {
                ...
                $result = sC_getTableDataFlat("users JOIN module_hcd_employee_works_at_branch ON users.login = module_hcd_employee_works_at_branch.users_login","distinct login, email", "users.active = 1 AND module_hcd_employee_works_at_branch.branch_ID IN (".$branches_list.") AND module_hcd_employee_works_at_branch.assigned = '1'");
            }
            break;
        case 'specific_skill':
            $result = sC_getTableDataFlat("users JOIN module_hcd_employee_has_skill ON users.login = module_hcd_employee_has_skill.users_login","distinct login, email", "users.active = 1 AND module_hcd_employee_has_skill.skill_ID = '".$form -> exportValue('skill_recipients') ."'");
            break;
        case 'specific_group':
            $result = sC_getTableDataFlat("users JOIN users_to_groups ON users.login = users_to_groups.users_LOGIN","distinct login, email", "users_to_groups.groups_ID = '".$form -> exportValue('group_recipients') ."'");
            break;
        default:
            break;
        }
        foreach ($result['email'] as $key => $value) {
            if (!sC_checkParameter($value, 'email')) {
        ...
        if (sizeof($result['email']) > 0) {
            ...
            // Debug:
            //pr($result);
            $newresult = sC_mail($GLOBALS['configuration']['system_email'], $recipient, $form -> exportValue('subject'), $form -> exportValue('body'));
            if ($newresult == true) {
