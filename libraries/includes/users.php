...
if (isset($_GET['delete_user']) && eF_checkParameter($_GET['delete_user'], 'login')) { //The administrator asked to delete a user
    ...
    include 'includes/personal.php'; //User addition and manipulation is done through personal.
        ...
        $sql = "SELECT DISTINCT
                    u.*,
                    (SELECT count(ul.lessons_ID) FROM users_to_lessons as ul, lessons as l
                        WHERE ul.lessons_ID=l.id AND ul.archive=0 AND l.archive=0 AND ul.users_LOGIN = u.login)
                        as lessons_num,
        ...
        /* Como Assim? I'd not understod this code from the previous way of doing it */
