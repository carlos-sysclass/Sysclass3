...
if (!isset($GLOBALS['currentLesson'])) {
    if (isset($_GET['lesson_info'])) {
        $currentLesson = new MagesterLesson($_GET['lesson_info']);
  ...
  if ($lessonInformation[$key] != "" ) {
   $lessonInformation[$key] = str_replace ("\n","<br />", $lessonInformation[$key]);
    ...
    if (!$_admin_) {
        ...
        list($conditionsStatus, $lessonPassed) = MagesterStats :: checkConditions($seenContent[$currentLesson -> lesson['id']][$currentUser -> user['login']], $conditions, $visitableContentIds, $testsIds);
