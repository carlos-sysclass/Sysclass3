...
if (str_replace(DIRECTORY_SEPARATOR, "/", __FILE__) == $_SERVER['SCRIPT_FILENAME']) {
 ...
 /********************* SHOW PROFILE POPUP ******************/
  ...
  if (isset($_GET['user'])) {
   ...
   try {
    $avatarfile = new MagesterFile($shownUser -> user['avatar']);
   } catch (MagesterFileException $ex) {
     ...
     if (isset($_GET['action']) && $_GET['action'] == 'change' && isset($id)) {
      ...
      if (eF_insertTableData("profile_comments", $comments_content)) {
       ...
       if ($_SESSION['s_login'] == $_GET['user']) {
        MagesterEvent::triggerEvent(array("type" => MagesterEvent::NEW_PROFILE_COMMENT_FOR_SELF, "users_LOGIN" => $_SESSION['s_login'], "users_name" => $currentUser -> user['name'], "users_surname" => $currentUser -> user['surname']));
       } else {
        $commentedUser = MagesterUserFactory::factory($_GET['user']);
        MagesterEvent::triggerEvent(array("type" => MagesterEvent::NEW_PROFILE_COMMENT_FOR_OTHER, "users_LOGIN" => $_SESSION['s_login'], "users_name" => $currentUser -> user['name'], "users_surname" => $currentUser -> user['surname'], "entity_ID" => $_GET['user'], "entity_name" => $commentedUser -> user['name'] . " " . $commentedUser -> user['surname']));
       }
 ...
 /********************* TIMELINES: Lesson and System ******************/
  ...
  if (isset ($_GET['lessons_ID'])) {
   if ($currentLesson -> lesson['lesson_ID'] == $_GET['lessons_ID']) {
    ...
    try {
     $editedLesson = new MagesterLesson($_GET['lessons_ID']);
