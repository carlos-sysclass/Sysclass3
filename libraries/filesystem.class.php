 ...
 */
class MagesterFileException extends Exception
{
 ...
 * Class for files in Magester file system
 ...
 */
class MagesterFile extends ArrayObject
{
    /**
     ...
     * $file = new MagesterFile(43);                                  //Instantiate object using id

     * $file = new MagesterFile('/var/www/test.txt');                 //Instantiate object using path
    ...
    function __construct($file) {
        ...
        $fileArray['name'] = MagesterFile :: decode(basename($fileArray['path']));
        ...
        isset(MagesterFile :: $mimeTypes[strtolower($fileArray['extension'])]) ? $fileArray['mime_type'] = MagesterFile :: $mimeTypes[strtolower($fileArray['extension'])] : $fileArray['mime_type'] = 'application/'.$fileArray['extension'];
        ...
        if (!is_file($this['path'])) { //If the file does not actually exist, then delete it from database and issue exception
            ...
            throw new MagesterFileException(_ILLEGALPATH.': '.$this['path'], MagesterFileException :: ILLEGAL_PATH); //The file must be inside root path, otherwise it is illegal
        }
    }
    /**
     ...
     * $file = new MagesterFile(34);                          //Instantiate file
     ...
     * $file = new MagesterFile(43);                                  //Instantiate file object
     ...
     * for the new file (An MagesterFile object corresponds to a file without DB representation
     ...
     * @return MagesterFile The copied file
    ...
    public function copy($destinationPath, $overwrite = true) {
        ...
        if (!$overwrite && is_file($destinationPath)) {
            throw new MagesterFileException(_CANNOTCOPYFILE.': '.$destinationPath.', '._FILEALREADYEXISTS, MagesterFileException :: FILE_ALREADY_EXISTS);//Use plain Exception rather than MagesterFileException, since the latter is caught right from the following catch block
        }
            ...
            if ($this['id'] != -1) {
                ...
                if ($fileId) {
                    ...
                    foreach ($fileMetadataArray as $key => $value) {
                        MagesterSearch :: insertText($value, $fileId, "files", "data");
                    }
                }
            }
    ...
    public function rename($destinationPath, $overwrite = false) {
        ...
        if (!$overwrite && (is_file($destinationPath))) {
            throw new MagesterFileException(_CANNOTMOVEFILE.': '.$this['name'].', '._FILEALREADYEXISTS, MagesterFileException :: FILE_ALREADY_EXISTS);//Use plain Exception rather than MagesterFileException, since the latter is caught right from the following catch block
        }
        if ($this['path'] != $destinationPath) {
            if (copy($this['path'], $destinationPath)) { //rename() acts as a move() function as well
                ...
                throw new MagesterFileException(_CANNOTMOVEFILE, MagesterFileException :: UNKNOWN_ERROR);
            }
        }
    }
    /**
     ...
     * This function does not apply to MagesterFile objects that don't have a database
    ...
    public function refresh() {
        ...
        $this['name'] = MagesterFile :: decode(basename($this['path']));
 ...
 /**
     ...
     * @return MagesterFile The compressed file
    ...
    public function compress($zipName = false, $decode = false) {
     if (!$zipName) {
            ...
            $zipName = $this['directory'].'/'.(MagesterFile :: encode(basename($zipName)));
        }
        try { //This way we delete the file, if it already exists
            $file = new MagesterFile($zipName);
            $file -> delete();
        } catch (Exception $e) {}
        if ($GLOBALS['configuration']['zip_method'] == 'system') {
            ...
            if ($zip -> open($zipName, ZIPARCHIVE::CREATE ) === true) {
                if ($decode) {
                 $zip -> addFile($this['path'], MagesterFile :: decode($this['name']));
                } else {
     ...
     * $file = new MagesterFile('/var/www/test.zip');
     ...
     * @return array An array of MagesterFile objects or file paths (depending on wheter a database representation exists)
     ...
     * $file = new MagesterFile('mydata.zip');
    ...
    public function listContents() {
        if ($this['extension'] == 'zip') {
            ...
            throw new _MagesterFileException(_UNSUPPORTEDFILETYPE.': '.$this['extension'], MagesterFileException :: UNKNOWN_COMPRESSION);
        }
    }
    /**
    ...
    public function share($lessonId = false) {
        ...
        if ($lessonId) {
            ...
            throw new MagesterFileException(_CANNOTSHAREFILE.': '.$this['path'], MagesterFileException :: NOT_LESSON_FILE);
        }
    }
    /**
     ...
     * $newName = MagesterFile :: encode($name);			//Encodeded version of name

     * </code>

     * A little word about the need of encoding:

     * Throughut SysClass UTF-8 is used as encoding. When uploading a file, for example, its name is encoded
     ...
     * $name = MagesterFile :: decode($encodedName);
 ...
 * Class for directories in Magester file system
 ...
 */
class MagesterDirectory extends ArrayObject
{
    /**
     ...
     * $file = new MagesterDirectory(43);                                  //Instantiate object using id

     * $file = new MagesterDirectory('/var/www/32/');                      //Instantiate object using path
    ...
    function __construct($directory) {
        ...
        if (strpos($directory, '/ult/') !== false) {
        	$directory = str_replace("/ult/", "/posult/", $directory);
        }
        */
     ...
     * echo MagesterDirectory :: normalize($path);	//Outputs c:/test.php
     ...
     * $directory = new MagesterDirectory(34);                     //Instantiate directory
        ...
        foreach ($it as $node => $value) {
            if ($value -> isFile()) {
                ...
                if (isset($files[$current])) {
                    $current = new MagesterFile($files[$current]);
                } else {
                    $fileArray = array('id' => -1, //Set 'id' to -1, meaning this file has not a database representation
                                       'path' => $value -> getPathName());
                    $current = new MagesterFile($fileArray);
                }
                $current -> delete();
            }
        }
     ...
     * $directory = new MagesterDirectory(43);                            //Instantiate directory object
     ...
     * @return MagesterDirectory The copied directory
    ...
    public function copy($destinationPath, $overwrite = false) {
        ...
        if (!is_dir($destinationPath)) {
            ...
            throw new MagesterFileException(_CANNOTCOPYDIRECTORY.': '.$destinationPath.', '._FILEALREADYEXISTS, MagesterFileException :: DIRECTORY_ALREADY_EXISTS);
        }
        $it = new MagesterREFilterIterator(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($this['path']), RecursiveIteratorIterator :: SELF_FIRST), array('/.svn/'), false);
        foreach ($it as $node => $value) {
            if ($value -> isDir()) {
     ...
     * @return MagesterDirectory The renamed/moved directory
    ...
    public function rename($destinationPath, $overwrite = false) {
        ...
        unset($it);
        rmdir($this['path']);
        $directory = new MagesterDirectory($destinationPath);
     ...
     * $directory = new MagesterDirectory('/var/www/magester/www/content/lessons/32/test_folder');
            ...
            if ($zip -> open($zipName, ZIPARCHIVE::CREATE ) === true) {
                ...
                foreach ($it as $node => $value) {
                    if ($value -> isFile()) {
                        ...
                        if ($decode) {
                            $zip -> addFile($node, MagesterFile :: decode($nameInZip));
                        } else {
     ...
     * MagesterDirectory :: createDirectory('/var/www/magester/www/content/lessons/32/new directory');
    ...
    public static function createDirectory($fullPath) {
        ...
        if (is_dir($fullPath)) {
            throw new Exception(_COULDNOTCREATEDIRECTORY.': '.$fullPath.', '._DIRECTORYALREADYEXISTS, MagesterFileException :: DIRECTORY_ALREADY_EXISTS);
        }
     ...
     * similar to MagesterFile :: getTypeImage().
 ...
 */
class FileSystemTree extends MagesterTree
{
    /**
    ...
    function __construct($dir = G_ROOTPATH, $shallow = false) {
        ...
        //pr($dir);echo "2";
        if (!is_dir($dir['path'])) {
            throw new MagesterFileException(_DIRECTORYDOESNOTEXIST.': '.$dir['path'], MagesterFileException :: DIRECTORY_NOT_EXIST);
        }
        $this -> dir = $dir;
        $this -> shallow = $shallow;
        if ($shallow) {
            $this -> iterator = new MagesterREFilterIterator(new RecursiveDirectoryIterator($this -> dir['path']), array('/.svn/', '/.htaccess/'), false);
        } else {
            $this -> iterator = new MagesterREFilterIterator(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($this -> dir['path']), RecursiveIteratorIterator :: SELF_FIRST), array('/.svn/', '/.htaccess/'), false);
        }
        $this -> reset();
    }
    /**
     ...
     * This function is not used by this MagesterTree implementation,

     * so it always returns false

     *

     * @param MagesterFile $node
    ...
    public function reset() {
        ...
        $it = $this -> iterator;
        //$it = new MagesterREFilterIterator(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($this -> dir['path']), RecursiveIteratorIterator :: SELF_FIRST), array('/.svn/', '/.htaccess/'), false);
        ...
        foreach ($it as $node => $value) {
            ...
            if (isset($files[$current])) {
                try {
                    $nodes[$current] = new MagesterFile($files[$current]);
                } catch (MagesterFileException $e) {
                    //Don't halt for illegal file arrays; The MagesterFile class constructor handles them properly in the database
                }
            } else {
                if ($value -> isFile()) {
                    $fileArray = array('id' => -1, //Set 'id' to -1, meaning this file/directory has not a database representation
                                       'path' => $current);
                    $nodes[$current] = new MagesterFile($fileArray);
                } else {
                    $nodes[$current] = new MagesterDirectory($current);
                }
            }
        }
     ...
     * @return MagesterFile The uploaded file
     ...
     * @return MagesterDirectory The created directory
    ...
    /*
     ...
     $curDir = MagesterDirectory :: normalize($form -> exportValue('copy_current_directory'));
     ...
     throw new MagesterFileException(_ILLEGALPATH.': '.$form -> exportValue('destination'), MagesterFileException :: ILLEGAL_PATH);
    ...
    public function toHTML($url, $currentDirectory = '', $ajaxOptions = array(), $options, $extraFileTools = array(), $extraDirectoryTools = array(), $extraHeaderOptions = array(), $defaultIterator = false, $show_tooltip = true, $extraColumns = array()) {
        ...
        //$currentDirectory = new MagesterDirectory($currentDirectory);
        if ($currentDirectory instanceof MagesterDirectory) {
            $currentDirectory = $currentDirectory['path'];
        }
        if (isset($_POST['upload_current_directory']) && strpos(MagesterDirectory :: normalize($_POST['upload_current_directory']), rtrim(G_ROOTPATH, "/")) !== false) {
            $currentDirectory = $_POST['upload_current_directory'];
        }
        if (isset($_POST['current_directory']) && strpos(MagesterDirectory :: normalize($_POST['current_directory']), rtrim(G_ROOTPATH, "/")) !== false) {
            $currentDirectory = $_POST['current_directory'];
        }
        if (isset($_POST['copy_current_directory']) && strpos(MagesterDirectory :: normalize($_POST['copy_current_directory']), rtrim(G_ROOTPATH, "/")) !== false) {
        ...
        if ($currentDirectory && $currentDirectory != $this -> dir['path']) {
            ...
            $currentDir = new MagesterDirectory($currentDirectory);
        ...
        */
            $iterator = new MagesterFileOnlyFilterIterator(new MagesterNodeFilterIterator(new ArrayIterator($currentDir))); //Plain ArrayIterator so that it iterates only on the current folder's files
            if ($options['db_files_only']) { //Filter out directories without database representation
                $iterator = new MagesterDBOnlyFilterIterator($iterator);
            }
        //}
  ...
  if ($_SESSION['supervises_branches'] != "" ) {
   ...
   foreach ($employees as $key2 => $value2) {
    ...
    $value = new MagesterFile($value); //Restore file/directory representation, so we can use its methods
 ...
 */
     ...
     * @param MagesterUser $currentUser The current user
        ...
        if (isset($_GET['delete']) && (eF_checkParameter($_GET['delete'], 'id') || strpos(urldecode($_GET['delete']), $this -> dir['path']) !== false)) {
            ...
            try {
                ...
                if ($file['name'] != $_GET['name']) {
                    $file -> rename(dirname($file['path']).'/'.MagesterFile :: encode(urldecode($_GET['name'])));
                }
    ...
    public function uploadFile($fieldName, $destinationDirectory = false, $offset = false) {
        ...
        if (!($destinationDirectory instanceof MagesterDirectory)) {
            $destinationDirectory = new MagesterDirectory($destinationDirectory);
        }
            ...
            if ($error) {
                switch ($error) {
                    case UPLOAD_ERR_INI_SIZE :
                        throw new MagesterFileException(_THEFILE." "._MUSTBESMALLERTHAN." ".ini_get('upload_max_filesize'), UPLOAD_ERR_INI_SIZE );
                        break;
                    case UPLOAD_ERR_FORM_SIZE :
                        throw new MagesterFileException(_THEFILE." "._MUSTBESMALLERTHAN." ".sprintf("%.0f", $_POST['MAX_FILE_SIZE']/1024)." "._KILOBYTES, UPLOAD_ERR_FORM_SIZE);
                        break;
                    case UPLOAD_ERR_PARTIAL :
                        throw new MagesterFileException(_FILEWASPARTIALLYUPLOADED, UPLOAD_ERR_PARTIAL);
                        break;
                    case UPLOAD_ERR_NO_FILE :
                        throw new MagesterFileException(_NOFILEUPLOADED, UPLOAD_ERR_NO_FILE);
                        break;
                    case UPLOAD_ERR_NO_TMP_DIR :
                        throw new MagesterFileException(_NOTMPDIR, UPLOAD_ERR_NO_TMP_DIR);
                        break;
                    case UPLOAD_ERR_CANT_WRITE :
                        throw new MagesterFileException(_UPLOADCANTWRITE, UPLOAD_ERR_CANT_WRITE);
                        break;
                    case UPLOAD_ERR_EXTENSION :
                        throw new MagesterFileException(_UPLOADERREXTENSION, UPLOAD_ERR_EXTENSION);
                        break;
                    default:
                        throw new MagesterFileException(_ERRORUPLOADINGFILE, MagesterFileException :: UNKNOWN_ERROR);
                        break;
                }
            } elseif ($size == 0) {
      ...
      $blackList = array();
        ...
        foreach ($blackList as $value) {
            if ($extension == trim(mb_strtolower($value))) {
                throw new MagesterFileException(_YOUCANNOTUPLOADFILESWITHTHISEXTENSION.': '.$extension, MagesterFileException::FILE_IN_BLACK_LIST);
            }
        }
      if ($GLOBALS['configuration']['file_white_list'] != '') {
       ...
       $whiteList = array();
        ...
        if (sizeof($whiteList) > 0 && !in_array($extension, $whiteList)) {
            throw new MagesterFileException(_YOUMAYONLYUPLOADFILESWITHEXTENSION.': '.$GLOBALS['configuration']['file_white_list'], MagesterFileException::FILE_NOT_IN_WHITE_LIST);
        }
    }
    /**
 ...
 */
class MagesterDirectoryOnlyFilterIterator extends FilterIterator
{
    /**
 ...
 */
class MagesterFileOnlyFilterIterator extends FilterIterator
{
    /**
 ...
 */
class MagesterDBOnlyFilterIterator extends FilterIterator
{
    /**
 ...
 */
class MagesterREFilterIterator extends FilterIterator
{
    /**
 ...
 */
class MagesterFileTypeFilterIterator extends FilterIterator
{
    /**
