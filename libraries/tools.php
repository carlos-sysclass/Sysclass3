...
function checkPermissions($dir) {
 ...
 foreach (new RecursiveIteratorIterator($d, RecursiveIteratorIterator::SELF_FIRST) as $key => $path) {
  if (!$path -> isWritable()) {
   if ($path->isDir()) {
    foreach ($magesterDirectories as $key2 => $value2)
...
function eF_getMenu()
{
   ...
   if (!isset($GLOBALS['currentUser'] -> coreAccess['dashboard']) || $GLOBALS['currentUser'] -> coreAccess['dashboard'] != 'hidden') {
    $menu['general']['personal'] = array('title' => _SETTINGS, 'link' => 'professor.php?ctg=personal', 'image' => 'user');
        ...
        case 'student':
            if ($_SESSION['s_lessons_ID'] != false) {
				//if (!isset($GLOBALS['currentUser'] -> coreAccess['control_panel']) || $GLOBALS['currentUser'] -> coreAccess['control_panel'] != 'hidden') {
				...
				//}
                if ($GLOBALS['currentUser'] -> coreAccess['content'] != 'hidden') {
                    $currentContent = new MagesterContentTree($_SESSION['s_lessons_ID']);
                    foreach ($iterator = new MagesterVisitableFilterIterator(new MagesterTheoryFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent -> tree), RecursiveIteratorIterator :: SELF_FIRST)))) as $key => $value) {
                        $theoryContentIds[$key] = $key;
                    }
                    foreach ($iterator = new MagesterVisitableFilterIterator(new MagesterExampleFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent -> tree), RecursiveIteratorIterator :: SELF_FIRST)))) as $key => $value) {
                        $exampleContentIds[$key] = $key;
                    }
                    foreach ($iterator = new MagesterVisitableFilterIterator(new MagesterTestsFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($currentContent -> tree), RecursiveIteratorIterator :: SELF_FIRST)))) as $key => $value) {
...
* correctly from the SysClass functions
...
function eF_getTimezones() {
  ...
  $timezones['America/La_Paz'] = "(GMT -04:00) La Paz";
        ...
        $timezones['America/Buenos_Aires'] = "(GMT -03:00) Brasilia, Buenos Aires, Georgetown, Greenland";
...
function eF_mail($sender, $recipient, $subject, $body, $attachments = false, $onlyText = false, $bcc = false) {
    ...
    if ($attachments) {
        $file = new MagesterFile($attachments[0]);
...
//	$mainScripts[]	= 'jquery/jquery.uniform.min';
	...
	$mainScripts[]	= 'jquery/date';
	$mainScripts[]	= 'jquery/jquery.topic';
