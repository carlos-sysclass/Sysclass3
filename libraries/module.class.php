...
* - onInstall()				// Function to be executed when the module is installed to an SysClass system

* - onUninstall()				// Function to be executed when a module is deleted from a SysClass system
...
abstract class MagesterModule
{
	...
	* The time of the event is implicitly printed by the SysClass system and should not be provided by your defined event messages
	...
	* 'id': a unique id of the link within the module (and NOT within the entire SysClass) framework. This id is used for link highlighting purposes
	...
	* '_magesterExtensions': you may optionally define two images for each link: one .png and .gif, which will appear under FF and IE respectively.

	*                     The filename (without the extension) and the path of the two pictures must be the same.

	*                     If '_magesterExtensions' => 1, then do not use an extension to the image filename
	...
	*  Example implementation:
	...
	'_magesterExtensions' => '1',                               // question_type_free_text.png and pens.gif must exist in 16x16
	...
	'_magesterExtensions' => '1',                                  // my_module_pic.gif and my_module_pic.png must exist in $this->moduleBaseDir . 'images/'
