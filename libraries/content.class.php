...
class MagesterUnit extends ArrayObject
{
    ...
    function __construct($array) {
        if (!is_array($array)) {
            if (eF_checkParameter($array, 'id')) {
                ...
                throw new MagesterContentException(_INVALIDID.": $array", MagesterContentException :: INVALID_ID);
            }
  }
  //pr($array['options']);exit;
    ...
    public function persist() {
        ...
        //The special string magester#special#text is used in order to remove the (heavy) content from the nodes when traversing it. However,
        //there is a chance that the tree traversal persists values as well. So, using this special string, we know that we must not
        //alter the content
        if ($this['data'] == 'magester#special#text') {
            unset($fields['data']);
        }
   ...
   /**
    ...
    public function getFiles($returnObjects = false) {
        ...
        foreach ($filesId as $file) {
            $returnObjects ? $files[] = new MagesterFile($file) : $files[] = $file;
        }
...
class MagesterContentTree extends MagesterTree
{
    ...
    public function reset() {
        ...
        foreach ($units as $node) { //Assign previous content ids as keys to the previousNodes array, which will be used for sorting afterwards
            if (!$this -> data) {
                $node['has_data'] ? $node['data'] = 'magester#special#text' : $node['data'] = ''; //Eliminate with 'magester#special#text' data for units that don't have any content, and set an empty space ' ' for units that have content. This is done so that the toHTML can handle differently the ones from the others. The magester#special#text is checked by persist() in order to leave data unchanged in case we are updating
            }
            $node = new MagesterUnit($node); //We convert arrays to array objects, which is best for manipulating data through iterators
        ...
        if (sizeof($rejected) > 0) { //Append rejected nodes to the end of the tree array, updating their parent/previous information
            foreach (new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($tree), RecursiveIteratorIterator :: SELF_FIRST)) as $lastUnit); //Advance to the last tree node
...
*/
    ...
    public function removeNode($removeId) {
        ...
        if ($iterator -> valid()) {
            ...
            throw new MagesterContentException(_UNITDOESNOTEXIST.': '.$removeId, MagesterContentException :: UNIT_NOT_EXISTS);
        }
    }
    /**
    ...
    public function insertNode($unit, $parentUnit = false, $previousUnit = false) {
        if (!isset($unit['id'])) {
            if (!isset($unit['previous_content_ID'])) {
                ...
                foreach ($iterator = new MagesterAttributeFilterIterator(new RecursiveIteratorIterator($children, RecursiveIteratorIterator :: SELF_FIRST), 'id') as $lastUnitId) {} //Iterate through the parent's children until you reach the last one
                $lastUnitId ? $unit['previous_content_ID'] = $lastUnitId : $unit['previous_content_ID'] = 0; //The new unit's parent last unit
            }
            $unit = MagesterUnit :: createUnit($unit);
        }
        $iterator = new MagesterNodeFilterIterator(new RecursiveIteratorIterator($this -> tree, RecursiveIteratorIterator :: SELF_FIRST)); //Get an iterator for the current tree. This iterator returns only whole unit arrays and not unit members separately (such as id, timestamp etc)
        $iterator -> rewind(); //Initialize iterator
        ...
        if ($iterator -> valid()) { //If there is a next unit
            ...
            foreach (new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($unit), RecursiveIteratorIterator :: SELF_FIRST)) as $lastUnit); //If the inserted unit is actually a tree branch with children, advance to the last one
            ...
            switch ($rule['rule_type']) {
                ...
                case 'hasnot_seen':
                    if (!in_array($rule['rule_content_ID'], array_keys($seenContent))) {
                        ...
                        } catch (MagesterContentException $e) {}
                    }
                    break;
                case 'hasnot_passed':
                ...
                case 'serial':
                    ...
                    foreach ($iterator = new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($this -> tree), RecursiveIteratorIterator :: SELF_FIRST))) as $key => $value) {
    ...
    public function copyTest($testId, $targetUnit = false, $copyQuestions = true) {
        ...
        $newTest = MagesterTest::createTest($newUnit, $oldTest -> test);
  ...
  preg_match_all("/view_file\.php\?file=(\d+)/", $data, $matchesId);
        ...
        foreach ($files as $file){
         try {
          ...
          if ($e -> getCode() == MagesterFileException :: FILE_ALREADY_EXISTS) {
           $copiedFile = new MagesterFile($lesson -> getDirectory().'/'.basename($sourceFile['path']));
           str_replace("view_file.php?file=".$file, "view_file.php?file=".$copiedFile -> offsetGet('id'), $data);
        ...
        if ($sourceUnit -> offsetGet('ctg_type') == 'tests') {
            ...
            if ($targetUnit) {
                if (!($targetUnit instanceof MagesterUnit)) {
                    $targetUnit = new MagesterUnit($targetUnit);
                }
            ...
            $sourceTree = new MagesterContentTree($sourceUnit -> offsetGet('lessons_ID'), true);
     ...
     if ($position !== false) {
      ...
      $copiedFile = $sourceFile -> copy($lesson -> getDirectory().basename($sourceFile['path']), false);
         ...
         if ($file && $unit['ctg_type'] == 'scorm' || $unit['ctg_type'] == 'scorm_test') {
          $d = new MagesterDirectory(dirname($file));
