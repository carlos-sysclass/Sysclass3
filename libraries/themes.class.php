 ...
 */
class MagesterThemesException extends Exception
{
 ...
 */
class themes extends MagesterEntity
{
    /**
...
/*
        ...
        if (!is_dir($this -> {$this -> entity}['path']) && !is_file($this -> {$this -> entity}['path'])) {
            ...
            if (!fopen($this -> {$this -> entity}['path'].'theme.xml', 'r')) {
                ...
                throw new MagesterEntityException(_ENTITYNOTFOUND.': '.htmlspecialchars($this -> {$this -> entity}['path']), MagesterEntityException :: ENTITY_NOT_EXIST);
...
*/
        ...
        try {
            if (isset($this -> options['favicon'])) {
                new MagesterFile($this -> options['favicon']);
            } else {
                throw new Exception();
            }
        } catch (Exception $e) {
  ...
  return $form;
   ...
   if (mb_substr($filesList[0], 0, mb_strpos($filesList[0] , "/")) == "default") {
     throw new Exception(_DEFAULTTHEMECANNOTBEIMPORTED, MagesterThemesException :: DEFAULT_IMPORTED);
   }
   ...
   $themeFile -> delete();
                $file = new MagesterFile($themeFile['directory'].'/'.str_replace('.zip', '', $themeFile['name'])."/theme.xml");
