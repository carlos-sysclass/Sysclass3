 ...
 public function setBackground($pdf)
 {
  ...
  if ($background) {
   ...
   if($start == 'http://')
    ...
    $backgroundFile = new MagesterFile(G_DEFAULTIMAGESPATH."certificate_logos/".$background['file']);
 ...
 public function showImages($pdf)
 {
  foreach ($this->template->certificate->images->image as $img) {
   ...
   if($start == 'http://')
    ...
    $imgFile = new MagesterFile(G_DEFAULTIMAGESPATH."certificate_logos/".$img['file']);
    $imgUrl = $imgFile['path'];
   }
 ...
 public function showLogo($pdf)
 {
  ...
  if ($logo) {
   ...
   if($start == 'http://')
    ...
    $logoFile = MagesterSystem::getSystemLogo();
  ...
  if($grade)
   $this->showLabel($pdf, $courseGrade, $grade['font'], $grade['weight'], $grade['size'], $grade['color'], $grade['x'], $grade['y']);
	...
	private function showLabelAligned($p, $txt, $font, $font_weight, $font_size, $color, $x, $y, $a = '')
	{
		...
		switch ($font_weight) {
			case ($font_weight == 'Bold' || $font_weight == 'bold' || $font_weight == 'BOLD'):
				$fw = 'B';
				break;
			case ($font_weight == 'Italic' || $font_weight == 'italic' || $font_weight == 'ITALIC'):
				$fw = 'I';
				break;
			case ($font_weight == 'Bold|Italic' || $font_weight == 'bold|italic' || $font_weight == 'BOLD|ITALIC'):
				$fw = 'BI';
				break;
		}
		$p->SetFont('');
