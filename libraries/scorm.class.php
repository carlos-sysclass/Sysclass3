...
class MagesterScorm
{
    ...
    public static function import($lesson, $manifestFile, $scormFolderName, $parameters) {
        ...
        $tagArray = MagesterScorm :: parseManifest($manifestXML);
        /**
            ...
            switch ($value['tag']) {
                case 'SCHEMAVERSION':
                    ...
                    if (stripos($scormVersion, '2004') !== false && (G_VERSIONTYPE == 'community' || G_VERSIONTYPE == 'standard')) { //This additional line is used in case we have the community edition
                        throw new MagesterContentException(_SCORM2004NOTSUPPORTED, MagesterContentException::UNSUPPORTED_CONTENT);
                    }
                    break;
                case 'TITLE':
                ...
                case 'ITEM':
                    ...
                    $hide_lms_ui[$key]['is_visible'] = $value['attributes']['ISVISIBLE'];
                    if ($scorm2004) {
                        $references[$key]['IDENTIFIERREF'] = MagesterContentTreeSCORM :: form_id($value['attributes']['IDENTIFIERREF']);
 ...
 //	exit();
        if (!$scorm2004) {
            ...
            //$directory = new MagesterDirectory(G_SCORMPATH);
            //$directory -> copy(MagesterDirectory :: normalize($currentLesson -> getDirectory()).'/'.$scormFolderName, true);
     ...
     * 1. Detect organization elements. Each one will consist a separate units structure inside magester

     * 2. Dive inside an organization and detect item elements. Each <item> corresponds to a "unit" in magester
    ...
    public static function import2($lesson, $manifest) {
        ...
        foreach ($xml -> organizations -> organization as $org) {
            ...
            $previousContent = $parentContent = MagesterUnit::createUnit(array('name' => $organization[$id]['title'],
                       'parent_content_ID' => 0,
          ...
          'completionThreshold' => (string)reset($org -> xpath("$dfn:item[@identifier='$itemId']/adlcp:completionThreshold")));
                    ...
                    //@todo: nested items
                    //@todo: metadata
                    $previousContent = MagesterUnit::createUnit(array('name' => $item['title'],
