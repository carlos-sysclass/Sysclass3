 ...
 */
class MagesterStatsException extends Exception
{
...
*/
        ...
        for ($i = 0; $i < sizeof($users); $i += 20) {
         $temp = MagesterStats :: getDoneTestsPerUser(array_slice($users, $i, 10));
        }
            ...
            foreach ($value as $testId => $testData) {
             if (is_array($testData)) {
                 foreach ($testData as $done_tests_ID => $dt) {
                     if (isset($dt['test']) && (($test = unserialize($dt['test'])) instanceof MagesterCompletedTest)) {
                         $unit = $test -> getUnit();
 ...
 /**
	 ...
	 * $doneTests = MagesterStats :: getDoneTests();				//Get done instances of all tests for all users

	 * $doneTests = MagesterStats :: getDoneTests('jdoe');		//Get done instances of all tests for user 'jdoe'

	 * $doneTests = MagesterStats :: getDoneTests(false, 23);		//Get done instances of test with id 23 for all users

	 * $doneTests = MagesterStats :: getDoneTests('jdoe', 23);	//Get done instances of test with id 23 for user 'jdoe'
 ...
 public static function getDoneTestsPerTest($users = false, $test = false, $from = false, $to = false, $lesson = false)
 {
  if ($from !== false && $to !== false) {
   $timeString = " and ct.timestamp between $from and $to ";
     ...
     * MagesterStats :: getUsersTimeAll();                          			//Get statistics for all users in all lessons

     * MagesterStats :: getUsersTimeAll(1259135220, 1259394420);             //Get statistics for all users in all lessons from 1259135220 to 1259394420
   ...
   $users = array_keys($logResults);
     ...
     * MagesterStats :: getUsersTime(3, 'jdoe');                          //Get statistics for user jdoe in lesson 3

     * MagesterStats :: getUsersTime(3, array('jdoe', 'george'));         //Get statistics for users george and jdoe in lesson 3

     * MagesterStats :: getUsersTime(3);                                  //Get statistics for all users in lesson 3
     ...
     * @param int $lesson Either the lesson id or the equivalent MagesterLesson object
     ...
     * MagesterStats :: getStudentsAssignedProjects(3, 'jdoe');                           //Get statistics for user jdoe in lesson 3

     * MagesterStats :: getStudentsAssignedProjects(3, array('jdoe', 'george'));          //Get statistics for users george and jdoe in lesson 3

     * MagesterStats :: getStudentsAssignedProjects(3);                                   //Get statistics for all users in lesson 3

     * </code>

     *

     * @param mixed $lessons Either a single lesson id or ana array of lesson ids (or MagesterLesson objects)
...
/*
        ...
        $usersDoneContent      = MagesterStats :: getStudentsSeenContent($studentLessons, array_keys($users));    //Calculate the done content for users in this lesson

		$usersAssignedProjects = MagesterStats :: getStudentsAssignedProjects($studentLessons, array_keys($users));

		$usersDoneTests        = MagesterStats :: getStudentsDoneTests($studentLessons, array_keys($users));

*/
     $usersDoneContent = MagesterStats :: getStudentsSeenContent($lessons, $users, $options); //Calculate the done content for users in this lesson
     $usersAssignedProjects = array();
     if (!isset($options['noprojects']) || !$options['noprojects']) {
   $usersAssignedProjects = MagesterStats :: getStudentsAssignedProjects($lessons, $users);
     }
     $usersDoneTests = array();
     if (!isset($options['notests']) || !$options['notests']) {
      $usersDoneTests = MagesterStats :: getStudentsDoneTests($lessons, $users);
     }
  ...
  $conditions = array();
    ...
    !isset($conditions[$lesson -> lesson['id']]) ? $conditions[$lesson -> lesson['id']] = array() : null;
                ...
                $lessonContent = new MagesterContentTree($lesson);
...
/*
                ...
                foreach ($iterator = new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'theory', 'active' => 1))) as $key => $value) {

                    $visitableContentIds[$key] = $key;                                                    //Get the not-test unit ids for this content

                }

                foreach ($iterator = new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'scorm', 'active' => 1))) as $key => $value) {
                ...
                foreach ($iterator = new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'examples', 'active' => 1))) as $key => $value) {
                ...
                foreach ($iterator = new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'tests', 'active' => 1))) as $key => $value) {

                    $visitableTestIds[$key] = $key;                                                    //Get the test unit ids for this content

                }

                foreach ($iterator = new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'scorm_test', 'active' => 1))) as $key => $value) {
                ...
                foreach ($iterator = (new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'tests'))) as $key => $value) {

                    $testIds[$key] = $key;                                                    //Get the test unit ids for this content

                }

                foreach ($iterator = (new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'scorm_test'))) as $key => $value) {
    ...
    public static function getUsersCourseStatusAll($courses = false, $users = false, $options = array())
    {
        ...
        foreach ($courses as $key => $course) {
            if (!($course instanceof MagesterCourse)) {
                $courses[$key] = new MagesterCourse($course);
            }
...
/*
        ...
        $lesson instanceOf MagesterLesson ? $cacheKey .= 'lesson:'.$lesson -> lesson['id'] : $cacheKey .= 'lesson:'.$lesson;
...
*/
     $usersDoneContent = MagesterStats :: getStudentsSeenContent($lesson, $user, $options); //Calculate the done content for users in this lesson
     $usersAssignedProjects = array();
     if (!isset($options['noprojects']) || !$options['noprojects']) {
   $usersAssignedProjects = MagesterStats :: getStudentsAssignedProjects($lesson, $user);
     }
     $usersDoneTests = array();
     if (!isset($options['notests']) || !$options['notests']) {
      $usersDoneTests = MagesterStats :: getStudentsDoneTests($lesson, $user);
     }
        $roles = MagesterLessonUser :: getLessonsRoles();
        //transpose projects array, from (login => array(project id => project)) to array(lesson id => array(login => array(project id => project)))
        $temp = array();
...
*/
     ...
     * $seenUnits = MagesterStats::getStudentsSeenContent(3, 'jdoe');     //Calculate seen content for user jdoe in lesson with id 3
     ...
     * $info = MagesterStats :: getUserCommunicationInfo('jdoe');                   //Get information for user jdoe

     * </code>

     * @param mixed $user Either a user login or a MagesterUser object
     ...
     * $info = MagesterStats :: getUserUsageInfo('jdoe');                   //Get usage information for user jdoe
  ...
  $info['last_ip'] = sC_decodeIP($login_info[0]['session_ip']);
        ...
        $timeReport = new MagesterTimes(array(time() - 2592000, time()));
        $month_mean_duration = round($timeReport -> getUserMeanSessionTime($user -> login)/60);
        $timeReport = new MagesterTimes(array(time() - 604800, time()));
...
*/
     ...
     * $info = MagesterStats :: getTestInfo($tests);                   //Get information for tests 2,4
    ...
    public static function getTestInfo($tests = false, $categories = false, $show_all = false, $lesson = false)
    {
        ...
        if ($users) {
         $doneTests = MagesterStats :: getDoneTestsPerTest(array_keys($users), false, false, false, $lesson);
        }
        foreach ($tests as $id) {
            $testInfo = array();
            $test = new MagesterTest($id);
            $unit = $test -> getUnit();
     ...
     * $info = MagesterStats :: getScomTestInfo($tests);                   //Get information for tests 2,4
   ...
   $tests = $tests['id'];
     ...
     * $info = MagesterStats :: getQuestionInfo($questions);                   //Get information for questions 2,4
     ...
     * $info = MagesterStats :: getProjectInfo($questions);                   //Get information for projects 2,4
    ...
    public static function getProjectInfo($projects = false)
    {
        ...
        foreach ($projects as $project_id) {
            $project_info = array();
            $project = new MagesterProject($project_id);
     ...
     * $information = MagesterStats :: doneTestInfo(32);										//Get information for test with id 32
     ...
     * $information = MagesterStats :: doneTestInfo($doneTests[32]);							//...And give them ready to the database, thus eliminating the need for queries
    ...
    public static function doneTestInfo($testInfo)
    {
        if (is_array($testInfo)) {
            ...
            throw new MagesterStatsException(_INVALIDPARAMETER.': '.$testInfo, MagesterStatsException :: INVALID_PARAMETER);
        }
        //Initialize statistics array
        $stats = array('timesDone' => 0, 'meanScore' => 0, 'meanMeanScore' => 0, 'lastTimesMeanScore' => 0);
        //Calculate statistics per user
        foreach ($doneTests as $doneTestId => $doneTest) {
            if (!($doneTest['test'] instanceof MagesterCompletedTest)) { //Unserialize test parameter, only if needed (otherwise it is already unserialized)
 ...
 /**
	 ...
	 * $stats = MagesterStats :: getQuestionsUnitStatistics($completedTest -> questions);
	 ...
	 * @param array $questions An array of MagesterQuestion objects, taken from a completed test
 ...
 /**
	 ...
	 * $stats = MagesterStats :: getQuestionsStatistics(23);
	 ...
	 * @param mixed $test Either a test id or and MagesterTest object. If it is left blank, statistics are calculated for all tests
...
//	    pr($questionStats[78]);
 ...
 /**
	 ...
	 * $times = MagesterStats :: getUserTimes('john', $interval);
 ...
 /**
	 ...
	 * $participation = MagesterStats :: getParticipationStatistics($users, $from, $to);
