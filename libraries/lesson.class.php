...
/**
* MagesterLessonException class
*
* This class extends Exception class and is used to issue errors regarding lessons
* @author Andre Kucaniz <andre@ult.com.br>
* @package SysClass
* @version 1.0
*/
class MagesterLessonException extends Exception
{
	const NO_ERROR = 0;
...
class MagesterLesson
{
	...
	*/
	public static function validateAndSanitize($field, $type) {
		try {
			...
			if ($e -> getCode() == MagesterLessonException::INVALID_PARAMETER) {
	...
	* /var/www/magester/www/content/lessons/ and the new lesson gets the id 35,

	* a corresponding folder will be created: /var/www/magester/www/content/lessons/35/

	*

	* @param array $fields The new lesson characteristics

	* @return MagesterLesson the new lesson object
	...
	*   $lesson = new MagesterLesson(32);                     //32 is the lesson id
	...
	* Get the possible roles for a user in a lesson. This function caches MagesterLessonUser :: getLessonsRoles() results
	...
	* $courses = $this -> getCourses(true);                //Return an array of MagesterCourse objects

	* </code>

	*

	* @param boolean $returnObjects Whether to return MagesterCourse objects or a simple arrays

	* @return array The courses list, where keys are ids and values are either arrays or MagesterCourse objects
	...
	public function addUsers($users, $roles = 'student', $confirmed = true, $modality_id = 0) {
		...
		foreach ($users as $key => $user) {
			...
			if ($roleInLesson != 'administrator') {
				if ($this -> lesson['max_users'] && $this -> lesson['max_users'] <= $count++ && MagesterUser::isStudentRole($roleInLesson)) {
	...
	*/
	private function addUsersToLesson($usersData) {
		...
		foreach ($usersData as $value) {
         ...
         'from_timestamp' => $value['confirmed'] ? time() : 0,
         'user_type' => $value['role'],
		'modality_id'	=> is_numeric($value['modality_id']) ? $value['modality_id'] : 0,
         'positions' => '',
         'done_content' => '',
         'current_unit' => 0,
         'completed' => 0,
		...
		if (!defined(_DISABLE_EVENTS) || _DISABLE_EVENTS !== true) {
			foreach ($usersData as $value) {
				$event = array("type" => MagesterUser::isStudentRole($value['role']) ? MagesterEvent::LESSON_ACQUISITION_AS_STUDENT : MagesterEvent::LESSON_ACQUISITION_AS_PROFESSOR,
	...
	private function getAutoAssignProjects() {
		...
		foreach ($this -> getProjects() as $id => $project) {
			if ($project['auto_assign']) {
				$autoAssignProjects[$id] = new MagesterProject($id);
			}
		}
	...
	* @return array the lesson's tests (either an array of ids or an array of MagesterTest objects)
	...
	*/
	public function getInformation($user = false, $information = false) {
		...
		foreach (new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('active' => 1, 'ctg_type' => 'scorm_test')) as $key => $value) {
		...
		foreach (new MagesterTheoryFilterIterator(new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST)))) as $key => $value) {

		$theoryIds[$key] = $key;            //Count theory

		}

		//        foreach (new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'scorm', 'active' => 1)) as $key => $value) {
		...
		foreach (new MagesterVisitableFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'examples'))) as $key => $value) {
	...
	*/
	public function getStatisticInformation($user = false) {
		...
		$theoryIds = array();
		foreach (new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'theory', 'active' => 1)) as $key => $value) {
			$theoryIds[$key] = $key; //Count theory
		}
		foreach (new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'scorm', 'active' => 1)) as $key => $value) {
		...
		$exampleIds = array();
		foreach (new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'examples', 'active' => 1)) as $key => $value) {
			$exampleIds[$key] = $key; //Count examples
		}
		if (!$user) {
	...
	* $lesson = new MagesterLesson(13);			//Instantiate lesson object
		...
		foreach ($deleteEntities as $value) {
			switch ($value) {
				...
				case 'questions':
					...
					break;
				case 'rules':
					$content = new MagesterContentTree($this -> lesson['id']);
				...
				case 'content':
					$content = new MagesterContentTree($this -> lesson['id']);
					foreach (new MagesterNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($content -> tree), RecursiveIteratorIterator :: SELF_FIRST)) as $key => $unit) {
						$unit -> delete();
					}
					break;
				case 'users':
				...
				case 'files':
					...
					if (!$this -> lesson['share_folder'] && empty($commonFolderLessons)) {
						$directory = new MagesterDirectory($this -> directory);
						$directory -> delete();
						mkdir($this -> directory, 0755);
					}
					break;
	...
	*   $editedUser = MagesterUserFactory :: factory('joe');
	...
	* @param any _magester user object
	...
	*/
	...
	public function toIMS($path) {
		...
		$str .= '</magesterlesson>';
		if ($fp = fopen($path . "/lesson".$this->lesson['id']."/_magesterLesson.xml", "wb")) {
			fwrite($fp, $str);
		}
		fclose($fp);
	...
	*     $file = new MagesterFile($lesson -> getDirectory().'data.tar.gz');            //The file resides inside the lesson directory and is called 'data.tar.gz'
	...
	* @param MagesterFile $file The compressed lesson file object
			...
			if (isset($data['files'][$i]['file'])) {
				...
				if ($data['files'][$i]['original_name'] != basename($data['files'][$i]['file'])) {
					if (is_file(G_ROOTPATH.$newName)) {
						...
						$file = new MagesterFile(G_ROOTPATH.$newName);
						$file -> rename(G_ROOTPATH.dirname($newName).'/'.MagesterFile :: encode(rtrim($data['files'][$i]['original_name'], "/")));
					}
				}
			} else {
			...
			if (isset($data['files'][$i]['file'])) {
				...
				if ($data['files'][$i]['original_name'] != basename($data['files'][$i]['file'])) {
					if (is_dir(G_ROOTPATH.$newName)) {
						$file = new MagesterDirectory(G_ROOTPATH.$newName);
						$file -> rename(G_ROOTPATH.dirname($newName).'/'.MagesterFile :: encode(rtrim($data['files'][$i]['original_name'], "/")));
					}
				}
			}
			...
			if ($table == "lessons") { //from v3 lessons parameters also imported
				...
				for ($i = 0; $i < sizeof($tabledata); $i++) {
					...
					if ($tabledata[$i]) {
						...
						if ($table == "content") {
							MagesterSearch :: insertText($content_name, $new_id, "content", "title");
							MagesterSearch :: insertText(strip_tags($content_data), $new_id, "content", "data");
						}
	...
	* Fix magester v1.x trees

	*

	* This function is used for fixing lessons from magester version 1 that
	...
	* @return MagesterFile The object of the exported data file
	...
	*/
	public function export($exportEntities, $rename = true, $exportFiles = true) {
		...
		if ($exportFiles) {
			$lessonDirectory = new MagesterDirectory($this -> directory);
			$file = $lessonDirectory -> compress($this -> lesson['id'].'_exported.zip', false); //Compress the lesson files
		} else {
			$dataFile = new MagesterFile($this -> directory.'/'."data.dat");
		...
		try {
			$existingFile = new MagesterFile($userTempDir.'/'.MagesterFile :: encode($this -> lesson['name']).'.zip'); //Delete any previous exported files
			$existingFile -> delete();
		} catch (Exception $e) {}
		if ($rename) {
			$newName = str_replace(array('"', '>', '<', '*', '?', ':'), array('&quot;', '&gt;', '&lt;', '&#42;', '&#63;', '&#58;'), $this -> lesson['name']);
			$file -> rename($userTempDir.'/'.MagesterFile :: encode($newName).'.zip', true);
		}
	...
	* $independent_lessons = _MagesterLesson :: getStandAloneLessons();
	...
	* $lesson = new MagesterLesson(4);           //Instantiate lesson with id 4
	...
	*/
	public function persist() {
      ...
      'show_catalog' => $this -> lesson['course_only'] ? 1 : $this -> lesson['show_catalog'], //if lesson is available only via course, it can not be hidden from catalog
		...
		MagesterSearch :: insertText($fields['name'], $this -> lesson['id'], "lessons", "title");
	}
	/**
	...
	* $lesson = new MagesterLesson(65);                          //Create new lesson object

	* $projectList = $lesson -> getProjects();                 //Get all projects for this lesson

	* $projectList = $lesson -> getProjects(true);             //Get all projects for this lesson as an MagesterProject objects list

	* $projectList = $lesson -> getProjects(true, 'jdoe');     //Get projects assigned to 'jdoe' for this lesson, as an MagesterProject objects list

	* </code>

	*

	* @param boolean $returnObjects Whether to return MagesterProject objects or just an array with projects properties
	...
	*/
	public function getProjects($returnObjects = false, $login = false, $nonExpired = false) {
		...
		$projects = array();
		foreach ($result as $value) {
			$returnObjects ? $projects[$value['id']] = new MagesterProject($value) : $projects[$value['id']] = $value;
		}
		return $projects;
	}
	/**
	...
	* @param array $lessonInformation The information to display (According to the MagesterLesson :: getInformation() format)
		...
		if ($topic_ID) {
			...
			foreach($related_events as $key => $event) {
				...
				if ($prev_event) {
					...
					if ($event['users_LOGIN'] == $prev_event['event']['users_LOGIN'] && $event['type'] == $prev_event['event']['type'] && $prev_event['event']['timestamp'] - $event['timestamp'] < MagesterEvent::SAME_USER_INTERVAL) {
	...
	* $instance = MagesterLesson :: createInstance(43);

	* </code>

	*

	* @param mixed $instanceSource Either a lesson id or an MagesterLesson object.

	* @return MagesterLesson The new lesson instance
	...
	*/
	public static function createInstance($instanceSource, $originateCourse) {
		...
		$instance = MagesterLesson :: createLesson($result[0]);
	...
	* @param mixed $lesson The lesson argument, can be an id or an MagesterLesson object
	...
	* Convert argument to an MagesterLesson object

	*

	* @param mixed $lesson The parameter to convert

	* @return MagesterLesson a new MagesterLesson object
