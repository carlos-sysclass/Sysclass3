...
if (!$smarty -> is_cached('index.tpl', $cacheId) || !$GLOBALS['configuration']['smarty_caching']) {
	...
	$languages = array();
	foreach (MagesterSystem :: getLanguages() as $key => $value) {
		if ($value['active']) {
...
$form -> addElement('password', 'password', _PASSWORD, 'class = "indent round_all" tabindex = "0"');
	...
	if ($form -> isSubmitted()) {
		if ($form -> validate()) {
          ...
          "user_types_ID" => $values['user_types_ID']);
			...
			try {
				$newUser = MagesterUser :: createUser($user_data);
				$encrypted = true; //needed for autologin
				MagesterEvent::triggerEvent(array("type" => MagesterEvent::SYSTEM_REGISTER, "users_LOGIN" => $user_data['login'], "users_name" => $user_data['name'], "users_surname" => $user_data['surname']));
	...
	if (!$smarty -> is_cached('index.tpl', $cacheId) || !$GLOBALS['configuration']['smarty_caching']) {
		...
		try {
			if (isset($_GET['lessons_ID'])) {
				...
				$content = new MagesterContentTree($lesson);
				...
				$smarty -> assign("T_LANGUAGES", MagesterSystem :: getLanguages(true));
				...
				if ($course -> course['instance_source']) {
					$parentCourse = new MagesterCourse($course -> course['instance_source']);
...
if (!$smarty -> is_cached('index.tpl', $cacheId) || !$GLOBALS['configuration']['smarty_caching']) {
	...
	$smarty -> assign("T_ONLINE_USERS_LIST", MagesterUser :: getUsersOnline($GLOBALS['configuration']['autologout_time'] * 60));
