...
function askChat()
{
 ...
 if (isset($_GET['get_users'])) {
  ...
  if ($_GET['chatrooms_ID'] == 0) {
   ...
   if (empty($other_room_users)) {
    ...
    $magester_general_users = array_diff($all_users['users_LOGIN'], $other_room_users['users_LOGIN']);
   }
   foreach ($magester_general_users as $user) {
    $data .= $user . "<br>";
   }
  } else {
   // @performance: 1DB
 ...
 // Special treatment for the genaral SysClass room
...
function askInformation()
{
 try {
  ...
  if (isset($_GET['common_lessons']) && isset($_GET['user1']) && isset($_GET['user2'])) {
   $user1 = MagesterUserFactory::factory($_GET['user1']);
