...
$smarty->assign("T_CHECK_XCHAT", $checkXchat[0]['active']);
...
if (isset($GLOBALS['currentTheme'] -> options['sidebar_interface']) && $GLOBALS['currentTheme'] -> options['sidebar_interface'] < 2) {
	...
	# initialized here, might change later
	$active_menu = 1;
	...
	if ($_SESSION['s_type'] == 'administrator') {
		...
		*/
		...
		if (!isset($currentUser -> coreAccess['languages']) || $currentUser -> coreAccess['languages'] != 'hidden') {
			$systemMenu[] = array('id' => 'languages_a', 'title' => _LANGUAGES, 'link' => "administrator.php?ctg=languages");
		}
		if (!isset($currentUser -> coreAccess['statistics']) || $currentUser -> coreAccess['statistics'] != 'hidden') {
			$systemMenu[] = array('id' => 'statistics_a', 'title' => _STATISTICS, 'link' => "administrator.php?ctg=statistics");
		}
		if (!isset($currentUser -> coreAccess['backup']) || $currentUser -> coreAccess['backup'] != 'hidden') {
			$systemMenu[] = array('id' => 'backup_a', 'title' => _BACKUP." - "._RESTORE, 'image' => "32x32/backup_restore.png", 'link' => "administrator.php?ctg=backup");
		}
		if (!isset($currentUser -> coreAccess['maintenance']) || $currentUser -> coreAccess['maintenance'] != 'hidden') {
			$systemMenu[] = array('id' => 'maintenance_a', 'title' => _MAINTENANCE, 'link' => "administrator.php?ctg=maintenance");
		}
		...
		if (!isset($currentUser -> coreAccess['lessons']) || $currentUser -> coreAccess['lessons'] != 'hidden') {
			$contentMenu[] = array('id' => 'directions_a', 'title' => _DIRECTIONS, 'link' => "administrator.php?ctg=directions");
		...
		*/
		...
		if ($GLOBALS['configuration']['disable_dashboard'] != 1 && (!isset($currentUser -> coreAccess['dashboard']) || $currentUser -> coreAccess['dashboard'] != 'hidden')) {
			$userMenu[] = array("id" => "personal_a", "image" => "user", "link" => "administrator.php?ctg=users&edit_user=".$_SESSION['s_login'], "title" => _PERSONALDATA);
		}
		if (!isset($currentUser -> coreAccess['users']) || $currentUser -> coreAccess['users'] != 'hidden') {
			$userMenu[] = array('id' => 'users_a', 'title' => _USERS, 'link' => "administrator.php?ctg=users");
		}
		if (!isset($currentUser -> coreAccess['user_types']) || $currentUser -> coreAccess['user_types'] != 'hidden') {
			$userMenu[] = array('id' => 'user_types_a', 'title' => _ROLES, 'link' => "administrator.php?ctg=user_types");
		}
		if (!isset($currentUser -> coreAccess['users']) || $currentUser -> coreAccess['users'] != 'hidden') {
		...
		*/
		...
		if ((!isset($currentUser -> coreAccess['personal_messages']) || $currentUser -> coreAccess['personal_messages'] != 'hidden') && $GLOBALS['configuration']['disable_messages'] != 1) {
			$communicationMenu[] = array('id' => 'messages_a', 'title' => _MESSAGES, 'link' => "administrator.php?ctg=messages");
		}
		if ((!isset($currentUser -> coreAccess['forum']) || $currentUser -> coreAccess['forum'] != 'hidden') && $GLOBALS['configuration']['disable_forum'] != 1) {
			$communicationMenu[] = array('id' => 'forum_a', 'title' => _FORUM, 'link' => "administrator.php?ctg=forum");
		}
		if ($GLOBALS['configuration']['chat_enabled']) {
