...
/api.php?token=<token>&action=login&username=<login>&password=<password> 		logs <login> in SysClass API (<login> must be an administrator account)

/api.php?token=<token>&action=magesterlogin&login=<login> 			logs <login> in SysClass
...
/api.php?token=<token>&action=groups                                                returns all groups defined in SysClass
...
/api.php?token=<token>&action=lessons												returns all lessons defined in SysClass

/api.php?token=<token>&action=courses												returns all courses defined in SysClass
...
/api.php?token=<token>&action=logout												logs out from SysClass API
			...
			case 'login':{
				...
				if (isset($_GET['token']) && checkToken($_GET['token'])){
					if (isset($_GET['login']) && isset($_GET['password']) && isset($_GET['email']) && isset($_GET['languages']) && isset($_GET['name']) && isset($_GET['surname'])){
						...
						$insert['password'] = MagesterUser :: createPassword($_GET['password']);
