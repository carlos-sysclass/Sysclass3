...
try {
    ...
    if ($_GET['mode'] == 'lesson') {
        ...
        $rootDir = new MagesterDirectory(G_EXTERNALPATH);
    ...
    if (isset($_GET['directory'])) {
        $directory = new MagesterDirectory($_GET['directory']);
        if (strpos($directory['path'], $rootDir['path']) === false) {
            $directory = $rootDir;
        } else {
            if (MagesterDirectory :: normalize($directory['directory']) == MagesterDirectory :: normalize($rootDir['path'])) {
 ...
 $smarty -> assign("T_OFFSET", $t_offset);
    ...
    foreach (new MagesterDirectoryOnlyFilterIterator(new MagesterNodeFilterIterator(new ArrayIterator($tree, RecursiveIteratorIterator :: SELF_FIRST))) as $key => $value) {
        $value['image'] = $value -> getTypeImage();
        $files[] = $value;
    }
    foreach (new MagesterFileOnlyFilterIterator(new MagesterFileTypeFilterIterator(new MagesterNodeFilterIterator(new ArrayIterator($tree, RecursiveIteratorIterator :: SELF_FIRST)), $filter, $mode)) as $key => $value) {
        $value['image'] = $value -> getTypeImage();
        $files[] = $value;
    }
