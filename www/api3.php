...
/api2.php?token=<token>&action=login&username=<login>&password=<password> 		logs <login> in SysClass API (<login> must be an administrator account)

/api2.php?token=<token>&action=magesterlogin&login=<login> 			logs <login> in SysClass
...
/api2.php?token=<token>&action=groups                                                		returns all groups defined in SysClass
...
/api2.php?token=<token>&action=lessons													returns all lessons defined in SysClass

/api2.php?token=<token>&action=courses													returns all courses defined in SysClass
...
/api2.php?token=<token>&action=logout													logs out from SysClass API
 ...
 * 
API returns xml corresponding to the action argument. For actions like SysClass login, activate_user etc it returns a status entity ("ok" or "error").
	...
	if (isset($_GET['module'])) {
		...
		switch($_GET['action']) {
			...
			*/
			...
			case 'create_user' : {
				if (isset($_GET['token']) && checkToken($_GET['token'])){
					if (isset($_GET['login']) && isset($_GET['password']) && isset($_GET['email']) && isset($_GET['languages']) && isset($_GET['name']) && isset($_GET['surname'])){
						...
						$insert['user_types_ID'] = isset($_GET['user_types_ID']) ? $_GET['user_types_ID'] : '0';
				...
				if (isset($_GET['token']) && checkToken($_GET['token'])) {
					...
					try {
						...
						if ($user->user['id'] == $userData['id']) {
							// MAKE autologin CAPABLE
							if ($user -> user['autologin'] == "" ) {
								$convert = $userData['login']."_".$user -> user['timestamp'];
							...
							echo "<xml>";
							echo "<status>ok</status>";
							echo "<hash>" . $user -> user['autologin'] . "</hash>";
							echo "</xml>";
						} else {
							echo "<xml>";
							echo "<status>error</status>";
							echo "<message>Invalid User ID</message>";
							echo "</xml>";
						}
					} catch (Exception $e) {
						echo "<xml>";
						echo "<status>error</status>";
						echo "<message>Erro desconhecido</message>";
						echo "</xml>";
					}
				} else {
					echo "<xml>";
			...
			case 'create_user_details':{
				if (isset($_GET['token']) && checkToken($_GET['token'])) {
					...
					try {
						...
						if (isset($values['user_type']) || isset($values['user_types_ID'])) {
							$user = MagesterUserFactory :: factory($values['login']);
							
							if (isset($values['user_type'])) {
								$user->user['user_type'] = $values['user_type'];
							}
							if (isset($values['user_types_ID'])) {
								$user->user['user_types_ID'] = $values['user_types_ID'];
							}
							$user->persist();
						} 
			...
			case 'notify' : {
				$values = $_GET;
				if (isset($values['token']) && checkToken($values['token'])) {
					...
					try {
						...
						$result = $modules['module_subscribe']->trigger($values['event'], array('user'	=> $user));
			...
			case 'polos' : {
				...
				if (isset($values['token']) && checkToken($values['token'])) {
					...
					try {
						...
						$result = $modules['module_polos'] -> getPolos();
						
						if ($result) {
							echo "<xml>";
							echo '<polos>';
							foreach($result as $polo) {
								echo '<polo>';
								foreach($polo as $key => $value) {
									echo sprintf('<%1$s>%2$s</%1$s>', $key,	$value);
								}
								echo '</polo>';
							}
							echo '</polos>';
							...
							echo "<xml>";
							echo "<status>error</status>";
							echo "<message>No Polos Found</message>";
							echo "</xml>";
						}
					} catch (Exception $e) {
						var_dump($e);
				...
				if (isset($_GET['token']) && checkToken($_GET['token'])) {
					...
					if (isset($_GET['login']) && isset($_GET['course'])) {
						...
						try {
							$course = new MagesterCourse($_GET['course']);
							$user = MagesterUserFactory :: factory($_GET['login']);
							$_GET['type'] != "professor" && $_GET['type'] != 'student' && $_GET['type'] != '6' && $_GET['type'] != '12' && $_GET['type'] != '13' && $_GET['type'] != '22' ? $_GET['type'] = '6' : null;
