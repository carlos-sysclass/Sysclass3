...
(function (jQuery) {
    ...
    function build(options) {
		...
		function build() {
			...
			if (opts.start != null) {
				startDate = Date.Parse(opts.start);
				alert(startEnd[0].compareTo(startDate));
			}
	...
	function handleMethod(method, value) {
		if (method == "setSlideWidth") {
			var div = jQuery("div.ganttview", this);
			div.each(function () {
				var vtWidth = jQuery("div.ganttview-vtheader", div).outerWidth();
				jQuery(div).width(vtWidth + value + 1);
				jQuery("div.ganttview-slide-container", this).width(value);
			});
		}
	}
