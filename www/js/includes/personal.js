...
function suggestNewLogin(name, surname, login) {
...
function usersCourseClassAjaxPost(login, course, courseclass, el, table_id) {
	// OPEN DIALOG TO SHOW 
	var url = location.toString();
	var parameters = {postAjaxRequest:'usersclasses', method: 'get', edit_course : course, courseclass : courseclass, login: login};
	
	if (login) {
		var userType = $('course_type_'+course).options[$('course_type_'+course).selectedIndex].value;
...
function checkLoginExixts(login) {
