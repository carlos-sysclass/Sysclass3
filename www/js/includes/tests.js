...
/* AJAX QUESTIONS RESULT */
var interval = null;
function SC_js_initTestAutoSaveTimer() {
	interval = window.setInterval(SC_js_doTestAutoSave, 1000 * 60); // ONE MINUTE SAVE
}
function SC_js_stopTestAutoSaveTimer() {
	window.clearInterval(interval);
}
function SC_js_doTestAutoSave() {
	var url = location.toString(); 
		
	var urlData = jQuery("#test_form").serialize() + "&ajax=1&question_answers=1";
...
jQuery("#test_form :input").change(function() {
	SC_js_doTestAutoSave();
});
