 ...
 * This class implements the RSS SysClass module

 * @version 0.1

 */
class module_rss extends MagesterExtendedModule
{
  ...
  return true;
   ...
   if ($_GET['lesson']) {
    ...
    private function getRssFeeds($refresh = false) {
     ...
     foreach ($feeds as $key => $feed) {
      $str = '';
      if ($feed['lessons_ID'] != -1 && $feed['lessons_ID'] && $_SESSION['s_lessons_ID'] && $feed['lessons_ID'] != $_SESSION['s_lessons_ID']) {
       unset ($feeds[$key]);
      } else {
       if (!$refresh && $str = Cache::getCache('rss_cache:'.$key)) {
        ...
        if ($feed['title'] != $feedTitle) {
         $feedTitle = '<li class="rss-title" style = "display:none;font-weight:bold" onmouseover = "pauseList()" onmouseout = "continueList()"> '._RSS_NEWSFROM.' &quot;<span style = "font-style:italic">'.$feed['title'].'</span>&quot;</li>';
         $str .= $feedTitle;
        }
        ...
        foreach ($response as $value) {
         ...
         $str .= sprintf(
         	'<li style = "" onmouseover = "pauseList()" onmouseout = "continueList()"> '.
         	'	%s &nbsp; %s'.
			'	<div class="blockFooter">'.
			'		<span class="to-left">'.
			'			<img '.
			'				src = "themes/default/images/others/transparent.gif"'.
			'				class="sprite16 sprite16-calendar3"'.
			'				border = "0"/>'.
			'			<span>%s</span>'.
			'		</span>'.
			'		<span class="to-right">'.
			'			<img border="0" class="sprite16 sprite16-n_pointer" src="themes/default/images/others/transparent.gif">'.
			'			<a href="%s" title="Saiba Mais", target="_NEW">'.
   ...
   $currentLesson = $this -> getCurrentLesson();
    ...
    public function getSidebarLinkInfo() {
	    ...
	    $link_of_menu_system = array (array ('id' => 'rss_link_id1',
                                              	'title' => "RSS",
                                              	'image' => $this -> moduleBaseDir.'images/rss16',
...
/*
    		...
    		case 'history':
				...
				foreach ($result as $value) {
					...
					$eventObject = new MagesterEvent($value);
