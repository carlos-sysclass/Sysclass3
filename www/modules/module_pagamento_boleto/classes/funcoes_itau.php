...
if (!function_exists('monta_linha_digitavel')) {
	...
	function monta_linha_digitavel($codigo)
	{
			...
			$dv3      = itau_modulo10($resag.$contadac.$zeros);
			// campo 4
...
$conta = formata_numero($dadosboleto["conta_corrente"]['conta'],5,0);
$conta_dv = formata_numero($dadosboleto["conta_corrente"]['conta_dv'],1,0);

//carteira 175
$carteira = $dadosboleto["carteira"];
//nosso_numero no maximo 8 digitos
$nnum = formata_numero($dadosboleto["nosso_numero"],8,0);
$codigo_barras = $codigobanco.$nummoeda.$fator_vencimento.$valor.$carteira.$nnum.itau_modulo10($agencia.$conta.$carteira.$nnum).$agencia.$conta.$conta_dv.'000';
...
echo $codigobanco. '<br />' .
	...
	$carteira. '<br />' .
	$nnum. '<br />' .
	modulo_10($agencia.$conta.$carteira.$nnum). '<br />' .
	$agencia. '<br />' .
	$conta. '<br />' .
	modulo_10($agencia.$conta) . '<br />' .
	'000' . '<br />';
*/
...
/*
echo '<br />';
echo $agencia . ' ' . $conta . ' ' . $carteira . ' ' . $nnum;
echo '<br />';
*/
/*
echo modulo_10("341917500");
echo '<br />';
echo itau_modulo10("341917500");
echo '<br />';
*/
$nossonumero = $carteira.'/'.$nnum.'-'.itau_modulo10($agencia.$conta.$carteira.$nnum);
