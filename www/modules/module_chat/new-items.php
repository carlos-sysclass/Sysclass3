...
if ($_SESSION['s_lessons_ID']) {
	...
	if (!isset($_SESSION["lessonid"]) || $_SESSION["lessonid"] != $_SESSION['s_lessons_ID']) {
		...
		$lsn = sC_getTableData("lessons", "name", "id='".$_SESSION['s_lessons_ID']."'");
...
function getConnectedUsers()
{
	...
	$result = sC_getTableData("user_times,users,module_chat_users", "users_LOGIN, users.name, users.surname, users.user_type, timestamp_now, session_timestamp", "users.login=user_times.users_LOGIN and users.login=module_chat_users.username and session_expired=0", "timestamp_now desc");
