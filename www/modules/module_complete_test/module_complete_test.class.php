 ...
 * Complete test class
 *
 * This class implements the Complete test SysClass module
 * @version 0.1
 */
class module_complete_test extends MagesterModule
{
	...
	public function getSmartyTpl() {
		...
		if ($currentLesson = $this -> getCurrentLesson()) {
			...
			if (isset($_GET['test']) && isset($_GET['login'])) {
				$currentUnit = new MagesterUnit($_GET['test']);
	...
	private function handleImportCsvForm(&$form) {
		...
		if ($form -> isSubmitted() && $form -> validate()) {
			...
			$selectedTest = new MagesterTest($values['select_test'], true);
	...
	private function buildCorrelateDataForm($selectedTest, $uploadedFile) {
		if (isset($selectedTest)) {
			...
			$selectedTest = new MagesterTest($_GET['test_id']);
		}
	...
	private function handleCorrelateDataForm(&$form) {
		...
		if ($form  -> isSubmitted() && $form -> validate()) {
			...
			$selectedTest = new MagesterTest($formValues['test_id']);
			$uploadedFile = new MagesterFile($formValues['uploaded_file']);
			if (($handle = fopen($uploadedFile['path'], "r")) !== FALSE) {
				while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
					$parsedContents[] = $data;
				}
