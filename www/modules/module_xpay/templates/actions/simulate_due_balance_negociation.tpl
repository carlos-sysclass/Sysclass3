{capture name="t_xpay_simulate_statement"}
	...
	<div class="xpay-invoice-params-selection form-container" id="xpay-invoice-params-selection" title="{$smarty.const.__XPAY_CREATE_NEW_NEGOCIATION}">
		<form {$T_XPAY_INVOICE_PARAMS_FORM.attributes}>
			...
			<!-- 		
			<div>
				<button class="form-button icon-save" type="{$T_XPAY_INVOICE_PARAMS_FORM.submit_invoice_params.type}" value="{$T_XPAY_INVOICE_PARAMS_FORM.submit_invoice_params.value}">
					<img width="29" height="29" src="images/transp.png">
					<span>{$T_XPAY_INVOICE_PARAMS_FORM.submit_invoice_params.label}</span>
				</button>	
			</div>
			 -->
		 </form>
...
<!-- 				<th style="text-align: center;">Parcela</th>  -->
		...
		<tbody>
			{foreach item="invoice" from=$T_XPAY_STATEMENT.sugested_invoices}
