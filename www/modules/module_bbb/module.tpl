                ...
                <form {$T_BBB_FORM.attributes}>
                    ...
                    <table class = "formElements">
                        <tr><td class = "labelCell">{$smarty.const._BBB_BBBSERVERNAME}:&nbsp;</td>
                            ...
                            <td class = "elementCell" align="left" width="100%">&nbsp;<a href="javascript:void(0)" onClick="document.getElementById('server_input').value = ''" >
                               <img src="images/16x16/go_into.png" title="{$smarty.const._BBB_RESETDEFAULTSERVER}" alt="{$smarty.const._BBB_RESETDEFAULTSERVER}" border =0 style="vertical-align:middle"/></a>	
                            </td>
...
<!--ajax:BBBUsersTable-->
                    <table style = "width:100%" class = "sortedTable" size = "{$T_USERS_SIZE}" sortBy = "0" id = "BBBUsersTable" useAjax = "1" rowsPerPage = "20"  url = "{$T_BBB_MODULE_BASEURL}&edit_BBB={$smarty.get.edit_BBB}&">
                        <tr class = "topTitle">
                            ...
                            <td class = "topTitle" name="user_type_unique">{$smarty.const._USERTYPE}</td>
                            <td class = "topTitle" name="surname">{$smarty.const._SURNAME}</td>
                            <td class = "topTitle" name="email">{$smarty.const._EMAIL}</td>
                            <td class = "topTitle noSort" name="check" align="center">{$smarty.const._CHECK}</td>
                        </tr>
    ...
    {else}
        {capture name = 't_BBB_list_code'}
            ...
            <table class="sortedTable" id = "module_BBB_sortedTable" border = "0" width = "100%" sortBy = "0">
                ...
                {assign var="cur_lesson" value="-1"}

                {foreach name =BBB item=meeting from = $T_BBB}
                	{if $cur_lesson !=  $meeting->lesson_id}
                		{* OPEN GROUP *}
                		<tr class = "tr_group oddRowColor">
                			<td colspan="5">{$smarty.const._LESSON} {$meeting.lesson_name}</td>
                		</tr>		
                		{assign var="cur_lesson" value=$meeting->lesson_id}
                	{/if}
	                    ...
	                    <td align = "center">
	                        {if $T_BBB_CURRENTLESSONTYPE == "professor"}
	                         ...
	                         {else}
	                            ...
	                            {elseif $meeting.status == "1" && $meeting.inServer}
