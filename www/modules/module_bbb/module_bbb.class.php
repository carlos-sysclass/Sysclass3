...
//				'welcome='.urlencode(_WELCOMETO.' %%CONFNAME%%'),
//				'duration=',
				'record=true',
    ...
    public function getModule() {
        ...
        if ($currentUser -> getType() == "administrator") {
            ...
            if ($form -> isSubmitted() && $form -> validate()) {
                ...
                MagesterConfiguration::setValue("module_BBB_server", $server_name);
                MagesterConfiguration::setValue("module_BBB_salt", $salt_string);
                MagesterConfiguration::setValue("module_BBB_server_version", $version_choice);
...
//pr($_GET);
        ...
        if (isset($_GET['delete_BBB']) && eF_checkParameter($_GET['delete_BBB'], 'id') && $userRole == "professor") {
            ...
            if (isset($_GET['edit_BBB'])) {
                ...
                if (isset($_GET['ajax']) && $_GET['ajax'] == 'BBBUsersTable') {
                    ...
                    $users = eF_getTableData("users JOIN users_to_lessons ON users.login = users_to_lessons.users_LOGIN
                                                    ...
                                                    CASE WHEN users.user_types_ID IS NULL OR users.user_types_ID = 0
                                                    THEN users.user_type
                                                    ELSE users.user_types_ID
                                                    END AS user_type_unique, users.user_type, users.user_types_ID
                                                    ",
                    ...
                    $users = eF_getTableData("users JOIN users_to_lessons ON users.login = users_to_lessons.users_LOGIN
                                                    JOIN module_BBB ON module_BBB.lessons_ID = users_to_lessons.lessons_ID
                                                    LEFT OUTER JOIN module_BBB_users_to_meeting ON module_BBB.id = module_BBB_users_to_meeting.meeting_ID AND users.login = module_BBB_users_to_meeting.users_LOGIN",
                                                    "users.login, users.name, users.surname, meeting_ID, users.user_type, users.user_types_ID,
                                                    CASE WHEN users.user_types_ID IS NULL OR users.user_types_ID = 0
                                                    THEN users.user_type
                                                    ELSE users.user_types_ID
                                                    END AS user_type_unique
                                                    ",
                                                    "users_to_lessons.lessons_ID = '".$_SESSION['s_lessons_ID']."' AND users.login <> '".$currentUser -> user['login'] . "' AND module_BBB.id = '".$_GET['edit_BBB']."' AND users.active = 1 AND users_to_lessons.active = 1");
            ...
            foreach ($BBB as $key => $meeting) {
            	$ONE_HOUR = (60 * 60);
            	$ONE_HOUR = 0;
                if ($meeting['timestamp'] -  $ONE_HOUR < $now) {
                    $BBB[$key]['mayStart'] = 1;
