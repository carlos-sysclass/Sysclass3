...
class module_banners extends MagesterModule
{
    ...
    public function getModule()
    {
        ...
        if (isset($_GET['delete_banner']) && sC_checkParameter($_GET['delete_banner'], 'id')) {
            ...
            try {
                ...
                $existingImages["--"] = "--";
                foreach (new MagesterFileTypeFilterIterator(new MagesterFileOnlyFilterIterator(new MagesterNodeFilterIterator(new RecursiveIteratorIterator($bannerFileSystemTree -> tree, RecursiveIteratorIterator :: SELF_FIRST))), array('png', 'jpg', 'gif', 'jpeg', 'bmp')) as $key => $value) {
                ...
                try {
                    if ($_FILES['file_upload']['size'] > 0) {
                        ...
                        if ($selectedImage != "--") {
                            ...
                            $lfile = new MagesterFile($selectedImage['path']);
            ...
            for ($i = 0; $i < sizeof($banners['image_id']); $i++) {
                $imageFile = new MagesterFile($banners['image_id'][$i]);
