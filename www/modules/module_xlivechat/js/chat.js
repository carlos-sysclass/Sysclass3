		...
		jQuery.each(data.items, function(i,item){
			if (item)	{ // fix strange ie bug
				...
				if (item.s == 2) {
					...
					jQuery(".xlivechatboxmessage").append('<div class="chatboxmessage"><span class="chatboxinfo">'+item.m+'</span></div>');
				} else {
					...
					jQuery(".xlivechatboxmessage").append('<div class="chatboxmessage"><span class="chatboxmessagefrom">'+item.f+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">'+item.m+'</span></div>');
				}

				jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").scrollTop(jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").first().scrollHeight);
				itemsfound += 1;
				
				jQuery(".xlivechatboxmessage").scrollTop(jQuery(".xlivechatboxmessage").first().scrollHeight);
				itemsfound += 1;
	...
	jQuery.ajax({
	  ...
	  success: function(data) {
		...
		for (i=0;i<chatBoxes.length;i++) {
			...
			jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").scrollTop(
				jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").first().scrollHeight
			);
