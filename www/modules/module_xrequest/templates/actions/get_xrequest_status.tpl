...
{foreach name = 'ranges_loop' key = "id" item = "range" from = $T_REQUEST_STATUS}
	<tr id="row_{$range.id}" class="{cycle values = "oddRowColor, evenRowColor"}">
		...
		<td class="centerAlign">
			<a href="{$T_GRADEBOOK_BASEURL}&edit_range={$range.id}&popup=1" target="POPUP_FRAME" onclick="sC_js_showDivPopup('{$smarty.const._GRADEBOOK_EDIT_RANGE}', 0)"><img src="{$T_GRADEBOOK_BASELINK}images/edit.png" alt="{$smarty.const._EDIT}" title="{$smarty.const._EDIT}" border="0"></a>
