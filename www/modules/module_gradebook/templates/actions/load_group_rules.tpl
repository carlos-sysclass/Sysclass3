<table class="style1">
	...
	<tbody>
		{foreach name = 'columns_loop' key = "id" item = "column" from = $T_GRADEBOOK_LESSON_COLUMNS}
			<tr class="gradebook-column-row" id="gradebook-column-row-{$column.id}">
				<th>{$column.name}</th>
				...
				<th>
					...
					{/if}
					 -->
					<a href="javascript: _sysclass('load', 'gradebook').deleteColumn({$column.id});" onclick="return confirm('{$smarty.const._IRREVERSIBLEACTIONAREYOUSURE}');">
