...
<table class="style1">
	...
	<tbody>
		{foreach key=id item=user from=$T_GRADEBOOK_LESSON_USERS}
			...
			<tr id="row_{$user.uid}" class="{cycle values = "oddRowColor, evenRowColor"} {if !$user.active}ui-state-disabled{/if}">
				...
				{foreach key=id item=column from=$T_GRADEBOOK_LESSON_COLUMNS}
...
{sC_template_printBlock title=$smarty.const._GRADEBOOK_NAME data=$smarty.capture.t_gradebook_professor_code image=$T_GRADEBOOK_BASELINK|cat:'images/gradebook_logo.png' absoluteImagePath = 1}
