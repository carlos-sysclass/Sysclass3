	...
	<tr id="row_{$user.uid}" class="{cycle values = "oddRowColor, evenRowColor"} {if !$user.active}ui-state-disabled{/if}">
		...
		{foreach name = 'columns_loop' key = "id" item = "column" from = $T_GRADEBOOK_LESSON_COLUMNS}
			...
			<td class="rightAlign">
				...
				<input type="text"  value="{$user.grades[$oid].grade}" size="5" maxlength="5" />
				<img class="ajaxHandle" src="{$T_GRADEBOOK_BASELINK|cat:'images/success.png'}" title="{$smarty.const._SAVE}" alt="{$smarty.const._SAVE}" onclick="_sysclass('load', 'gradebook').setGrade('{$column.id}', '{$user.users_LOGIN}', this);"/>
			</td>
		{/foreach}
<!-- 		
