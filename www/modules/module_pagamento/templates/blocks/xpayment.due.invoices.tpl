{if $T_PAYMENT.invoices|@count > 0}
	<table id="_PAGAMENTO_DASHBOARD_INVOICES" class="style1">
		<thead>
			<tr>
					...
					<tr class="{cycle values="odd,even"} {$invoice.status|lower} {if $invoice.bloqueio == 1}bloqueio{/if} {if $invoice.pago == 1}pago{/if}" metadata="{Mag_Json_Encode data=$invoice}">
						...
						<td align="center">
							<div class="button_display">
								<a target="_blank" href="{$T_PAGAMENTO_BASEURL}&action=get_invoice&payment_id={$invoice.payment_id}&invoice_id={$invoice.parcela_index}" class="invoicePrintLink" style="{if $invoice.pago == 0}visibility: visible;{else}visibility: hidden;{/if}">
									<img width="16" height="16" alt="Editar" src="images/16x16/printer.png"/>
									<span>Imprimir</span>
