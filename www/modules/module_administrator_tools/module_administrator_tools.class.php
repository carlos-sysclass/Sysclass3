...
class module_administrator_tools extends MagesterExtendedModule {
  ...
  */  
  ...
  return true;
     ...
     * Function to be executed when the module is removed from an SysClass system
  ...
  //eF_executeNew("DROP TABLE ;");
     ...
     * - Use the $this -> setMessageVar($message, $message_type) function to export information to SysClass users with header messages
   ...
   if ($form -> isSubmitted() && $form -> validate()) {
    try {
     ...
     $user = MagesterUserFactory::factory($values['users_LOGIN']);
     try {
      $existingUser = true;
      $newUser = MagesterUserFactory::factory($values['new_login']);
     } catch (Exception $e) {
      $existingUser = false;
     }
     if ($existingUser) {
