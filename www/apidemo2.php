  ...
  case 'course_lessons':{
    ...
    if ($form -> isSubmitted()) {
     ...
     /*
					 ...
					 * echo "<script>var mine = window.open('api2.php?action=magesterlogin&token=".$token."&login=".$login."', 'api', 'width=1,height=1,left=0,top=0,scrollbars=no');</script>";
					 ...
					 */
                    if ($stream = fopen(G_SERVERNAME.'api2.php?action=magesterlogin&token='.$token.'&login='.$login, 'r')) {
                        $output = stream_get_contents($stream);
                        fclose($stream);
                    }
                    //echo "<script>var mine = window.open('api2.php?action=magesterlogin&token=".$token."&login=".$login."', 'api', 'width=1,height=1,left=0,top=0,scrollbars=no');</script>";
    ...
    case 'magesterlogout':{
                    $token = $values['token'];
     $login = $values['login'];
                    if ($stream = fopen(G_SERVERNAME.'api2.php?action=magesterlogout&token='.$token.'&login='.$login, 'r')) {
                        $output = stream_get_contents($stream);
                        fclose($stream);
                    }
                    break;
