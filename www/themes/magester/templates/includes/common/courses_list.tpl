...
{if in_array('price_presencial', $T_DATASOURCE_COLUMNS) && $T_CONFIGURATION.disable_payments != 1} <td class = "topTitle price_presencial" name = "price_presencial">{$smarty.const._PRICE_PRESENCIAL}</td>{/if}
{if in_array('price_web', $T_DATASOURCE_COLUMNS) && $T_CONFIGURATION.disable_payments != 1} <td class = "topTitle price_web" name = "price_web">{$smarty.const._PRICE_WEB}</td>{/if}
{if in_array('price', $T_DATASOURCE_COLUMNS) && $T_CONFIGURATION.disable_payments != 1} <td class = "topTitle price" name = "price">{$smarty.const._PRICE}</td>{/if}
...
{/if}
{if in_array('price_presencial', $T_DATASOURCE_COLUMNS) && $T_CONFIGURATION.disable_payments != 1}
   <td class = "price_presencial">
    {if $course.price_presencial == 0}-{else}{$course.price_presencial_string}{/if}
   </td>
{/if}
{if in_array('price_web', $T_DATASOURCE_COLUMNS) && $T_CONFIGURATION.disable_payments != 1}
   <td class = "price_web">
    {if $course.price_web == 0}-{else}{$course.price_web_string}{/if}
   </td>
{/if}
...
{if in_array('has_course', $T_DATASOURCE_COLUMNS)}
       ...
       {if $_change_handles_}
        ...
        {elseif $T_SORTED_TABLE == 'instancesTable' || !$course.has_instances}
			<input class = "inputCheckBox" type="checkbox" id="course_{$course.id}" name="{$course.id}" {if $course.has_course == 1}checked{/if} onclick ="ajaxPost('{$course.id}', this, 'coursesTable');" style="display: none;">
