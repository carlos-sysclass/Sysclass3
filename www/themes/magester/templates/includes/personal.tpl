...
{if (isset($smarty.get.add_user))}
	...
	$j(document).ready(function($j) {
		$j(":input[name='new_login']").val(
			suggestNewLogin($j(":input[name='name']").val(), $j(":input[name='surname']").val())
		);
	
		
		$j(":input[name='name'], :input[name='surname']").keyup(function() {
			$j(":input[name='new_login']").val(
				suggestNewLogin($j(":input[name='name']").val(), $j(":input[name='surname']").val())
			);
		});
...
{capture name = 't_users_to_classes_dialog_code'}
	{if !$T_SORTED_TABLE || $T_SORTED_TABLE == 'classesuserTable'}
<!--ajax:classesuserTable-->
		<table style = "width:100%" class = "sortedTable" size = "{$T_TABLE_SIZE}" sortBy = "4" order = "desc" useAjax = "1" id = "classesuserTable" rowsPerPage = "{$smarty.const.G_DEFAULT_TABLE_SIZE}" url = "{$smarty.server.PHP_SELF}?ctg=users&">
			<tr class = "topTitle defaultRowHeight">
		    	...
		    	<td class = "topTitle centerAlign" name = "has_class" >{$smarty.const._SELECT}</td>
			</tr>
			{foreach name = 'classes_list3' key = 'key' item = 'classe' from = $T_DATA_SOURCE}
				<tr class = "defaultRowHeight {cycle values = "oddRowColor, evenRowColor"} {if !$classe.active}deactivatedTableElement{/if}">
					<td>{$classe.name}</td>
					...
					<td class = "centerAlign">
						<input type = "checkbox" id = "{$classe.id}" onclick = "usersCourseClassAjaxPost('{$smarty.get.edit_user}', '{$smarty.get.edit_course}', '{$classe.id}', this);" {if $classe.user_has_class != "0"}checked{/if}>{if $classe.user_has_class != "0"}<span style = "display:none">checked</span>{/if}
					</td>
				</tr>
			{foreachelse}
...
<div id="user_class_select_dialog" style="display: none;">
	{sC_template_printBlock title = $smarty.const._COURSESCLASSES data = $smarty.capture.t_users_to_classes_dialog_code image = '32x32/courses.png'}
</div>
  ...
  <!-- <fieldset class = "fieldsetSeparator">
   ...
   {* enterprise edition: Insert a second column - new table *}
	...
	{if (isset($smarty.get.add_user))}
     ...
     <tr><td class = "labelCell">{$smarty.const._PASSWORD}:&nbsp;</td>
      <td style="white-space:nowrap;"><strong>{$T_NEW_PASSWORD}</strong></td></tr>
<!--      
     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.password_.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.password_.html}</td></tr>
     <tr><td></td><td class = "infoCell">{$smarty.const._PASSWORDMUSTBE6CHARACTERS|replace:"%x":$T_CONFIGURATION.password_length}</td></tr>
 ...
 -->
  ...
  {/if}
 {/capture}
 {capture name = 't_personal_details_code'}
   
   <table class = "formElements" width="90%">
     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.data_nascimento.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.data_nascimento.html}</td></tr>
      
     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.rg.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.rg.html}</td></tr>
      
     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.cpf.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.cpf.html}</td></tr>

     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.cep.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.cep.html}</td></tr>
      
     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.endereco.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.endereco.html}</td></tr>

     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.numero.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.numero.html}</td></tr>

     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.complemento.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.complemento.html}</td></tr>

     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.bairro.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.bairro.html}</td></tr>

     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.cidade.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.cidade.html}</td></tr>

     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.estado.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.estado.html}</td></tr>

     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.telefone.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.telefone.html}</td></tr>

     <tr><td class = "labelCell">{$T_PERSONAL_DATA_FORM.celular.label}:&nbsp;</td>
      <td style="white-space:nowrap;">{$T_PERSONAL_DATA_FORM.celular.html}</td></tr>
...
{capture name = 't_user_code'}
 ...
 {elseif $T_PERSONAL_CTG}
  ...
  {elseif $T_OP == "account"}
   <div class="tabber">
    ...
    <!-- 
    <div class="tabbertab{if ($smarty.get.tab == "details")} tabbertabdefault {/if}" title = "{$T_TITLES.account.details}">
     {sC_template_printBlock title = $T_TITLES.account.details data = $smarty.capture.t_personal_details_code image = '32x32/user.png'  }
    </div>
     -->
 ...
 {else}
  ...
  <!-- <div id="user_class_select_dialog" style="display: none;">
  	{*sC_template_printBlock title = $T_TITLES.status.courses data = $smarty.capture.t_users_to_classes_dialog_code image = '32x32/courses.png'*}
  </div>
   -->
  {/if}
 {/if}
...
{*** The user page appearance ***}
 {if isset($smarty.get.add_user)}
 	{capture name = "t_add_user_form_block"}

		{$T_PERSONAL_DATA_FORM.javascript}
		<form {$T_PERSONAL_DATA_FORM.attributes}>
		{$T_PERSONAL_DATA_FORM.hidden}
		 	<div class="tabber">
		  		{sC_template_printBlock tabber = "main" title = $smarty.const._PERSONALDATA data = $smarty.capture.t_user_code image = '32x32/user.png'  }
		  	</div>
		</form>
  	{/capture}
  	{sC_template_printBlock title = $smarty.const._NEWUSER data = $smarty.capture.t_add_user_form_block image='32x32/tools.png' }
 {*elseif $T_PERSONAL_CTG*}
 <!-- 
   ...
   {*/if*}
	...
	{*capture name = "t_edit_user_form_block"*}
		{*$T_PERSONAL_DATA_FORM.javascript*}
		<form {*$T_PERSONAL_DATA_FORM.attributes*}>
		{*$T_PERSONAL_DATA_FORM.hidden*}
			{*sC_template_printBlock title = $smarty.const._PERSONALDATA data = $smarty.capture.t_user_code image = '32x32/profile.png'*}
		</form>
   {*/capture*}
  	dsfdsfds
	{*sC_template_printBlock title = $smarty.const._EDITUSER data = $smarty.capture.t_edit_user_form_block image='32x32/tools.png'*}
   -->
 {else}
  ...
  {elseif $smarty.get.print == 1}
   {*sC_template_printBlock alt= $T_USERNAME title = $T_EMPLOYEE_FORM_CAPTION titleStyle = 'font-size:16px;font-weight:bold;' data = $smarty.capture.t_personal_form_data_code image = $T_SYSTEMLOGO options=$T_EMPLOYEE_FORM_OPTIONS*}
  {elseif $T_OP == "status"}
  	{sC_template_printBlock tabber = "main" title = $smarty.const._PERSONALDATA data = $smarty.capture.t_user_code image = '32x32/profile.png' main_options = $T_TABLE_OPTIONS}
  {else}
	{capture name = "t_edit_user_form_block"}
		...
		{*$T_PERSONAL_DATA_FORM.hidden*}
			<div class="tabber">
				...
				{*sC_template_printBlock tabber = "details" title = $smarty.const._PERSONALDETAILS data = $smarty.capture.t_personal_details_code image = '32x32/user.png'  *}
			</div>
		<!--  /form-->
   {/capture}
   
   {sC_template_printBlock title = $smarty.const._EDITUSER data = $smarty.capture.t_edit_user_form_block image='32x32/tools.png' main_options = $T_TABLE_OPTIONS }
  	
   {* vsC_template_printBlock title = "`$smarty.const._USEROPTIONSFOR`<span class = 'innerTableName'>&nbsp;&quot;#filter:login-`$T_EDITEDUSER->user.login`#&quot;</span>" data = $smarty.capture.t_edit_user_form_block image = '32x32/profile.png' main_options = $T_TABLE_OPTIONS options = $T_STATISTICS_LINK *}
