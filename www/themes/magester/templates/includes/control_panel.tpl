...
<!--/ajax:lessonTimelineTable-->
 ...
 {*moduleDigitalLibrary: Print the digital library section and file list*}
    ...
    {if $T_CURRENT_USER->coreAccess.control_panel != 'hidden' && (!$_student_ || ($T_CURRENT_LESSON && $T_CURRENT_LESSON->options.show_student_cpanel))}
    	{eF_assign_optionsGroups var="moduleIconFunctions" title=$smarty.const._OPTIONS columns=4 links=$T_CONTROL_PANEL_OPTIONS image='32x32/options.png' expandOrder = $T_POSITIONS_FIRST expandOrder2 = $T_POSITIONS_SECOND groups = $T_CONTROL_PANEL_GROUPS}
     ...
     <!-- <tr><td class = "moduleCell">
         ...
         {* {eF_template_printOptions title=$smarty.const._OPTIONS columns=4 links=$T_CONTROL_PANEL_OPTIONS image='32x32/options.png' expand = $T_POSITIONS_VISIBILITY.moduleIconFunctions groups = $T_CONTROL_PANEL_GROUPS} *}
        </td></tr>
         -->
        {* {/capture} *}
 {/if}
