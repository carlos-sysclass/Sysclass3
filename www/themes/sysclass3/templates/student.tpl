...
{if $smarty.session.s_lessons_ID}
    ...
    {if $T_NO_HORIZONTAL_MENU}
    	{assign var = "title_onclick" value = "top.sideframe.hideAllLessonSpecific();"}
    {/if}
...
{capture name = "center_code"}
 {if $smarty.get.message}
 	{sC_template_printMessageBlock content=$smarty.get.message type=$smarty.get.message_type class="homeStudentMessage"}
 {/if}
 {if $T_MESSAGE}
 	{sC_template_printMessageBlock content=$T_MESSAGE type=$T_MESSAGE_TYPE class="full homeStudentMessage"}
 {/if}
 {if $T_SEARCH_MESSAGE || $smarty.get.search_message}
     ...
     {/if}
  	{sC_template_printMessageBlock content=$T_SEARCH_MESSAGE type=$T_MESSAGE_TYPE class="homeStudentMessage"}
 {/if}
 <div>
