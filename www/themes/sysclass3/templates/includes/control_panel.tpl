				...
				{capture name='t_news_code'}
					<table class = "">
						{foreach name = 'news_list' item = "item" key = "key" from = $T_NEWS}
							<tr><td style="text-align:left;">{$smarty.foreach.news_list.iteration}. <a title = "{$item.title}" href = "{$smarty.server.PHP_SELF}?ctg=news&view={$item.id}&popup=1" target = "POPUP_FRAME" onclick = "sC_js_showDivPopup('{$smarty.const._ANNOUNCEMENT}', 1);">{$item.title}</a></td>
...
{if $_admin_}
	{capture name = "moduleIconFunctions"}
		{foreach from = $T_CONTROL_PANEL_GROUPS_ORDER item = "grupo" key = "i"}
			{if $grupo.itens|@count > 0}
				<section id="sectionOption-{$i}" class="sectionOption">
					<h3><a href="#" title="{$grupo.title}">{$grupo.title}</a></h3>
					<nav>
						<ul>
							{foreach from = $grupo.itens item = "option"}
								<li>
									<a href="{$option.href}" class="optionLinkImage optionLink-{$option.class}" title="{$option.text}"><span></span></a>
									<a href="{$option.href}" class="optionLinkText" title="{$option.text}">{$option.text}</a>
								</li>
							{/foreach}
						</ul>
					</nav>
