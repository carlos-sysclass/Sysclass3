...
{elseif $T_OP == 'course_classes'}
  {capture name = 't_course_classe_code'}
	<div class = "headerTools">
		<span>
			<img src = "images/16x16/autocomplete.png" title = "{$smarty.const._AUTOCOMPLETE}" alt = "{$smarty.const._AUTOCOMPLETE}"/>
			<a href = "{$smarty.session.s_type}.php?ctg=module&op=module_xclasse&action=new_course_class&xcourse_id={$smarty.get.course}">{$smarty.const.__XCLASSE_NEWCLASS}</a>
		</span>
	</div>
	<div class="clear"></div>
  	...
  	<table class="style1">
  		<thead>
  			<tr>
				...
				<th style="text-align: center;">{$smarty.const._CLASSSTARTON}</th>
				<th style="text-align: center;">{$smarty.const._CLASSFINISHON}</th>
				<th style="text-align: center;">{$smarty.const._MAXSTUDENTS}</th>
				<th style="text-align: center;">{$smarty.const._STUDENTSCOUNT}</th>
				<th style="text-align: center;">{$smarty.const._FUNCTIONS}</th>
  			</tr>
  		</thead>
  		<tbody>
  			{foreach item="classe" from=$T_XCOURSE_CLASSES_LIST}
	  			<tr>
  					<td>
						<a href = "{$smarty.session.s_type}.php?ctg=module&op=module_xclasse&action=edit_course_class&xcourse_id={$smarty.get.course}&xclasse_id={$classe.id}">
							{$classe.name}
						</a>
					</td>
					...
					<td class = "center">
						<div class="button_display">
							<button class="table-icon courseClasseEdit" onclick="window.location.href = '{$smarty.session.s_type}.php?ctg=module&op=module_xclasse&action=edit_course_class&xcourse_id={$smarty.get.course}&xclasse_id={$classe.id}'">
								<img class="sprite16 sprite16-edit" src="images/others/transparent.gif" />
							</button>
							<button class="table-icon courseClasseCalendar">
								<img class="sprite16 sprite16-calendar" src="images/others/transparent.gif" />
							</button>
							{if $classe.count_users == 0}
								<button class="table-icon courseClasseDelete"  onclick="window.location.href = '{$smarty.session.s_type}.php?ctg=module&op=module_xclasse&action=delete_course_class&xcourse_id={$smarty.get.course}&xclasse_id={$classe.id}'">
  ...
  {/capture}
  {eF_template_printBlock title = $smarty.const.__COURSE_CLASSE data = $smarty.capture.t_course_classe_code image = '32x32/import.png' main_options = $T_TABLE_OPTIONS options = $T_COURSE_OPTIONS }
