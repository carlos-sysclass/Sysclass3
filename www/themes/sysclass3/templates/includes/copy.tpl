     ...
     {capture name = 't_copy_content_code'}
         <table>
             <tr><td class = "labelCell">{$smarty.const._SELECTLESSONTOCOPYFROM}:&nbsp;</td>
                 ...
                 <select name = 'user_lessons' onchange = "document.location='{$smarty.server.PHP_SELF}?ctg=copy&from='+this.options[this.selectedIndex].value" style="font-weight: normal;">
                     <option value = "0">{$smarty.const._SELECTLESSON}</option>
                 {foreach name = 'directions_list' key = key1 item = course from = $T_USER_LESSONS}
                 	<optgroup label="{$course.name}"></optgroup>
                     {foreach name = "lessons_list" key = key2 item = lesson from = $course.lessons}
                     <option value = "{$lesson.id}" {if $lesson.id == $smarty.get.from}selected{/if}>{$lesson.name}</option>
                     {/foreach}
                 {/foreach}
                 </select>
             </td></tr>
