...
{if $smarty.get.type == 'forum' && ($smarty.get.add || $smarty.get.edit)}
 {capture name = 't_add_forum_code'}
     ...
     {if !$smarty.get.forum_id}
             ...
             <tr><td class = "labelCell">{$smarty.const._COURSECLASS}:&nbsp;</td>
                 <td class = "elementCell">{$T_ENTITY_FORM.classe_ID.html}</td></tr>    
   ...
   {capture name = 't_poll_code'}
       {if $T_ACTION == 'view' || !$T_POLL.isopen}
           ...
           <table width="100%">
               ...
               <tr><td>&nbsp;</td></tr>
               {if $T_CURRENT_POLL_USER_TYPE == 'professor' || $T_CURRENT_POLL_USER_TYPE == 'administrator'}
               <tr><td colspan="100%">
		           ...
		           {section name = 'votes_list' loop = $T_POLL_VOTES}
		           	<li style="border-bottom: 2px solid">
		           		...
		           		<span class="poll_vote" style="width: 30%; display: inline-block;">{$T_POLL_VOTES[votes_list].text}</span>
		           		<span class="poll_vote" style="width: 30%; display: inline-block;">
							...
							{$T_POLL_VOTES[votes_list].perc*100}%
						</span>
						{$T_POLL_VOTES.users}
						<ul>
							<li>Alunos desta opção</li>
						{foreach name = 'users_votes_list' item="login" from = $T_POLL_VOTES[votes_list].users}
							<li><a href="{$T_CURRENT_POLL_USER_TYPE}.php?ctg=module&op=module_xuser&action=edit_xuser&xuser_login={$login}">{$login}</a></li>
						{foreachelse}
