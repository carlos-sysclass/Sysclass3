...
{capture name = "center_code"}
	{if $T_MESSAGE && !$T_FACEBOOK_ACCOUNT_MERGE_POPUP}
		{sC_template_printMessageBlock content = $T_MESSAGE type = $T_MESSAGE_TYPE class="full"}
		<br />
	{/if}
    ...
    {elseif $smarty.get.ctg == 'reset_pwd'}
     {assign var = "title" value = "`$title`<span>&nbsp;&raquo;&nbsp;</span><a class='titleLink' href = '`$smarty.server.PHP_SELF`?ctg=reset_pwd'>`$smarty.const._RESETPASSWORD`</a>"}
        {*sC_template_printBlock title = $smarty.const._RESETPASSWORD content = $smarty.capture.t_reset_pwd_code image = "32x32/exclamation.png"*}
         {$smarty.capture.t_reset_pwd_code}
  ...
  {/if}
    ...
    {elseif $smarty.get.ctg == 'login'}
        ...
        {*sC_template_printBlock
        	 title = $smarty.const._LOGINENTRANCE 
        	 content = 
        	 image = "32x32/keys.png"
        	 contentclass="grid_8 prefix_8"
        *}
 ...
 {else}
     {if isset($T_POSITIONS.centerList)}
          ...
          {else if $T_CUSTOM_BLOCKS[$item].content}
           {insert name = "customBlock" file = "`$T_CUSTOM_BLOCKS[$item].name`.tpl" assign = "content"}
            {$content}
           {*sC_template_printBlock title = $T_BLOCKS[$item].title content = $content image = "`$T_BLOCKS[$item].image`" class="container_24" contentclass="grid_8 prefix_8"*}
          {/if}
            {/foreach}
        {else}
         {if $T_CONFIGURATION.lessons_directory == 1}
...
{if $smarty.get.ctg == 'agreement' && $smarty.session.s_login}
  ...
  {capture name = 'agreement_code'}
      <div class = "license"><textarea readonly="readonly" style="width: 100%; min-height: 500px;">{$T_CONFIGURATION.license_note}</textarea></div>
     ...
     {/capture}
     {sC_template_printBlock title = $smarty.const._IMPORTANTNOTICE content = $smarty.capture.agreement_code image = "32x32/exclamation.png" class="agreementBlock"}
     {assign var = "layoutClass" value = "hideBoth"}
    {/capture}
{/if}
 ...
 <div id = 'facebook_login' style = "display:none" align = "left">
  ...
  {capture name = 't_facebook_login'}
   {include file = "includes/blocks/facebook_login.tpl"}
     {/capture}
     {sC_template_printBlock title = $smarty.const._FACEBOOKCONNECT content = $smarty.capture.t_facebook_login image = "32x32/facebook.png"}
 </div>
{/if}
