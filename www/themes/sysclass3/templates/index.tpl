 ...
 {else}
     {if isset($T_POSITIONS.centerList)}
          ...
          {else if $T_CUSTOM_BLOCKS[$item].content}
           {insert name = "customBlock" file = "`$T_CUSTOM_BLOCKS[$item].name`.tpl" assign = "content"}
            {$content}
           {*eF_template_printBlock title = $T_BLOCKS[$item].title content = $content image = "`$T_BLOCKS[$item].image`" class="container_24" contentclass="grid_8 prefix_8"*}
          {/if}
            {/foreach}
        {else}
         {if $T_CONFIGURATION.lessons_directory == 1}
...
{if $smarty.get.ctg == 'agreement' && $smarty.session.s_login}
  ...
  {capture name = 'agreement_code'}
     ...
     {/capture}
     {eF_template_printBlock title = $smarty.const._IMPORTANTNOTICE content = $smarty.capture.agreement_code image = "32x32/exclamation.png" class="agreementBlock"}
     {assign var = "layoutClass" value = "hideBoth"}
    {/capture}
{/if}
