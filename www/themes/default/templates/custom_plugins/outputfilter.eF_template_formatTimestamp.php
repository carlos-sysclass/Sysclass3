...
*/
function smarty_outputfilter_sC_template_formatTimestamp($compiled, &$smarty)
{
    switch ($GLOBALS['configuration']['date_format']) {
        case "YYYY/MM/DD": $format = '%Y %b %d'; break;
        case "MM/DD/YYYY": $format = '%b %d %Y'; break;
        case "DD/MM/YYYY": default: $format = '%d/%m/%Y'; break;
    }
    ...
    $new = preg_replace("/#filter:timestamp_interval-(\d{9,10})#/e", "sC_convertIntervalToTime(time() - \$1, true)", $new);
