...
function smarty_outputfilter_sC_template_loginToMessageLink($compiled, &$smarty)
{
    if (!$GLOBALS['currentUser'] -> coreAccess['personal_messages'] || $GLOBALS['currentUser'] -> coreAccess['personal_messages'] != 'hidden') {
        $new = preg_replace("/#filter:user_login-(.*)#/U", "<span style = \"white-space:nowrap;font-weight:bold\"><a href = \"".basename($_SERVER['PHP_SELF'])."?ctg=messages&add=1&recipient=\$1&popup=1\" onclick = \"sC_js_showDivPopup('"._NEWMESSAGE."', 2)\" title=\"\\1\" target = \"POPUP_FRAME\">#filter:login-\$1#</a></span>", $compiled);
        $new = preg_replace("/#filter:user_loginNoIcon-(.*)#/U", "<a href = \"".basename($_SERVER['PHP_SELF'])."?ctg=messages&add=1&recipient=\$1&popup=1\" onclick = \"sC_js_showDivPopup('"._NEWMESSAGE."', 2)\" title=\"\\1\" target = \"POPUP_FRAME\">#filter:login-\\1#</a>", $new);
        $compiled = $new;
    } else {
