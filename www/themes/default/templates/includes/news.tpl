   ...
   <form {$T_ENTITY_FORM.attributes}>
       ...
       {$T_ENTITY_FORM.data.html}</td></tr>
           <tr><td class = "labelCell">{$smarty.const._FROM}:&nbsp;</td>
               <td class = "elementCell">{sC_template_html_select_date prefix="from_" time=$T_FROM_TIMESTAMP start_year="-1" end_year="+5" field_order = $T_DATE_FORMATGENERAL} {$smarty.const._TIME}: {html_select_time prefix="from_" time = $T_FROM_TIMESTAMP display_seconds = false}</td></tr>
           <tr><td class = "labelCell">{$smarty.const._TO}:&nbsp;</td>
               <td class = "elementCell">{sC_template_html_select_date prefix="to_" time=$T_TO_TIMESTAMP start_year="-1" end_year="+5" field_order = $T_DATE_FORMATGENERAL} {$smarty.const._TIME}: {html_select_time prefix="to_" time = $T_TO_TIMESTAMP display_seconds = false}</td></tr>
           <tr><td class = "labelCell">{$smarty.const._DISPLAYONCALENDAR}:&nbsp;</td>
               <td class = "elementCell">{sC_template_html_select_date prefix="calendar_" time=$T_CALENDAR_TIMESTAMP start_year="-1" end_year="+5" field_order = $T_DATE_FORMATGENERAL} {$smarty.const._TIME}: {html_select_time prefix="to_" time = $T_TO_TIMESTAMP display_seconds = false}</td></tr>
...
*}
  ...
  {sC_template_printBlock title = $smarty.const._ANNOUNCEMENT data = $smarty.capture.t_add_code image = '32x32/announcements.png' help ='Announcements'}
 {elseif $smarty.get.view}
  {sC_template_printBlock title = $T_NEWS.title data = $T_NEWS.data image = '32x32/announcements.png' help ='Announcements'}
 {else}
     {capture name = "t_news_code"}
         {if !$_student_ && $_change_}
             <div class = "headerTools">
                 ...
                 <a href = "{$smarty.server.PHP_SELF}?ctg=news&add=1&popup=1" onclick = "sC_js_showDivPopup('{$smarty.const._ANNOUNCEMENTADD}', 2)" title = "{$smarty.const._ANNOUNCEMENTADD}" target = "POPUP_FRAME">{$smarty.const._ANNOUNCEMENTADD}</a>
             </div>
         {/if}
          ...
          {if $smarty.session.s_type != 'student' && $_change_}
              <td class = "centerAlign">
               {if $T_CURRENT_USER->user.login == $item.users_LOGIN || $T_CURRENT_USER->user.user_type == 'administrator'}
                   <a href = "{$smarty.server.PHP_SELF}?ctg=news&edit={$item.id}&popup=1" target = "POPUP_FRAME" onClick = "sC_js_showDivPopup('{$smarty.const._EDITANNOUNCEMENT}', 2);"><img src = "images/16x16/edit.png" alt = "{$smarty.const._EDIT}" title = "{$smarty.const._EDIT}" border = "0"/></a>&nbsp;
     ...
     {sC_template_printBlock title = $smarty.const._ANNOUNCEMENTS data = $smarty.capture.t_news_code image = '32x32/announcements.png' help = 'Announcements'}
 {/if}
 </td></tr>
{/capture}
