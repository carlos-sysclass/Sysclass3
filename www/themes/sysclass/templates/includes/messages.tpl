 ...
 {elseif $smarty.get.add}
  ...
  {/if}
        ...
        {$T_ADD_MESSAGE_FORM.hidden}
	        {if $T_DESTINATARYALREADYSELECTED}
				{eF_template_printBlock title = $smarty.const._RECIPIENTSSELECTION data = $smarty.capture.t_recipients_code image = '32x32/directory.png' help = 'Messages' style= 'display: none;'}
				{eF_template_printBlock title = $smarty.const._DESTINATARYSELECTIONTITLE data = $T_DESTINATARYSELECTION image = '32x32/directory.png' help = 'Messages'}
         ...
         {if $T_DESTINATARYALREADYSELECTED}
	        ...
	        var specific_recipient = '{$T_DESTINATARYALREADYSELECTED}';
	        {literal}
		        $j(document).ready(function () {
		        	 eF_js_selectRecipients(specific_recipient);
				});
			{/literal}
	         </script>
         {/if}
