...
{* <tr><td class = "labelCell">{$T_COURSE_FORM.course_code.label}:&nbsp;</td>
   ...
   {if $T_CONFIGURATION.disable_payments != 1}
     ...
     <tr><td class = "labelCell">{$T_COURSE_FORM.enable_presencial.label}:&nbsp;</td>
      <td class = "elementCell">{$T_COURSE_FORM.enable_presencial.html}</td></tr>

     <tr><td class = "labelCell">{$T_COURSE_FORM.price_presencial.label}:&nbsp;</td>
      <td class = "elementCell">{$T_COURSE_FORM.price_presencial.html} {$T_CURRENCYSYMBOLS[$T_CONFIGURATION.currency]}</td></tr>

     <tr><td class = "labelCell">{$T_COURSE_FORM.enable_web.label}:&nbsp;</td>
      <td class = "elementCell">{$T_COURSE_FORM.enable_web.html}</td></tr>

     <tr><td class = "labelCell">{$T_COURSE_FORM.price_web.label}:&nbsp;</td>
      <td class = "elementCell">{$T_COURSE_FORM.price_web.html} {$T_CURRENCYSYMBOLS[$T_CONFIGURATION.currency]}</td></tr>
   {/if}
     <tr><td></td>
  ...
  {/capture}
	{capture name = 't_classes_to_courses_code'}
    <div class = "headerTools">
		<span>
			<img src = "images/16x16/add.png" title = "{$smarty.const._NEWCOURSECLASS}" alt = "{$smarty.const._NEWCOURSECLASS}">
			<a 
				target="POPUP_FRAME"
    			title = "{$smarty.const._NEWCOURSECLASS}" 
    			onclick="sC_js_showDivPopup('{$smarty.const._NEWCOURSECLASS}', 1)" 
    			href="{$smarty.server.PHP_SELF}?ctg=courses&courseclass={$T_EDIT_COURSE->course.id}&add_courseclass=1&popup=1">
    			{$smarty.const._NEWCOURSECLASS}
   			</a>
		</span>
	</div>
		{if !$T_SORTED_TABLE || $T_SORTED_TABLE == 'classesTable'}
			{include file="includes/courses/classes.list.tpl"}
		{/if}
	{/capture}
    ...
    <div style="display: none;">
      <input class = "inputCheckbox" type = "checkbox" name = "checked_{$login}" id = "checked_{$login}" onclick = "usersAjaxPost('{$login}', this);" {if !$user.active_in_course|@is_null}checked = "checked"{/if} />{if !$user.active_in_course|@is_null}<span style = "display:none">checked</span>{/if}
		...
		<img 
    		...
    		onclick = "sC_js_setTableUrl('classesuserTable', '{$smarty.server.PHP_SELF}?ctg=courses&edit_course={$smarty.get.edit_course}&login={$login}&'); sC_js_redrawPage('classesuserTable', true); sC_js_showDivPopup('{$smarty.const._SELECTCLASSESCOURSE}', 2, 'user_class_select_dialog')"/>
...
<!--ajax:classesuserTable-->
			<table style = "width:100%" class = "sortedTable" size = "{$T_TABLE_SIZE}" sortBy = "4" order = "desc" useAjax = "1" id = "classesuserTable" rowsPerPage = "{$smarty.const.G_DEFAULT_TABLE_SIZE}" url = "{$smarty.server.PHP_SELF}?ctg=courses&edit_course={$smarty.get.edit_course}&">
				<tr class = "topTitle defaultRowHeight">
					<td class = "topTitle" name = "name">{$smarty.const._NAME} </td>
				...
				{foreach name = 'classes_list3' key = 'key' item = 'classe' from = $T_DATA_SOURCE}
					<tr class = "defaultRowHeight {cycle values = "oddRowColor, evenRowColor"} {if !$classe.active}deactivatedTableElement{/if}">
						...
						<td class = "centerAlign">
							<input type = "checkbox" id = "{$classe.id}" onclick = "usersCourseClassAjaxPost('{$smarty.get.login}', '{$classe.id}', this);" {if $classe.user_has_class != "0"}checked{/if}>{if $classe.user_has_class != "0"}<span style = "display:none">checked</span>{/if}
						</td>
					</tr>
				{foreachelse}
					<tr class = "defaultRowHeight oddRowColor"><td class = "emptyCategory" colspan = "100%">{$smarty.const._NODATAFOUND}</td></tr>
...
<!--/ajax:classesuserTable-->
  ...
  {capture name = 't_course_code'}
   ...
   {else}
    ...
    {assign var = "classes_block_title" value = $smarty.const._EDITCLASSESCOURSE}
    {assign var = "classes_block_select_title" value = $smarty.const._SELECTCLASSESCOURSE}
   ...
   {if $smarty.get.edit_course}
    ...
    {sC_template_printBlock tabber = "classes" title ="`$classes_block_title`" data = $smarty.capture.t_classes_to_courses_code image = '32x32/classes.png'}
   ...
   <div id="user_class_select_dialog" style="display: none;">
   	{sC_template_printBlock tabber = "classes_users" title = "`$classes_block_select_title`" data = $smarty.capture.t_users_to_classes_dialog_code image = '32x32/classes.png'}
   </div>
  ...
  {/if}
 {elseif $smarty.get.courseclass}
 	{capture name = "t_courseclass_form"}
 		{include file="includes/courses/classes.form.tpl"}

     {/capture}
     {sC_template_printBlock title = $T_COURSECLASS_FORM_TITLE data  = $smarty.capture.t_courseclass_form image = '32x32/courseclass.png'}
