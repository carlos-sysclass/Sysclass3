...
<head>
    ...
    <link rel = "stylesheet" type = "text/css" href = "{$smarty.const.G_CURRENTTHEMEURL}css/all.css">
    <link rel = "stylesheet" type = "text/css" href = "{$smarty.const.G_CURRENTTHEMEURL}css/theme/switcher.css">
    <link rel = "stylesheet" type = "text/css" href = "{$smarty.const.G_CURRENTTHEMEURL}css/adminica.reset.css">
    ...
    {if $T_HEADER_LOAD_STYLESHEETS}
	    <link rel = "stylesheet" type = "text/css" href = "css/stylesheets.php?build={$smarty.const.G_BUILD}&load={$T_HEADER_LOAD_STYLESHEETS}" />
    {/if}
  ...
  {assign var='sitemotto' value=$T_CONFIGURATION.site_motto|replace:'"':'\"'}
        top.document.title = "{if $T_CONFIGURATION.site_name}{$sitename}{else}{$smarty.const._MAGESTERNAME}{/if} | {if $T_TITLE_BAR}{$T_TITLE_BAR}{else}{if $T_CONFIGURATION.site_motto}{$sitemotto}{else}{$smarty.const._THENEWFORMOFADDITIVELEARNING}{/if}{/if}";
...
{if $smarty.server.PHP_SELF|@basename != 'new_sidebar.php'}
	<body {if isset($T_CURRENT_CTG)}id = "body_{$T_CURRENT_CTG}"{/if} onkeypress = "if (window.sC_js_keypress) sC_js_keypress(event);" onbeforeunload = "if (window.getPeriodicData) getPeriodicData();">
{/if}