...
function smarty_outputfilter_sC_template_formatDatetime($compiled, &$smarty)
{
    ...
    $ext_date_format = "%d de %B";
    $ext_datetime_format = $ext_date_format . " às %R";
    ...
    $new = preg_replace("/#filter:ext-date-(.*?)#/e", "iconv(_CHARSET, 'UTF-8', strftime('$ext_date_format', strtotime('\$1')))", $new);
    $new = preg_replace("/#filter:ext-datetime-(.*?)#/e", "iconv(_CHARSET, 'UTF-8', strftime('$ext_datetime_format', strtotime('\$1')))", $new);
