<?php
/**
 * This file lets you to make requests to SysClass and get some useful information in xml format.
 * You need to be an administrator to use SysClass API. First of all you need a token that must be used
 ...
 *
 * /api2.php?token=<token>&action=login&username=<login>&password=<password>
 *         logs <login> in SysClass API (<login> must be an administrator account)
 *
 * /api2.php?token=<token>&action=magesterlogin&login=<login>
 *         logs <login> in SysClass
 *
 * /api2.php?token=<token>&action=create_lesson&name=<name>&category=<category_id>&course_only=<course_only>&language=<language>&price=<price>
 *         creates a new lesson with corresponding fields
 *
 * /api2.php?token=<token>&action=create_user&login=<login>&password=<password>&email=<email>&languages=<languages>&name=<name>&surname<surname>
 *         creates a new user with corresponding fields
 *
 * /api2.php?token=<token>&action=update_user&login=<login>&password=<password>&email=<email>&name=<name>&surname<surname>
 *         updates a user profile with corresponding fields
 *
 * /api2.php?token=<token>&action=deactivate_user&login=<login>
 *         deactivates user <login>
 *
 * /api2.php?token=<token>&action=activate_user&login=<login>
 *         activates user <login>
 *
 * /api2.php?token=<token>&action=remove_user&login=<login>
 *         deletes user <login>
 *
 * /api2.php?token=<token>&action=groups
 *         returns all groups defined in SysClass
 *
 * /api2.php?token=<token>&action=group_info&group=<group_id>
 *         returns <group_id> information
 *
 * /api2.php?token=<token>&action=group_to_user&login=<login>&group=<group_id>
 *         assigns group with <group_id> to user <login>
 *
 * /api2.php?token=<token>&action=group_from_user&login=<login>&lesson=<group_id>
 *         undo assignment for group with <group_id> to user <login>
 *
 * /api2.php?token=<token>&action=lesson_to_user&login=<login>&lesson=<lesson_id>&type=<user_type>
 *         assigns lesson with <lesson_id> to user <login> with role <user_type>
 *
 * /api2.php?token=<token>&action=activate_user_lesson&login=<login>&lesson=<lesson_id>
 *         activate assignment for lesson with <lesson_id> to user <login>
 *
 * /api2.php?token=<token>&action=deactivate_user_lesson&login=<login>&lesson=<lesson_id>
 *         deactivate assignment for lesson with <lesson_id> to user <login>
 *
 * /api2.php?token=<token>&action=lesson_from_user&login=<login>&lesson=<lesson_id>
 *         undo assignment for lesson with <lesson_id> to user <login>
 *
 * /api2.php?token=<token>&action=course_to_user&login=<login>&course=<course_id>&type=<user_type>
 *         assigns course with <course_id> to user <login> with role <user_type>
 *
 * /api2.php?token=<token>&action=course_from_user&login=<login>&courses=<course_id>
 *         undo assignment for course with <course_id> to user <login>
 *
 * /api2.php?token=<token>&action=user_lessons&login=<login>
 *         returns the lessons that are assigned to the user <login>
 *
 * /api2.php?token=<token>&action=user_courses&login=<login>
 *         returns the courses that are assigned to the user <login>
 *
 * /api2.php?token=<token>&action=lesson_info&lesson=<lesson_id>
 *         returns <lesson_id> information
 *
 * /api2.php?token=<token>&action=user_info&login=<login>
 *         returns <login> information
 *
 * /api2.php?token=<token>&action=lessons
 *         returns all lessons defined in SysClass
 *
 * /api2.php?token=<token>&action=courses
 *         returns all courses defined in SysClass
 *
 * /api2.php?token=<token>&action=course_info&course=<course_id>
 *         returns <course_id> information
 *
 * /api2.php?token=<token>&action=course_lessons&course=<course_id>
 *         returns lessons containing in <course_id>
 *
 * /api2.php?token=<token>&action=activate_user_course&login=<login>&course=<course_id>
 *         activate assignment for all lessons within <course_id> to user <login>
 *
 * /api2.php?token=<token>&action=deactivate_user_course&login=<login>&course=<course_id>
 *         deactivate assignment for all lessons within <course_id> to user <login>
 *
 * /api2.php?token=<token>&action=catalog
 *         returns the list with all courses and lessons of the system
 *
 * /api2.php?token=<token>&action=logout
 *         logs out from SysClass API
 *
 * /api2.php?token=<token>&action=curriculum_to_user&login=<login>&curriculum=<curriculum_id>
 *         assigns curriculum with <curriculum_id> to user <login>
 *
 *
 * API returns xml corresponding to the action argument. For actions like magesterlogin,
 * activate_user etc it returns a status entity ("ok" or "error").
        ...
        switch($_GET['action']) {
            ...
            case 'login':
                if (isset($_GET['username']) && isset($_GET['password']) && isset($_GET['token'])) {
                    try {
                        ...
                        if ($e->getCode() == MagesterUserException :: INVALID_PARAMETER) {
                            echo "<xml>";
                            echo "<status>error</status>";
                    ...
                    if ($user->user['user_type'] == "administrator") {
                        ...
                        echo "<xml>";
                        echo "<status>error</status>";
                        echo "<message>You must have an administrator account to login to SysClass API</message>";
                        echo "</xml>";
                    }
                } else {
                    echo "<xml>";
            ...
            case 'create_lesson':
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    if (isset($_GET['name']) && isset($_GET['category']) && isset($_GET['course_only']) && isset($_GET['language'])) {
                        ...
                        try {
                            $newLesson = MagesterLesson :: createLesson($fields); //$newLesson is now a new lesson object
                            echo "<xml>";
                            echo "<status>ok</status>";
                            echo "</xml>";
                        } catch (Exception $e) {
                ...
                break;
            case 'generate_login_and_password':
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    if (isset($_GET['name']) && isset($_GET['surname'])) {
                        ...
                        try {
                            ...
                            $login		= MagesterUser::generateNewLogin($insert['name'], $insert['surname']);
                            $password 	= MagesterUser::generateMD5Password();
            ...
            case 'create_user':
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    if (isset($_GET['login']) && isset($_GET['password']) && isset($_GET['email']) && isset($_GET['languages']) && isset($_GET['name']) && isset($_GET['surname'])) {
                        ...
                        $insert['name'] = $_GET['name'];
                        $insert['surname'] = $_GET['surname'];
                        $insert['user_type'] = isset($_GET['user_type']) ? $_GET['user_type'] : 'student';
                        ...
                        try {
                            // DELETING USER BEFORE INSERT, ONLY FOR TEST
                            ...
                            if ($e->getCode() == MagesterUserException :: INVALID_LOGIN) {
                                echo "<xml>";
                                echo "<status>error</status>";
                                echo "<message>Invalid login format</message>";
                                echo "</xml>";
                                exit;
                            } elseif ($e->getCode() == MagesterUserException :: MAXIMUM_REACHED) {
                                echo "<xml>";
                                echo "<status>error</status>";
                                echo "<message>Maximum number of users reached</message>";
                                echo "</xml>";
                                exit;
                            } elseif ($e->getCode() == MagesterUserException :: USER_EXISTS) {
                                echo "<xml>";
                                echo "<status>error</status>";
                                echo "<message>User already exists</message>";
                                echo "</xml>";
                                exit;
                            } elseif ($e->getCode() == MagesterUserException :: INVALID_PARAMETER) {
                                echo "<xml>";
                                echo "<status>error</status>";
            ...
            case 'create_user_details':
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    ...
                    try {
                        MagesterUserDetails :: injectDetails($values['login'], $user_details);
                ...
                break;
            case 'link_user_with_student_login':
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    $values = $_GET;
                    $userData = array(
                        'login'			=> $values['login'],
                        'student_login'	=> $values['student_login']
                    );
                    try {
                        $user = MagesterUserFactory :: factory($userData['login']);

                        if ($user instanceof MagesterResponsible) {
                            if ($user->linkWithStudent($userData['student_login'])) {
                                echo "<xml>";
                                echo "<status>ok</status>";
                                echo "</xml>";
                                exit;
                ...
                break;
            case 'register_user_payment':
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    $values = $_GET;

                    $modules = $currentUser->getModules();

                    $send_emails = ($values['send_emails'] == 'false') ? false : true;
                    try {
                        $result = $modules['module_pagamento']->registerPayment($values['login'], $values['courses'], $values['payment_type'], $send_emails);

                        echo "<xml>";
                        echo "<status>ok</status>";
                        echo '<result>';
                        foreach ($result as $key => $data) {
                            echo sprintf('<%1$s>%2$s</%1$s>', $key, $data);
                        }
                        echo '</result>';
                        echo "</xml>";
                    } catch (Exception $e) {
                ...
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    if (isset($_GET['login']) && isset($_GET['password']) && isset($_GET['email']) && isset($_GET['name']) && isset($_GET['surname'])) {
                        ...
                        try {
                            $user = MagesterUserFactory :: factory($_GET['login']);
                            //pr($user);	exit;
                            $user->user['password'] = MagesterUser::createPassword($_GET['password']);
                            ...
                            if ($e->getCode() == MagesterUserException :: INVALID_PARAMETER) {
                                ...
                                echo "</xml>";
                                exit;
                            } elseif ($e->getCode() == MagesterUserException :: USER_NOT_EXISTS) {
                                echo "<xml>";
                                echo "<status>error</status>";
            ...
            case 'groups':
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    $groups = MagesterGroup::getGroups(true, true);
                    echo "<xml>";
                    echo "<groups>";
            ...
            case 'group_to_user':
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    if (isset($_GET['login']) && isset($_GET['group'])) {
                        ...
                        try {
                            $group = new MagesterGroup($_GET['group']);
                            $group->addUsers(array($_GET['login']));
                            echo "<xml>";
                            echo "<status>ok</status>";
                            echo "</xml>";
                        } catch (Exception $e) {
                            if ($e->getCode() == MagesterGroupException :: USER_ALREADY_MEMBER) {
                                echo "<xml>";
                                echo "<status>error</status>";
                ...
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    if (isset($_GET['login']) && isset($_GET['group'])) {
                        ...
                        try {
                            ...
                            if ($e->getCode() == MagesterUserException :: USER_NOT_EXISTS) {
                                echo "<xml>";
                                echo "<status>error</status>";
                                echo "<message>User does not exist</message>";
                                echo "</xml>";
            ...
            case 'lesson_to_user':
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    if (isset($_GET['login']) && isset($_GET['lesson'])) {
                        ...
                        try {
                            $lesson = new MagesterLesson($_GET['lesson']);
                            $user = MagesterUserFactory :: factory($_GET['login']);
                            ...
                            if ($e->getCode() == MagesterUserException :: USER_NOT_EXISTS) {
                                ...
                                echo "</xml>";
                                exit;
                            } elseif ($e->getCode() == MagesterCourseException :: MAX_USERS_LIMIT) {
                                echo "<xml>";
                                echo "<status>error</status>";
                                echo "<message>Maximum number of users reached</message>";
                                echo "</xml>";
                ...
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    ...
                    foreach ($courses as $key => $course) {
                        ...
                        foreach ($course_classes as $classKey => $classItem) {
                            echo "<class>";
                            echo "<id>".$classItem['id']."</id>";
                            echo "<name>".$classItem['name']."</name>";
                            echo "<courses_ID>".$classItem['courses_ID']."</courses_ID>";
                            echo "<active>".$classItem['active']."</active>";
                            echo "<language>".$classItem['languages_NAME']."</language>";
                            echo "<max_users>".$classItem['max_users']."</max_users>";
                            echo "<count_users>".$classItem['count_users']."</count_users>";
                            echo "<start_date>".$classItem['start_date']."</start_date>";
                            echo "<end_date>".$classItem['end_date']."</end_date>";
                            // GET SCHEDULES FOR CLASS
                            echo "<schedules>";
                            foreach ($classItem['schedules'] as $scheduleItem) {
                                echo "<schedule>";
                                echo "<id>".$scheduleItem['id']."</id>";
                                echo "<week_day>".$scheduleItem['week_day']."</week_day>";
                                echo "<start>".$scheduleItem['start']."</start>";
                                echo "<end>".$scheduleItem['end']."</end>";
                                echo "</schedule>";
                            }
                            echo "</schedules>";
                ...
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    if (isset($_GET['login']) && isset($_GET['course'])) {
                        ...
                        try {
                            $course = new MagesterCourse($_GET['course']);
                            $user = MagesterUserFactory :: factory($_GET['login']);
                            $_GET['type'] != "professor" && $_GET['type'] != 'student' && $_GET['type'] != '6' ? $_GET['type'] = 'student' : null;
                            $_GET['confirmed'] == "false" ? $_GET['confirmed'] = false : $_GET["confirmed"] = true;
                            if (($course->isStudentInCourse($_GET['login']) === true && ($_GET['type'] == 'student' || $_GET['type'] == '6')) || ($course->isProfessorInCourse($_GET['login']) === true && $_GET['type'] == 'professor')) {
                                echo "<xml>";
                                echo "<status>error</status>";
                                echo "<message>Assignment already exists</message>";
                                echo "</xml>";
                            ...
                            $course ->addUsers($_GET['login'], $_GET['type'], $_GET["confirmed"], $_GET["course_type"]);
                            ...
                            if ($e->getCode() == MagesterUserException :: USER_NOT_EXISTS) {
                                ...
                                echo "</xml>";
                                exit;
                            } elseif ($e->getCode() == MagesterCourseException :: COURSE_NOT_EXISTS) {
                                echo "<xml>";
                                echo "<status>error</status>";
                                echo "<message>Course doesn't exist</message>";
                                echo "</xml>";
                ...
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    if (isset($_GET['login']) && isset($_GET['course'])) {
                        ...
                        try {
                            ...
                            if ($e->getCode() == MagesterCourseException :: COURSE_NOT_EXISTS) {
                                echo "<xml>";
                                echo "<status>error</status>";
                                echo "<message>Course doesn't exist</message>";
                                echo "</xml>";
                ...
                if (isset($_GET['token']) && checkToken($_GET['token'])) {
                    if (isset($_GET['login']) && isset($_GET['curriculum'])) {
                        ...
                        try {
                            ...
                            if ($e->getCode() == MagesterCurriculumException :: EMPTY_COURSE) {
                                echo "<xml>";
                                echo "<status>error</status>";
...
function checkToken($token)
{
    ...
    if ($token == 'logged') {
        global $currentUser;
        $currentUser = MagesterUserFactory::factory($tmp[0]['users_LOGIN']);
        return true;
    } else {
        return false;
    }
