...
if ($form -> isSubmitted()) {
	if ($form -> validate()) {
     ...
     /*
					 ...
					 * echo "<script>var mine = window.open('api.php?action=magesterlogin&token=".$token."&login=".$login."', 'api', 'width=1,height=1,left=0,top=0,scrollbars=no');</script>";
					 ...
					 *			<script>new Ajax.Request("api.php?action=magesterlogin&token='.$token.'&login=professor")</script>';

					 */
                    if ($stream = fopen(G_SERVERNAME.'api.php?action=magesterlogin&token='.$token.'&login='.$login, 'r')) {
                        $output = stream_get_contents($stream);
                        fclose($stream);
                    }
                    //echo "<script>var mine = window.open('api.php?action=magesterlogin&token=".$token."&login=".$login."', 'api', 'width=1,height=1,left=0,top=0,scrollbars=no');</script>";
    ...
    case 'magesterlogout':{
                    $token = $values['token'];
     $login = $values['login'];
                    if ($stream = fopen(G_SERVERNAME.'api.php?action=magesterlogout&token='.$token.'&login='.$login, 'r')) {
                        $output = stream_get_contents($stream);
                        fclose($stream);
                    }
                    break;
